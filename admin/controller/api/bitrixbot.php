...
class ControllerApiBitrixBot extends Controller { 
	...
	public function index(){
		$this->bitrixLog = new Log('bitrix24-bot.log');
		...
		if ($this->request->request['event'] == 'ONIMBOTDELETE') {
			...
			$result = $this->Bitrix24->restCommand('imbot.register', array(
				...
				'PROPERTIES'            => array(
					...
					'PERSONAL_WWW'      => '',				
					'PERSONAL_GENDER'   => 'M',				
					'PERSONAL_PHOTO'    => base64_encode(file_get_contents( DIR_CATALOG . 'icon/bitrixbotlogo.png' )),		
				),
			),
			...
			$result = $this->Bitrix24->restCommand('imbot.update', Array(
				...
				'FIELDS' => Array(		            
					'PROPERTIES' => Array(
						...
						'PERSONAL_PHOTO' => base64_encode(file_get_contents( __DIR__ .'/botlogo.png')), 	
					)
				)
			...
			$result = $this->Bitrix24->restCommand('imbot.command.register', Array(				
				...
				'LANG' => Array(
					Array(
						...
						'PARAMS' => 'Код/артикул/штрихкод товара'),
				),
			...
			$result = $this->Bitrix24->restCommand('imbot.command.register', Array(				
				...
				'LANG' => Array(
					Array(
						...
						'PARAMS' => 'Ссылка, которую необходимо укоротить'),
				),
				'EVENT_COMMAND_ADD' => $handlerBackUrl,

			), $this->request->request["auth"]);

			$this->bitrixLog->write($result);				
			...
			$this->bitrixLog->write('ControllerApiBitrixBot::index::data: ' . json_encode($this->request->request['data']));
			$this->parseCommand($this->request->request['data']['COMMAND']);			
		} 
	}
	...
	public function parseCommand($commands = [], $param = ''){
		$this->load->model('kp/bitrixBot');
			...
			if ($command['COMMAND'] == 'нал')
			{
				...
				try {
					...
					if (!$params || !$params_array){
						...
						$this->bitrixLog->write('ControllerApiBitrixBot::order1C::' . json_encode($order1c));
					...
					$this->bitrixLog->write('ControllerApiBitrixBot::parseCommand::reply::' . $reply);					
					$this->bitrixLog->write(serialize($attach));
