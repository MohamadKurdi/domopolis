...
class ControllerCatalogProduct extends Controller {
	...
	public function ob_category() {
		...
		public function getOptimalPrice () {
			...
			$resultArray['local_suppliers'] = array();
		...
		//	var_dump($this->data['product_additional_offer']);
			...
			if (isset($this->request->post['product_price_national_to_yam'])) {
				...
				$this->data['product_price_national_to_yam'] = array();
			}
			...
			if (isset($this->request->post['product_price_to_store'])) {
				...
				$this->data['product_price_to_store'] = array();
			}
			...
			if (isset($this->request->post['product_price_national_to_store'])) {
				...
				$this->data['product_price_national_to_store'] = array();
			}
			...
			if (isset($this->request->post['product_stock_limits'])) {
				...
				$this->data['product_stock_limits'] = array();
			}
			...
			if (isset($this->request->post['product_stock_status'])) {
				...
				$this->data['product_stock_status'] = array();
			}
			...
			//get options for virtual options
			$data = array(
				'start' => 0,
				'limit' => 20000
			);
		...
		protected function validateForm() {
			...
			/*
				...
				if ($language_id != $de_language_id) {
				
				if ((utf8_strlen($value['name']) < 1) || (utf8_strlen($value['name']) > 255)) {
				$this->request->post['product_description'][$language_id]['name'] = $temp_name;
			...
			*/
			...
			public function updateProductStockLimitAjax(){
				...
				if ($type == 'min'){
					$this->db->query("INSERT INTO product_stock_limits (product_id, store_id, min_stock)
						VALUES ('" .(int)$product_id. "', '" .(int)$store_id. "', '" .(int)$value. "') 
							ON DUPLICATE KEY UPDATE min_stock = '" .(int)$value. "'
							");
					}

					if ($type == 'rec'){
						$this->db->query("INSERT INTO product_stock_limits (product_id, store_id, rec_stock)
							VALUES ('" .(int)$product_id. "', '" .(int)$store_id. "', '" .(int)$value. "') 
								ON DUPLICATE KEY UPDATE rec_stock = '" .(int)$value. "'
								");
						}
					}
