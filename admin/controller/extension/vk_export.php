    ...
    public function check_app_security() {
		...
		try {
			...
			if ($e->getCode() == 17) {
                ...
                // вариант 2 (20.09.2014)
				$exp2 = '/<span class="field_prefix">([\d\s-+]*)<\/span>[\n\s]*';
				$exp2 .= '<input type="text" class="textfield field_inline" name="code" style="width:75px;" \/>[\n\s]*';
				$exp2 .= '<span class="field_prefix">(?:&nbsp;)*([\s\d-]*)<\/span>/';
				if ((preg_match($exp, $page, $matches) || preg_match($exp2, $page, $matches)) && preg_match('/action="([^"]*?)"/', $page, $url)) {
                    ...
                    $log_file = DIR_SYSTEM . 'logs/' . '.vkExportLog_appcheck_phone_' . date('d.m.y_H.i.s') . '_' . substr(md5(microtime()), 0, 7);
                    file_put_contents($log_file, $page);
                }
			}
			else {
				$response = $e;
