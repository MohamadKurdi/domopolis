...
class ControllerKPSalary extends Controller {
	...
	public $complete_cancel_percent_params 	= [];
	public $average_confirm_time_params 	= [];
	public $average_process_time_params 	= [];

	public $percentage_params 				= [];
	public $fixed_salary 					= 0;

	public $head_percentage_params 	= [];
	public $head_fixed_salary 		= 0;
	...
	private function setCoefficients(){
		$this->load->model('kp/work');

		$this->complete_cancel_percent_params 	= $this->model_kp_work->getKPIArrayParams('complete_cancel_percent_params');
		$this->average_confirm_time_params 		= $this->model_kp_work->getKPIArrayParams('average_confirm_time_params');
		$this->average_process_time_params 		= $this->model_kp_work->getKPIArrayParams('average_process_time_params');
		$this->percentage_params				= $this->model_kp_work->getKPIArrayParams('percentage_params');
		$this->head_percentage_params 			= $this->model_kp_work->getKPIArrayParams('head_percentage_params');		 	

		$this->fixed_salary 		= (float)$this->config->get('config_kpi_fixed_salary');	
		$this->head_fixed_salary 	= (float)$this->config->get('config_kpi_head_fixed_salary');	 	
	}

	private function reloadCoefficients($manager_id){
	...
	public function getBrandManagerReport($data = array()){
		...
		$days_between 	= dateDifference($date_from, $date_to);
		$brands = array();
	...
	public function getManagerKPIFor1C($info_type, $month, $year){
		...
		if ($data['info_type'] == 'getvalues') {
			...
			foreach ($all_managers as $one_manager) {
				$this->reloadCoefficients($one_manager['user_id']);
	...
	public function alreadyPaidFor($order_id, $return_parties = false, $filter_dzioev_bug = false, $filter_count_time = false, $filter_count_time_days = false){
		...
		if ($filter_count_time) {
			...
			if (dateDiff($times['date_accepted'], $times['date_closed']) > $filter_count_time_days) {
	...
	public function countManagers($param_filter_month = false, $param_filter_year = false, $param_filter_manager = false, $param_return_data = false, $param_filter_dzioev_bug = false, $param_filter_count_time = false, $param_filter_count_time_days = false){
		...
		$this->reloadCoefficients($filter_manager);
		...
		if (empty($this->request->get['filter_count_time_days'])) {
			$filter_count_time_days = (int)$this->config->get('config_kpi_default_filter_count_days');
		} else {
		...
		if ($is_headsales) {
			$owned_filter_link = $this->url->link('sale/order', 'token=' . $this->session->data['token'] . '&filter_date_added=' . $filter_year . '-' . $filter_month . '-01' . '&filter_date_added_to=' . $filter_year . '-' . $filter_month . '-' . date('t', strtotime($filter_year . '-' . $filter_month . '-01')));
		} else {
			$owned_filter_link = $this->url->link('sale/order', 'token=' . $this->session->data['token'] . '&filter_manager_id=' . $filter_manager . '&filter_date_added=' . $filter_year . '-' . $filter_month . '-01' . '&filter_date_added_to=' . $filter_year . '-' . $filter_month . '-' . date('t', strtotime($filter_year . '-' . $filter_month . '-01')));
		}

		$this->data['cstats'] = array(
			'total_days' => $this->model_kp_work->countDays($filter_manager, $filter_month, $filter_year),
			'total_time' => secToHRCyr($this->model_kp_work->countTimeDiffSum($filter_manager, $filter_month, $filter_year)),
			...
			'total_inbound_duration' 	=> secToHRCyr($this->model_kp_work->countFieldSum($filter_manager, 'inbound_call_duration', $filter_month, $filter_year)),
			'total_outbound' 		=> $this->model_kp_work->countFieldSum($filter_manager, 'outbound_call_count', $filter_month, $filter_year),
			'total_outbound_duration' 	=> secToHRCyr($this->model_kp_work->countFieldSum($filter_manager, 'outbound_call_duration', $filter_month, $filter_year)),
			...
			'total_owned_order_filter'  	=> $owned_filter_link,
		...
		if ($is_headsales) {
			$orders_results = $this->model_kp_work->getClosedOrdersForMonth(false, $filter_month, $filter_year);
		} else {
			$orders_results = $this->model_kp_work->getClosedOrdersForMonth($filter_manager, $filter_month, $filter_year);
		}
		...
		if ($orders_results) {
			foreach ($orders_results as $order_result) {
				$problems = array();
				$alerts = array();
				unset($used_unused_parties);
				unset($already_paid);

				$_rorder = $this->model_sale_order->getOrder($order_result['order_id']);
				...
				if (date('Y.m.d', strtotime($_rorder['date_added'])) >= date('Y.m.d', strtotime($order_result['date_added']))) {
					$problems[] = 'Быстрое закрытие';
				}

				$check_status_count = $this->model_sale_order->getOrderClosingStatusCount($order_result['order_id']);        			
				...
				if (dateDiff($times['date_added'], $times['date_accepted']) > 10) {
					$problems[] = 'Подтверждение более 10 дней (' . dateDiff($times['date_added'], $times['date_accepted']) . ')';
				}

				$total_confirm_time += dateDiff($times['date_added'], $times['date_accepted']);
				if (dateDiff($times['date_accepted'], $times['date_closed']) > (int)$this->config->get('config_kpi_default_filter_count_days_problem')) {
					$problems[] = 'Выполнение более '. (int)$this->config->get('config_kpi_default_filter_count_days_problem') .' дн. (' . dateDiff($times['date_accepted'], $times['date_closed']) . ')';
				}
				...
				$orders[$_rorder['store_id']]['orders'][] = array(
					...
					'date_accepted_diff' => dateDiff($times['date_added'], $times['date_accepted']),
					'date_aссepted_class' => tdClassRev(dateDiff($times['date_added'], $times['date_accepted']), 10, $filter_count_time_days, 42),
					'date_closed'       => date('Y.m.d', strtotime($times['date_closed'])),
					'date_closed_diff'  => dateDiff($times['date_added'], $times['date_closed']),
					'date_closed_class' => tdClassRev(dateDiff($times['date_added'], $times['date_closed']), 10, $filter_count_time_days, 42),
					'is_time_bad'       => $filter_count_time?(dateDiff($times['date_accepted'], $times['date_closed']) > $filter_count_time_days):false,
					'problems'          => $problems,
					'alerts'            => $alerts,
		...
		foreach ($countries as $country) {
			if ($country['country_id'] != 81) {
				if (!isset($total_good_sum[$country['name']])) {
					$total_good_sum[$country['name']] = array();
					$total_good_sum[$country['name']]['total'] = 0;
					$total_good_sum[$country['name']]['text'] = 0;
					$total_good_sum[$country['name']]['already_paid'] = 0;
					$total_good_sum[$country['name']]['left_to_pay'] = 0;
					$total_good_sum[$country['name']]['currency'] = '';
					$total_good_sum[$country['name']]['total_text'] = 0;
					$total_good_sum[$country['name']]['already_paid_text'] = 0;
					$total_good_sum[$country['name']]['left_to_pay_text'] = 0;
					$total_good_sum[$country['name']]['course'] = 0;
					$total_good_sum[$country['name']]['course_real'] = 0;
					$total_good_sum[$country['name']]['left_to_pay_text_uah'] = 0;
				}
			}
		}
		...
		$act = $orders_results?round($total_confirm_time / count($orders_results), 2):0;
		$apt = $orders_results?round($total_process_time / count($orders_results), 2):0;
		$kpi_params = array(
			'complete_cancel_percent'           => $ccp . '%',
			'complete_cancel_percent_clr'       => tdClassRev($ccp, $this->complete_cancel_percent_params[0], $this->complete_cancel_percent_params[1], $this->complete_cancel_percent_params[2]),
			'complete_cancel_percent_params'    => $this->complete_cancel_percent_params,
			'complete_cancel_percent_percent'   => countPercentByParam($ccp, $this->complete_cancel_percent_params, ($is_headsales)?$this->head_percentage_params:$this->percentage_params),
			'complete_cancel_percent_value'     => countValueByParam($das_total_in_uah, $ccp, $this->complete_cancel_percent_params, ($is_headsales)?$this->head_percentage_params:$this->percentage_params),
			'complete_cancel_percent_value_txt' => $this->currency->format(countValueByParam($das_total_in_uah, $ccp, $this->complete_cancel_percent_params, ($is_headsales)?$this->head_percentage_params:$this->percentage_params), 'UAH', 1),

			'average_confirm_time'        => $act,
			'average_confirm_time_clr'    => tdClassRev($act, $this->average_confirm_time_params[0], $this->average_confirm_time_params[1], $this->average_confirm_time_params[2]),
			'average_confirm_time_params' => $this->average_confirm_time_params,
			'average_confirm_time_percent'  => countPercentByParam($act, $this->average_confirm_time_params, ($is_headsales)?$this->head_percentage_params:$this->percentage_params),
			'average_confirm_time_value'    => countValueByParam($das_total_in_uah, $act, $this->average_confirm_time_params, ($is_headsales)?$this->head_percentage_params:$this->percentage_params),
			'average_confirm_time_value_txt' => $this->currency->format(countValueByParam($das_total_in_uah, $act, $this->average_confirm_time_params, ($is_headsales)?$this->head_percentage_params:$this->percentage_params), 'UAH', 1),

			'average_process_time'        	=> $apt,
			'average_process_time_clr'    	=> tdClassRev($apt, $this->average_process_time_params[0], $this->average_process_time_params[1], $this->average_process_time_params[2]),
			'average_process_time_params' 	=> $this->average_process_time_params,
			'average_process_time_percent'  => countPercentByParam($apt, $this->average_process_time_params, ($is_headsales)?$this->head_percentage_params:$this->percentage_params),
			'average_process_time_value'    => countValueByParam($das_total_in_uah, $apt, $this->average_process_time_params, ($is_headsales)?$this->head_percentage_params:$this->percentage_params),
			'average_process_time_value_txt' => $this->currency->format(countValueByParam($das_total_in_uah, $apt, $this->average_process_time_params, ($is_headsales)?$this->head_percentage_params:$this->percentage_params), 'UAH', 1),
		);
	...
	public function countCustomerService($param_filter_month = false, $param_filter_year = false, $param_filter_manager = false, $param_return_data = false){
		...
		foreach ($results as $result) {
			...
			$orders_results = $this->model_sale_coupon->getGoodCouponOrdersForMonth($result['code'], $filter_month, $filter_year);
			...
			$this->data['promocodes'][] = array(
				...
				'cao_class' 			=> $actiontemplate_count?tdClass(min(round(($actiontemplate_opened / $actiontemplate_count) * 100), 100), 40, 60, 100):'',
				...
				'cac_class' 			=> $actiontemplate_count?tdClass(min(round(($actiontemplate_clicked / $actiontemplate_count) * 100), 100), 40, 60, 100):'',
				'usage_good'      		=> $usage_good,
				'conv_usage_good' 		=> $actiontemplate_count?min(round(($usage_good / $actiontemplate_count_total) * 100, 2), 100):0,
				'ug_class' 			=> $actiontemplate_count_total?tdClass(min(round(($usage_good / $actiontemplate_count_total) * 100, 2), 100), 1, 5, 10):'',
				'usage_bad'      		=> $usage_bad,
				'conv_usage_bad' 		=> $actiontemplate_count?min(round(($usage_bad / $actiontemplate_count_total) * 100, 2), 100):0,
				'ub_class' 			=> $actiontemplate_count_total?tdClass(min(round(($usage_bad / $actiontemplate_count_total) * 100, 2), 100), 1, 5, 10):'',
				...
				'ubm_class' 			=> $actiontemplate_count?tdClass(min(round(($usage_bad_month / $actiontemplate_count) * 100, 2), 100), 1, 5, 10):'',
				'orders' 			=> $orders,
			);
