...
class ControllerModulecustomproduct extends Controller {
    ...
    public function index() {   
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {            
            ...
            $this->redirect($this->url->link('module/customproduct', 'token=' . $this->session->data['token'], 'SSL'));
        }
        ...
        if (isset($this->error['image'])) {
          ...
          $this->data['breadcrumbs'] = array();
           ...
           $this->data['breadcrumbs'][] = array(
            ...
            'href'      => $this->url->link('module/customproduct', 'token=' . $this->session->data['token']),
              'separator' => ' :: '
           );
        
        $this->data['action'] = $this->url->link('module/customproduct', 'token=' . $this->session->data['token']);
		...
		foreach ($this->data['modules'] as &$module){
			if (isset($module['image2']) && $module['image2']){
				$module['thumb2'] = $this->model_tool_image->resize($module['image2'], 100, 100);
			} else {
				$module['thumb2'] = $this->data['no_image'];
			}		
		}
