...
class ControllerSaleSupplier extends Controller
{
    ...
    public function field_manufacturer()
    {
        $this->load->model('sale/supplier');

        if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
            $this->model_sale_supplier->updateSupplierManufacturerField($this->request->post['supplier_manufacturer_id'], $this->request->post['field'], $this->request->post['value']);
        }
    }
    ...
    public function clear_manufacturers($supplier_id = null)
    {
        $this->load->model('sale/supplier');
        ...
        if ($supplier && $supplier['parser']) {
            $this->supplierAdaptor->use($supplier['parser'], $supplier);
            $this->supplierAdaptor->SupplierManufacturer->clearManufacturers($supplier_id);
        }
    ...
    public function update_manufacturers($supplier_id = null)
    {
        $this->load->model('sale/supplier');
        ...
        if ($supplier && $supplier['parser']) {
            $this->supplierAdaptor->use($supplier['parser'], $supplier);
            $this->supplierAdaptor->SupplierManufacturer->updateManufacturers($this->supplierAdaptor->getManufacturers());
        }
    ...
    public function cron($supplier_id = null)
    {
        ...
        foreach ($suppliers as $supplier) {
            ...
            foreach ($products as $product) {
                ...
                $product_id = $this->supplierAdaptor->SupplierProduct->parseProduct($product);

                if ($product_id && !empty($product['variants'])) {
            ...
            $this->supplierAdaptor->SupplierManufacturer->unsetManufacturers();
    ...
    protected function getList()
    {
        ...
        foreach ($results as $result) {
            ...
            $this->data['suppliers'][] = [
                ...
                'skip_no_manufacturer' => $result['skip_no_manufacturer'],
    ...
    protected function getForm()
    {
        ...
        if (isset($this->request->post['skip_no_manufacturer'])) {
            $this->data['skip_no_manufacturer'] = $this->request->post['skip_no_manufacturer'];
        } elseif (!empty($supplier_info)) {
            $this->data['skip_no_manufacturer'] = $supplier_info['skip_no_manufacturer'];
        } else {
            $this->data['skip_no_manufacturer'] = 0;
        }
        ...
        $this->data['supplier_manufacturers'] = [];
        $this->data['supplier_manufacturers_total'] = 0;
        if (!empty($supplier_info)) {
            $this->data['supplier_manufacturers_total'] = $this->model_sale_supplier->getTotalSupplierManufacturers($supplier_info['supplier_id']);
            $supplier_manufacturers = $this->model_sale_supplier->getSupplierManufacturers($supplier_info['supplier_id']);

            foreach ($supplier_manufacturers as $supplier_manufacturer) {
                $this->data['supplier_manufacturers'][] = [
                    'supplier_manufacturer_id' => $supplier_manufacturer['supplier_manufacturer_id'],
                    'manufacturer' => $supplier_manufacturer['manufacturer'],
                    'manufacturer_full' => $supplier_manufacturer['manufacturer_full'],
                    'products' => $supplier_manufacturer['products'],
                    'stocks' => $supplier_manufacturer['stocks'],
                    'prices' => $supplier_manufacturer['prices'],
                ];
            }
        }
        ...
        if (!empty($supplier_info)) {
            ...
            $this->data['update_manufacturers'] = $this->url->link('sale/supplier/update_manufacturers', 'token=' . $this->session->data['token'] . '&supplier_id=' . $supplier_info['supplier_id']);
            $this->data['clear_manufacturers'] = $this->url->link('sale/supplier/clear_manufacturers', 'token=' . $this->session->data['token'] . '&supplier_id=' . $supplier_info['supplier_id']);
