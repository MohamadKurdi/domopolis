...
class ControllerShippingCdek extends Controller {
	...
	public function index() {
		...
		$installed = $this->model_module_cdek_license->chechInstalled('cdek','extension/shipping');
		if(!$installed['status'])
		{
		...
		$this->data['entry_use_postcode'] = $this->language->get('entry_use_postcode');
		...
		*/
		...
		if (isset($this->request->post['cdek_use_postcode'])) {
			$this->data['cdek_use_postcode'] = $this->request->post['cdek_use_postcode'];
		} elseif (!is_null($this->config->get('cdek_use_postcode'))) {
			$this->data['cdek_use_postcode'] = $this->config->get('cdek_use_postcode');
		} else {
			$this->data['cdek_use_postcode'] = 1;
		}
	...
	private function validate() {
		...
		$hits_log = new Log('hits_module.log');
		$hits_log_ret = print_r($this->error, TRUE);
		$hits_log->write($hits_log_ret);
		if (!$this->error) {
			return true;
		} else {
			return false;
