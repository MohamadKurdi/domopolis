...
class ControllerShippingPickupAdvanced extends Controller
{
	...
	public function index()
	{
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate())
		{
			...
			$this->redirect($this->url->link('shipping/pickup_advanced', 'token=' . $this->session->data['token'], 'SSL'));
		}
		...
		$this->data['entry_display_threshold'] = $this->language->get('entry_display_threshold');
		...
		(
			...
			'href'      => $this->url->link('shipping/pickup_advanced', 'token=' . $this->session->data['token']),
			'separator' => ' :: '
		);
		
		$this->data['action'] = $this->url->link('shipping/pickup_advanced', 'token=' . $this->session->data['token']);
		...
		$this->data['no_image'] = $this->model_tool_image->resize('no_image.jpg', 47, 47);
