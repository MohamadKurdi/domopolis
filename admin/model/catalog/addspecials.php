<?php
    class ModelCatalogAddSpecials extends Model {
        ...
        public function DelSpecial ($product_id, $product_del) {
            if (!empty($product_del)) {
                if($product_del['additional_offer']) {
                    $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_additional_offer WHERE product_id = '" . (int)$product_id . "'");
        ...
        public function getProducts($data = array()) {
            $sql = "SELECT pd.name, p.model, p.sku, p.product_id, p.price, p.quantity FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id)";
