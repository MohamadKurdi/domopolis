...
class ModelCatalogGroupPrice extends Model
{
    ...
    {
        $this->db->query("CREATE TABLE IF NOT EXISTS customer_group_price (
    ...
    {
        $prices = array();
        $query = $this->db->query("SELECT * FROM customer_group_price");
        if ($query->num_rows) {
        ...
        foreach ($post['group_price'] as $category_id => $groups) {
            foreach ($groups as $customer_group_id => $price) {
                ...
                if (isset($categories_price[$category_id][$customer_group_id])) {
                    $this->db->query("UPDATE customer_group_price SET price = " . (float)$price . ", type = " . (int)$type . "  
                        WHERE customer_group_id = " . (int)$customer_group_id . " AND category_id = " . (int)$category_id);
                } elseif (!empty($price)) {
                    $this->db->query("INSERT INTO customer_group_price (customer_group_id, category_id, price, type) 
    ...
    public function deleteCategoryPrices($category_id)
    {
        ...
        if ($category_id !== null && $category_id !== false) {
            $this->db->query("DELETE FROM customer_group_price  WHERE category_id = " . (int)$category_id);
        }
    }
        
    public function getLastCategoryId()
    {
        $query = $this->db->query("SELECT MAX(category_id) AS category_id FROM category WHERE date_added > NOW()-2");
    ...
    public function updateDbPrice($get)
    {
        ...
        if ($category_id != 0) {
            $query .= " LEFT JOIN product_to_category p2c ON (p.product_id = p2c.product_id) ";
        }
