	...
	class ModelCatalogOCFilter extends Model {
		...
		public function copyAttributesToOCFilter($data = array()) {
			...
			if (!empty($data['filter'])) {
				...
				$this->db->query("INSERT IGNORE INTO " . DB_PREFIX . "ocfilter_option (option_id, `type`, `status`, sort_order) SELECT (filter_group_id + '" . (int)$last_option_id . "'), '" . $this->db->escape($data['type']) . "' AS type, '1' AS `status`, sort_order FROM `" . DB_PREFIX . "filter_group`");
			...
			if (!empty($data['attribute'])) {
				...
				$this->db->query("INSERT IGNORE INTO " . DB_PREFIX . "ocfilter_option (option_id, status, type, sort_order) SELECT (attribute_id + '" . (int)$last_option_id . "'), '1' AS status, '" . $this->db->escape($data['type']) . "' AS type, sort_order FROM " . DB_PREFIX . "attribute");
				...
				$this->db->query("INSERT IGNORE INTO " . DB_PREFIX . "ocfilter_option_value (option_id, value_id) SELECT (attribute_id + '" . (int)$last_option_id . "'), CRC32(CONCAT(attribute_id, text)) AS value_id FROM " . DB_PREFIX . "product_attribute WHERE language_id = '" . (int)$this->config->get('config_language_id') . "' GROUP BY attribute_id, `text`");
				$this->db->query("INSERT IGNORE INTO " . DB_PREFIX . "ocfilter_option_value_description (option_id, value_id, language_id, name) SELECT (attribute_id + '" . (int)$last_option_id . "'), CRC32(CONCAT(attribute_id, text)) AS value_id, language_id, text FROM " . DB_PREFIX . "product_attribute WHERE language_id = '" . (int)$this->config->get('config_language_id') . "' GROUP BY attribute_id, `text`");
				...
				$this->db->query("INSERT IGNORE INTO " . DB_PREFIX . "ocfilter_option_value_to_product (product_id, option_id, value_id) SELECT product_id, (attribute_id + '" . (int)$last_option_id . "'), CRC32(CONCAT(attribute_id, text)) AS value_id FROM " . DB_PREFIX . "product_attribute WHERE language_id = '" . (int)$this->config->get('config_language_id') . "'");
			}
			...
			if (!empty($data['option']) || !empty($data['filter']) || !empty($data['attribute'])) {
				...
				foreach ($option_query->rows as $option) {
					...
					foreach ($value_query->rows as $value) {
						...
						if ($slide_value_min) {
							$this->db->query("UPDATE IGNORE " . DB_PREFIX . "ocfilter_option_value_to_product SET value_id = '0', slide_value_min = '" . (float)$slide_value_min . "' WHERE option_id = '" . (int)$value['option_id'] . "' AND value_id = '" . (string)$value['value_id'] . "'");
						}
					}
				}
			}
