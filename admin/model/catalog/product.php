<?php
	class ModelCatalogProduct extends Model {
		...
		public function roundPrices(){
			...
			$query = $this->db->query("UPDATE " . DB_PREFIX . "product SET price=IF((price-FLOOR(price)<=0.3), FLOOR(price), CEILING(price)) WHERE price < 30 AND price > 5");
			$affected += $this->db->countAffected();
			
			$query = $this->db->query("UPDATE " . DB_PREFIX . "product SET price=IF((price-FLOOR(price)<0.5), FLOOR(price), CEILING(price)) WHERE price >= 30");
			$affected += $this->db->countAffected();
			/*	
				$query = $this->db->query("UPDATE " . DB_PREFIX . "product SET historical_price=IF((historical_price-FLOOR(historical_price)<=0.3), FLOOR(historical_price), CEILING(historical_price)) WHERE historical_price < 30 AND historical_price > 5");
				$query = $this->db->query("UPDATE " . DB_PREFIX . "product SET historical_price=IF((historical_price-FLOOR(historical_price)<0.5), FLOOR(historical_price), CEILING(historical_price)) WHERE historical_price >= 30");
			*/
			return $affected;
		}
		...
		public function setNewPrices($manufacturer_id = false) {
			...
			if ($manufacturer_id && $manufacturer_id > 0) {
				...
				$query = "DELETE FROM " . DB_PREFIX . "product_special WHERE product_id IN (SELECT product_id FROM " . DB_PREFIX . "product WHERE manufacturer_id = '" .(int)$manufacturer_id. "' AND ignore_parse = 0)";
				$this->db->query($query);
				...
				$query = "DELETE FROM " . DB_PREFIX . "product_special WHERE product_id IN (SELECT product_id FROM " . DB_PREFIX . "product WHERE ((special_cost AND special_cost > 0) OR (cost AND cost > 0)) AND ignore_parse = 0)";
				$this->db->query($query);
			}
			...
			foreach ($cs as $c) {
				...
				$query = "UPDATE " . DB_PREFIX . "product SET price = cost*" . $coef . " WHERE cost AND cost > ".$price." AND ignore_parse = 0";			
				$this->db->query($query);
				
				$query = "INSERT INTO " . DB_PREFIX . "product_special (`product_id`, `customer_group_id`, `price`, `date_start`, `date_end`) (SELECT product_id, '1', special_cost * ". $coef .", '2010-10-01', '2156-10-10' FROM " . DB_PREFIX . "product WHERE special_cost AND special_cost > ".$price." AND ignore_parse = 0)";
				$this->db->query($query);
				
				$query = "UPDATE " . DB_PREFIX . "product SET special_cost = '' WHERE special_cost AND special_cost > " . $price . " AND ignore_parse = 0";
				$this->db->query($query);
				
				$affected += $this->db->countAffected();
			}
			
			$query = "UPDATE " . DB_PREFIX . "product SET historical_cost = cost WHERE cost > 0";
			$this->db->query($query);
			
			$query = "UPDATE " . DB_PREFIX . "product SET stock_status_id = '" .(int)$this->config->get('config_stock_status_id'). "' WHERE cost > 0 AND ignore_parse = 0";
			$this->db->query($query);
			
			$query = "UPDATE " . DB_PREFIX . "product SET cost = '' WHERE cost > 0";
			$this->db->query($query);
			
			
			$query = "UPDATE " . DB_PREFIX . "product SET quantity = 0 WHERE quantity < 0";
			$this->db->query($query);
		...
		public function addProduct($data) {
			...
			$this->db->query("DELETE FROM " . DB_PREFIX . "product_stock_status WHERE product_id = '" . (int)$product_id . "'");
			...
			if (isset($data['product_stock_status'])) {			
				foreach ($data['product_stock_status'] as $key => $value){
					...
					if ($value > 0) {
						...
						$this->db->query("INSERT INTO " . DB_PREFIX . "product_stock_status SET store_id = '" . (int)$key . "', product_id = '" . (int)$product_id . "', stock_status_id = '" . (int)$value . "'");
			...
			if (isset($data['product_special'])) {
				foreach ($data['product_special'] as $product_special) {
					$this->db->query("INSERT INTO " . DB_PREFIX . "product_special SET product_id = '" . (int)$product_id . "', customer_group_id = '" . (int)$product_special['customer_group_id'] . "', store_id = '" . (int)$product_special['store_id'] . "', currency_scode = '" . $this->db->escape($product_special['currency_scode']) . "', priority = '" . (int)$product_special['priority'] . "', price = '" . (float)$product_special['price'] . "', date_start = '" . $this->db->escape($product_special['date_start']) . "', date_end = '" . $this->db->escape($product_special['date_end']) . "'");
				}
			}
				...
				foreach ($data['ocfilter_product_option'] as $option_id => $values) {
					foreach ($values['values'] as $value_id => $value) {
						if (isset($value['selected'])) {
							$this->db->query("INSERT INTO " . DB_PREFIX . "ocfilter_option_value_to_product SET product_id = '" . (int)$product_id . "', option_id = '" . (int)$option_id . "', value_id = '" . $this->db->escape($value_id) . "', slide_value_min = '" . (isset($value['slide_value_min']) ? (float)$value['slide_value_min'] : 0) . "', slide_value_max = '" . (isset($value['slide_value_max']) ? (float)$value['slide_value_max'] : 0) . "'");
							
							foreach ($value['description'] as $language_id => $description) {
								if (trim($description['description'])) {
									$this->db->query("INSERT INTO " . DB_PREFIX . "ocfilter_option_value_to_product_description SET product_id = '" . (int)$product_id . "', option_id = '" . (int)$option_id . "', value_id = '" . $this->db->escape($value_id) . "', language_id = '" . (int)$language_id . "', description = '" . $this->db->escape($description['description']) . "'");
								}
							}
						}
					}
		...
		public function editProduct($product_id, $data) {
			...
			$this->db->query("DELETE FROM " . DB_PREFIX . "product_price_to_store WHERE product_id = '" . (int)$product_id . "'");
			...
			if (isset($data['product_price_to_store'])) {
				...
				foreach ($data['product_price_to_store'] as $key => $value){
					...
					if ($value && (!empty($value['price'])) || !empty($value['dot_not_overload_1c'])) {
						...
						$this->db->query("INSERT INTO " . DB_PREFIX . "product_price_to_store SET store_id = '" . (int)$key . "', product_id = '" . (int)$product_id . "', price = '" . (float)$value['price'] . "', dot_not_overload_1c = '" . (int)$value['dot_not_overload_1c'] . "', settled_from_1c = '" . (int)$value['settled_from_1c'] . "'");
			...
			$this->db->query("DELETE FROM " . DB_PREFIX . "product_price_national_to_store WHERE product_id = '" . (int)$product_id . "'");
			...
			if (isset($data['product_price_national_to_store'])) {
				...
				foreach ($data['product_price_national_to_store'] as $key => $value){
					...
					if ($value && (!empty($value['price']) || !empty($value['dot_not_overload_1c']))) {					
						...
						$this->db->query("INSERT INTO " . DB_PREFIX . "product_price_national_to_store SET store_id = '" . (int)$key . "', product_id = '" . (int)$product_id . "', price = '" . (float)$value['price'] . "', dot_not_overload_1c = '" . (int)$value['dot_not_overload_1c'] . "', settled_from_1c = '" . (int)$value['settled_from_1c'] . "',  currency = '" . $this->db->escape($this->model_setting_setting->getKeySettingValue('config', 'config_regional_currency', (int)$key)) . "'");
			...
			$this->db->query("DELETE FROM " . DB_PREFIX . "product_price_national_to_yam WHERE product_id = '" . (int)$product_id . "'");
			...
			if (isset($data['product_price_national_to_yam'])) {
				...
				foreach ($data['product_price_national_to_yam'] as $key => $value){
					...
					if ($value && (!empty($value['price']) || !empty($value['dot_not_overload_1c']))) {					
						...
						$this->db->query("INSERT INTO " . DB_PREFIX . "product_price_national_to_yam SET store_id = '" . (int)$key . "', product_id = '" . (int)$product_id . "', price = '" . (float)$value['price'] . "', dot_not_overload_1c = '" . (int)$value['dot_not_overload_1c'] . "', settled_from_1c = '" . (int)$value['settled_from_1c'] . "',  currency = '" . $this->db->escape($this->model_setting_setting->getKeySettingValue('config', 'config_regional_currency', (int)$key)) . "'");
			...
			$this->db->query("DELETE FROM " . DB_PREFIX . "product_stock_limits WHERE product_id = '" . (int)$product_id . "'");
			...
			if (isset($data['product_stock_limits'])) {			
				foreach ($data['product_stock_limits'] as $key => $value){
					...
					if (is_array($value)) {
						...
						$this->db->query("INSERT INTO " . DB_PREFIX . "product_stock_limits SET store_id = '" . (int)$key . "', product_id = '" . (int)$product_id . "', min_stock = '" . (int)$value['min_stock'] . "', rec_stock = '" . (int)$value['rec_stock'] . "'");
			...
			/*Additional offer*/
			$this->db->query("DELETE FROM " . DB_PREFIX . "product_additional_offer WHERE product_id = '" . (int)$product_id . "'");
			...
			if (isset($data['product_additional_offer'])) {
				foreach ($data['product_additional_offer'] as $product_additional_offer) {
					...
					if (isset($product_additional_offer['product_additional_offer_id']) && $product_additional_offer['product_additional_offer_id']) {
						...
						$this->db->query("INSERT INTO " . DB_PREFIX . "product_additional_offer SET 
			...
			if (isset($data['product_category'])) {
				foreach ($data['product_category'] as $category_id) {
					...
					$this->db->query("INSERT IGNORE INTO " . DB_PREFIX . "feed_queue SET category_id = '" . (int)$category_id . "'");
				}		
			}
		...
		public function copyProduct($product_id) {
			...
			if ($query->num_rows) {
				...
				$this->db->query("UPDATE " . DB_PREFIX . "product SET stock_product_id = '" .(int)$product_id. "' WHERE product_id = '" . (int)$new_product_id . "'");
		...
		public function getProductPrice($product_id) {
			$query = $this->db->query("SELECT price FROM " . DB_PREFIX . "product WHERE product_id = '" . (int)$product_id . "' LIMIT 1");
		...
		public function getProductStorePrice($product_id, $store_id) {
			$query = $this->db->query("SELECT price FROM " . DB_PREFIX . "product_price_to_store WHERE product_id = '" . (int)$product_id . "' AND store_id = '" . (int)$store_id . "' LIMIT 1");
		...
		public function getProductStorePriceNational($product_id, $store_id) {
			$query = $this->db->query("SELECT price FROM " . DB_PREFIX . "product_price_national_to_store WHERE product_id = '" . (int)$product_id . "' AND store_id = '" . (int)$store_id . "' LIMIT 1");
		...
		public function getProductStorePricesAll($product_id) {
			$query = $this->db->query("SELECT price FROM " . DB_PREFIX . "product_price_to_store WHERE product_id = '" . (int)$product_id . "'");
		...
		public function getAdditionalOfferById($ao_id, $ao_product_id) {
			$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_additional_offer WHERE product_additional_offer_id = '" . (int)$ao_id . "' AND ao_product_id = '" . (int)$ao_product_id . "' ORDER BY priority, price LIMIT 1");
		...
		public function getProductAdditionalOffer($product_id, $active = false) {
			...
			if (!$active) {
				...
				$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_additional_offer WHERE product_id = '" . (int)$product_id . "' ORDER BY priority, price");
				
				} else {
				
				$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_additional_offer WHERE product_id = '" . (int)$product_id . "' AND date_end >= NOW() ORDER BY priority, price");
		...
		public function getProductAdditionalOfferGroups() {
			$query = $this->db->query("SELECT DISTINCT ao_group FROM " . DB_PREFIX . "product_additional_offer ORDER BY priority, price");
		...
		public function getProductAdditionalOfferByAOID($product_id) {
			$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_additional_offer WHERE ao_product_id = '" . (int)$product_id . "' ORDER BY priority, price");
			
			return $query->rows;
		}
		
		public function getProductAdditionalOfferStoresByAOID($product_additional_offer_id) {
			$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_additional_offer_to_store WHERE product_additional_offer_id = '" . (int)$product_additional_offer_id . "'");						
		...
		public function getProductWaitListOrders($product_id){
			$query = $this->db->query("SELECT order_id, quantity FROM " . DB_PREFIX . "order_product_nogood opn WHERE product_id = '" .(int)$product_id. "' AND waitlist = 1");
			
			return $query->rows;		
		}
		
		public function deleteProductFromWaitList($order_product_id){
			$this->db->query("UPDATE " . DB_PREFIX . "order_product_nogood SET waitlist = 0 WHERE order_product_id = '" . (int)$order_product_id . "'");				
		}
		
		public function createNewOrderFromWaitlist($order_product_id, $new_order_products){
			$order_id_query = $this->db->query("SELECT order_id FROM " . DB_PREFIX . "order_product_nogood WHERE order_product_id = '" . (int)$order_product_id . "'");	
			...
			if ($order_id_query->row['order_id']){
				...
				if ($old_order = $this->model_sale_order->getOrder($order_id_query->row['order_id'])){
					...
					foreach ($new_order_products as $new_product_id){								
						$this->db->query("UPDATE " . DB_PREFIX . "order_product_nogood SET waitlist = 0, new_order_id = '" .(int)$new_order_id. "' WHERE order_product_id = '" . (int)$new_product_id . "' LIMIT 1");	
						
						$product_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_product_nogood WHERE order_product_id = '" . (int)$new_product_id . "' LIMIT 1");	
					...
					//totals
					//main_total
					$this->db->query("UPDATE `" . DB_PREFIX . "order` SET total = '" . $main_total . "' WHERE order_id = '" . (int)$new_order_id . "'");
					$this->db->query("UPDATE `" . DB_PREFIX . "order` SET total_national = '" . $main_total_national . "' WHERE order_id = '" . (int)$new_order_id . "'");
					//sub_total
		...
		public function getProductsWaitList($data = array()) {
			...
			LEFT JOIN " . DB_PREFIX . "order_product_nogood opn ON (opn.product_id = p.product_id)
			LEFT JOIN `" . DB_PREFIX . "order` o ON (o.order_id = opn.order_id)
		...
		public function getProductAttributesByLanguage($product_id, $language_id) {
			...
			$query = $this->db->query("SELECT attribute_id, text FROM " . DB_PREFIX . "product_attribute WHERE product_id = '" . (int)$product_id . "' AND language_id = '" . (int)$language_id . "'");
		...
		public function getSimilarProductsByAttributes($product_id, $category_id, $language_id, $attributes = array(), $limit = 10, $stock = false){
			...
			$sql = "SELECT DISTINCT pa.product_id FROM " . DB_PREFIX . "product_attribute pa";
		...
		public function getProductActualCost($product_id) {
			$query = $this->db->query("SELECT actual_cost FROM " . DB_PREFIX . "product WHERE product_id = '" . (int)$product_id . "' LIMIT 1");
		...
		public function getProductSpecialOne($product_id) {
			$query = $this->db->query("SELECT price, currency_scode FROM " . DB_PREFIX . "product_special WHERE product_id = '" . (int)$product_id . "' ORDER BY priority, price LIMIT 1");
		...
		public function clearProductStorePricesNational($product_id){
			$query = $this->db->query("DELETE FROM " . DB_PREFIX . "product_price_national_to_store WHERE product_id = '" . (int)$product_id . "' AND dot_not_overload_1c = 0");
			
		}
		
		public function getProductStockLimits($product_id){
			$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_stock_limits WHERE product_id = '" . (int)$product_id . "'");
		...
		public function getProductSpecialsForCustomer($product_id, $customer_group_id, $order_currency = false, $store_id = false) {
			...
			$sql = "SELECT price, currency_scode FROM " . DB_PREFIX . "product_special ps WHERE ps.product_id = '" . (int)$product_id . "' AND ps.customer_group_id = '" . (int)$customer_group_id . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < NOW()) AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW())) ORDER BY ps.priority ASC, ps.price ASC LIMIT 1";
			
			if ($store_id){
				$sql = "SELECT price, currency_scode FROM " . DB_PREFIX . "product_special ps WHERE ps.product_id = '" . (int)$product_id . "' AND ps.customer_group_id = '" . (int)$customer_group_id . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < NOW()) AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW())) AND (ps.store_id = '" . (int)$store_id . "' OR ps.store_id = -1) ORDER BY ps.store_id DESC, ps.priority ASC LIMIT 1";
			}
		...
		public function getThisProductIsSet($product_id){
			$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "set` WHERE product_id = '" . (int)$product_id . "' LIMIT 1");
		...
		public function getSet($set_id){
			$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "product_to_set` WHERE set_id = '" . (int)$set_id . "'");
		...
		public function getTotalOrdersWaitList($data = array()) {
			$sql = "SELECT COUNT(DISTINCT o.order_id) AS total FROM " . DB_PREFIX . "product p
			LEFT JOIN " . DB_PREFIX . "order_product_nogood opn ON (opn.product_id = p.product_id)
			LEFT JOIN `" . DB_PREFIX . "order` o ON (o.order_id = opn.order_id)
			LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id)";
		...
		public function getTotalProductsWaitList($data = array()) {
			$sql = "SELECT COUNT(p.product_id) AS total FROM " . DB_PREFIX . "product p
			LEFT JOIN " . DB_PREFIX . "order_product_nogood opn ON (opn.product_id = p.product_id)
			LEFT JOIN `" . DB_PREFIX . "order` o ON (o.order_id = opn.order_id)
			LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id)";
		...
		public function getTotalProductsByCollectionId($collection_id) {
			$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "product WHERE collection_id = '" . (int)$collection_id . "'");
		...
		public function getStockQuantities($product_id){
			$query = $this->db->query("SELECT quantity_stock,quantity_stockK,quantity_stockM,quantity_stockMN,quantity_stockAS, FROM " . DB_PREFIX . "product WHERE product_id = '" . (int)$product_id . "'");
			
			return $query->row;
		}
		
		public function getChildStockProductId($product_id){
			$query = $this->db->query("SELECT product_id FROM " . DB_PREFIX . "product WHERE stock_product_id = '" . (int)$product_id . "'");
