...
class ModelCatalogProductMaster extends Model
{
    ...
    public function getMasterProductIdFromData($data)
    {
        ...
        if ($data['color_series_type'] == 'singleItem') {
            return -1;
        } elseif ($data['color_series_type'] == 'colorSeriesMaster') {
            return 0;
        } else {
    ...
    public function getLinkedProducts($product_id, $special_attribute_group_id)
    {
        ...
        //do nothing
            ...
            . " LEFT JOIN product_master pm on pm.product_id = p.product_id"
            . " LEFT JOIN product_description pd on pd.product_id = p.product_id"
            . " LEFT JOIN product_special_attribute psa ON psa.product_id = p.product_id"
            . " LEFT JOIN special_attribute sa ON sa.special_attribute_id = psa.special_attribute_id"
            . " AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "' "
