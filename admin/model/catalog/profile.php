...
class ModelCatalogProfile extends Model
{
    ...
    {
        return $this->db->query("SELECT * FROM `profile` WHERE profile_id = " . (int)$profile_id)->row;
    }
    ...
    public function getProfileDescription($profile_id)
    {
        ...
        $results = $this->db->query("SELECT * FROM `profile_description` WHERE `profile_id` = " . (int)$profile_id . "
		")->rows;
    ...
    {
        return $this->db->query("SELECT `p`.`profile_id`, `p`.`sort_order`, `pd`.`name` FROM `profile` AS `p` JOIN `profile_description` AS `pd` ON `pd`.`profile_id` = `p`.`profile_id` AND `pd`.`language_id` = " . (int)$this->config->get('config_language_id') . " ORDER BY p.sort_order ASC
		")->rows;
    }

    public function addProfile($data)
    {
        $this->db->query("INSERT INTO `profile` SET `sort_order` = " . (int)$data['sort_order'] . ", `status` = " . (int)$data['status'] . ", `price` = " . (double)$data['price'] . ", `frequency` = '" . $this->db->escape($data['frequency']) . "', `duration` = " . (int)$data['duration'] . ", `cycle` = " . (int)$data['cycle'] . ", `trial_status` = " . (int)$data['trial_status'] . ", `trial_price` = " . (double)$data['trial_price'] . ", `trial_frequency` = '" . $this->db->escape($data['trial_frequency']) . "', `trial_duration` = " . (int)$data['trial_duration'] . ", `trial_cycle` = " . (int)$data['trial_cycle']);

        $profile_id = $this->db->getLastId();

        foreach ($data['profile_description'] as $language_id => $profile_description) {
            $this->db->query("INSERT INTO `profile_description` (`profile_id`, `language_id`, `name`) VALUES (" . (int)$profile_id . ", " . (int)$language_id. ", '" . $this->db->escape($profile_description['name']) . "')");
        }
    ...
    {
        $this->db->query("DELETE FROM `profile` WHERE profile_id = " . (int)$profile_id);
        $this->db->query("DELETE FROM `profile_description` WHERE profile_id = " . (int)$profile_id);
        $this->db->query("DELETE FROM `product_profile` WHERE profile_id = " . (int)$profile_id);

        $this->db->query("UPDATE `order_recurring` SET `profile_id` = 0 WHERE `profile_id` = " . (int)$profile_id);
    }
