...
class ModelCatalogSet extends Model
{
    ...
    public function addSet($data)
    {
        ...
        $product_id = $this->addNewSetProduct($data);

        $this->db->query("INSERT INTO `set` SET product_id='" . (int)$product_id . "', price='" . (float)$data['total_set'] . "', enable_productcard = '" . (isset($data['enable_productcard']) ? 1 : 0 ) . "', sort_order='" . (int)$data['sort_order'] . "', status = '" . (int)$data['status'] . "', date_added = now()");
        $set_id = $this->db->getLastId();
        if (isset($data['set_image'])) {
            $this->db->query("UPDATE `set` SET image = '" . $this->db->escape(html_entity_decode($data['set_image'], ENT_QUOTES, 'UTF-8')) . "' WHERE set_id = '" . (int)$set_id . "'");
        }
        if (isset($data['set_category'])) {
            foreach ($data['set_category'] as $category_id) {
                $this->db->query("INSERT INTO set_to_category SET set_id = '" . (int)$set_id . "', category_id = '" . (int)$category_id . "'");
            }
        }
        if (isset($data['product'])) {
            foreach ($data['product'] as $product) {
                ...
                $this->db->query("INSERT INTO product_to_set SET set_id = '" . (int)$set_id . "', product_id='" . $this->db->escape($product['product_id']) . "', clean_product_id='" . (int)$clean_product_id . "', price='" . (float)$product['price'] . "', price_in_set='" . (isset($product['present']) ? 0 : (float)$product['price_in_set']) . "', quantity='" . (int)$product['quantity'] . "', present='" . (isset($product['present']) ? (int)$product['present'] : 0 ) . "', show_in_product='" . (isset($product['show_in_product']) ? (int)$product['show_in_product'] : 0 ) . "', sort_order='" . (int)$product['sort_order'] . "'");
            }
        }
        foreach ($data['set_description'] as $language_id => $value) {
            $this->db->query("INSERT INTO set_description SET set_id = '" . (int)$set_id . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "', description = '" . $this->db->escape($value['description']) . "'");
        }
        if (isset($data['set_store'])) {
            foreach ($data['set_store'] as $store_id) {
                $this->db->query("INSERT INTO set_to_store SET set_id = '" . (int)$set_id . "', store_id = '" . (int)$store_id . "'");
            }
        }
        return $set_id;
    }

    public function addNewSetProduct($data)
    {
        if (isset($data['enable_productcard'])) {
            $this->db->query("INSERT INTO product SET model = '', quantity = '99', manufacturer_id = '" . (int)$data['manufacturer_id'] . "', price = '" . (float)$data['total_set'] . "', status = '" . (int)$data['status'] . "', sort_order = '" . (int)$data['product_sort_order'] . "', date_added = NOW()");
            ...
            if (!empty($data['product_description'])) {
                foreach ($data['product_description'] as $language_id => $value) {
                    $this->db->query("INSERT INTO product_description SET product_id = '" . (int)$product_id . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "', meta_keyword = '" . $this->db->escape($value['meta_keyword']) . "', meta_description = '" . $this->db->escape($value['meta_description']) . "', description = '" . $this->db->escape($value['description']) . "'");
                }
            }
    ...
    public function editSet($set_id, $data)
    {
        ...
        if (isset($data['enable_productcard'])) {
            if ((int)$data['product_id']) {
                ...
                $this->db->query("UPDATE product SET manufacturer_id = '" . (int)$data['manufacturer_id'] . "', price = '" . (float)$data['total_set'] . "', status = '" . (int)$data['status'] . "', sort_order = '" . (int)$data['product_sort_order'] . "', date_modified = NOW() WHERE product_id = '" . (int)$product_id . "'");
            } else {
                $this->db->query("INSERT INTO product SET quantity = '99',  manufacturer_id = '" . (int)$data['manufacturer_id'] . "', price = '" . (float)$data['total_set'] . "', status = '" . (int)$data['status'] . "', sort_order = '" . (int)$data['product_sort_order'] . "', date_added = NOW()");
                $product_id = $this->db->getLastId();
            }
            if (isset($data['image'])) {
                $this->db->query("UPDATE product SET image = '" . $this->db->escape(html_entity_decode($data['image'], ENT_QUOTES, 'UTF-8')) . "' WHERE product_id = '" . (int)$product_id . "'");
        ...
        $this->db->query("UPDATE `set` SET product_id='" . (int)$product_id . "', price='" . (float)$data['total_set'] . "', enable_productcard = '" . (isset($data['enable_productcard']) ? 1 : 0 ) . "', sort_order='" . (int)$data['sort_order'] . "', tax_class_id = '" . (int)$data['tax_class_id'] . "', status = '" . (int)$data['status'] . "', count_persone = '" . (int)$data['count_persone'] . "', set_group = '" . $data['set_group'] . "' WHERE set_id='" . (int)$set_id . "'");
        if (isset($data['set_image'])) {
            $this->db->query("UPDATE `set` SET image = '" . $this->db->escape(html_entity_decode($data['set_image'], ENT_QUOTES, 'UTF-8')) . "' WHERE set_id = '" . (int)$set_id . "'");
        }
        
        $this->db->query("DELETE FROM set_to_category WHERE set_id = '" . (int)$set_id . "'");
        if (isset($data['set_category'])) {
            foreach ($data['set_category'] as $category_id) {
                $this->db->query("INSERT INTO set_to_category SET set_id = '" . (int)$set_id . "', category_id = '" . (int)$category_id . "'");
            }
        ...
        $this->db->query("DELETE FROM product_to_set WHERE set_id='" . (int)$set_id . "'");
        if (isset($data['product'])) {
            foreach ($data['product'] as $product) {
                $temp_product_id = explode(':', $product['product_id']);
                $clean_product_id = $temp_product_id[0];
        ...
        $this->db->query("DELETE FROM set_description WHERE set_id = '" . (int)$set_id . "'");
        foreach ($data['set_description'] as $language_id => $value) {
            $this->db->query("INSERT INTO set_description SET set_id = '" . (int)$set_id . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "', description = '" . $this->db->escape($value['description']) . "'");
        }
        $this->db->query("DELETE FROM set_to_store WHERE set_id = '" . (int)$set_id . "'");
        if (isset($data['set_store'])) {
            foreach ($data['set_store'] as $store_id) {
                $this->db->query("INSERT INTO set_to_store SET set_id = '" . (int)$set_id . "', store_id = '" . (int)$store_id . "'");
            }
    ...
    public function deleteSet($set_id)
    {
        ...
        $query = $this->db->query("SELECT product_id FROM `set` WHERE set_id='" . (int)$set_id . "'");
        ...
        $this->db->query("DELETE FROM `set` WHERE set_id = '" . (int)$set_id . "'");
        $this->db->query("DELETE FROM set_description WHERE set_id = '" . (int)$set_id . "'");
        $this->db->query("DELETE FROM set_to_store WHERE set_id = '" . (int)$set_id . "'");
        $this->db->query("DELETE FROM product_to_set WHERE set_id = '" . (int)$set_id . "'");
        $this->db->query("DELETE FROM set_to_category WHERE set_id = '" . (int)$set_id . "'");
    ...
    {
        $query = $this->db->query("SELECT * FROM `set` WHERE set_id='" . (int)$set_id . "'");
    ...
    public function getSetDescriptions($set_id)
    {
        ...
        $query = $this->db->query("SELECT * FROM set_description WHERE set_id = '" . (int)$set_id . "'");
    ...
    public function getSetStores($set_id)
    {
        ...
        $query = $this->db->query("SELECT * FROM set_to_store WHERE set_id = '" . (int)$set_id . "'");
    ...
    public function getSets($data = array())
    {
        ...
        if (!empty($data['filter_product_id'])) {
            $sql .= " LEFT JOIN product_to_set p2s ON (s.set_id = p2s.set_id)";
        }
    ...
    public function getTotalSets($data = array())
    {
        ...
        $sql = "SELECT COUNT(DISTINCT s.set_id) AS total FROM `set` s LEFT JOIN set_description sd ON (s.set_id = sd.set_id)";
    ...
    public function getProductsInSets($set_id = 0)
    {
        ...
        foreach ($query->rows as $product) {
            ...
            $sql2 = "SELECT * FROM product p LEFT JOIN product_description pd ON (p.product_id = pd.product_id)";
    ...
    public function getSetCategories($set_id)
    {
        ...
        $query = $this->db->query("SELECT * FROM set_to_category WHERE set_id = '" . (int)$set_id . "'");
    ...
    public function getSetByProduct($product_id)
    {
        ...
        $query = $this->db->query("SELECT * FROM `set` WHERE product_id = '" . (int)$product_id . "'");
    ...
    public function getOptionInfo($product_option_id, $option_value_id)
    {
        ...
        $query1 = $this->db->query("SELECT * FROM `product_option` WHERE product_option_id = '" . (int)$product_option_id . "'");
        if ($query1->row) {
            $option_id = (int)$query1->row['option_id'];
            $query2 = $this->db->query("SELECT o.type, od.name FROM `option` o LEFT JOIN `option_description` od  ON (o.option_id = od.option_id) WHERE o.option_id = '" . (int)$option_id . "' AND od.language_id = '" . (int)$this->config->get('config_language_id') . "'");
            if ($query2->row) {
            ...
            if ($option_type=='checkbox' && is_array($option_value_id)) {
                ...
                foreach ($option_value_id as $id) {
                    $query3 = $this->db->query("SELECT * FROM `product_option_value` WHERE product_option_value_id = '" . (int)$id . "'");
                    if ($query3->row) {
                        $query4 = $this->db->query("SELECT * FROM `option_value_description` WHERE option_value_id = '" . (int)$query3->row['option_value_id'] . "'");
                        if ($query4->row) {
                ...
                $query3 = $this->db->query("SELECT * FROM `product_option_value` WHERE product_option_value_id = '" . (int)$option_value_id . "'");
                if ($query3->row) {
                    $query4 = $this->db->query("SELECT * FROM `option_value_description` WHERE option_value_id = '" . (int)$query3->row['option_value_id'] . "'");
                    if ($query4->row) {
    ...
    public function getOptionInfoPrice($option_value_id)
    {
        $option_price = 0;
        if (is_array($option_value_id)) {
            foreach ($option_value_id as $id) {
                $query = $this->db->query("SELECT * FROM `product_option_value` WHERE product_option_value_id = '" . (int)$id . "'");
                if ($query->row&&$query->row['price']) {
            ...
            $query = $this->db->query("SELECT * FROM `product_option_value` WHERE product_option_value_id = '" . (int)$option_value_id . "'");
            if ($query->row&&$query->row['price']) {
                if ($query->row['price_prefix'] == '+') {
                    $option_price += $query->row['price'];
                } elseif ($query->row['price_prefix'] == '-') {
    ...
    public function copySet($setId, $newCountPersons, $product_id)
    {

        $sql = "INSERT INTO `set` (`product_id`, `image`, `price`, `percent`, `sort_order`, `enable_productcard`, `status`, `tax_class_id`, `date_added`, `count_persone`, `set_group`) SELECT '".$product_id."', `image`, `price`, `percent`, `sort_order`, `enable_productcard`, `status`, `tax_class_id`, `date_added`, '".(int)$newCountPersons."', `set_group` FROM `set` WHERE `set_id` = ".(int)$setId;
        $this->db->query($sql);

        $newId = $this->db->getLastId();

        $this->db->query("INSERT INTO `set_description` (`set_id`, `language_id`, `name`, `description`) SELECT '".(int)$newId."', `language_id`, `name`, `description` FROM `set_description` WHERE `set_id` = ".(int)$setId);
        $this->db->query("INSERT INTO `set_to_category` (`set_id`, `category_id`) SELECT '".(int)$newId."', `category_id` FROM `set_to_category` WHERE `set_id` = ".(int)$setId);
        $this->db->query("INSERT INTO `set_to_store` (`set_id`, `store_id`) SELECT '".(int)$newId."', `store_id` FROM `set_to_store` WHERE `set_id` = ".(int)$setId);
        $this->db->query("INSERT INTO `product_to_set` (`set_id`, `product_id`, `clean_product_id`, `price`, `price_in_set`, `quantity`, `present`, `show_in_product`, `sort_order`) SELECT '".(int)$newId."', `product_id`, `clean_product_id`, `price`, `price_in_set`, `quantity`, `present`, `show_in_product`, `sort_order` FROM `product_to_set` WHERE `set_id` = ".(int)$setId);
