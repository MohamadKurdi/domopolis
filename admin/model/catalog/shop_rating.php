...
class ModelCatalogShopRating extends Model
{
    ...
    public function getAllRatings()
    {
        ...
        $sql = "SELECT * FROM shop_rating ORDER BY date_added DESC";
    ...
    public function getCustomTypes()
    {
        ...
        $sql = "SELECT * FROM shop_rating_custom_types ORDER BY id";
    ...
    public function getRating($rate_id)
    {
        ...
        $sql = "SELECT * FROM shop_rating WHERE rate_id = '".$rate_id."'";
    ...
    public function getRatingAnswer($rate_id)
    {
        ...
        $sql = "SELECT * FROM shop_rating_answers WHERE rate_id = '".$rate_id."'";
    ...
    public function getRatingAnswers()
    {
        ...
        $sql = "SELECT id, rate_id FROM shop_rating_answers WHERE comment <> ''";
    ...
    public function customerRatingsCount($customer_id)
    {
        ...
        $sql = "SELECT COUNT(*) AS total FROM shop_rating WHERE store_id ='" . $store_id . "' AND customer_id = '".$customer_id."'";
    ...
    public function addAnswer($rate_id, $answer)
    {
        ...
        if ($answer['answer_id']) {
            $sql = "UPDATE shop_rating_answers SET comment='" . $answer['answer'] . "' WHERE id = '" . $answer['answer_id'] . "'";
            $this->db->query($sql);
            $answer_id=$answer['answer_id'];
        } else {
            $this->db->query("INSERT INTO shop_rating_answers SET rate_id = '" . $rate_id . "', user_id = '" . (int)$this->user->getId() . "', date_added =  NOW(), comment = '" . $this->db->escape($answer['answer']) . "'");
        ...
        if ($answer_id && $this->config->get('shop_rating_notify') && $answer['answer']) {
            ...
            if ($answer_item['notified'] != 1) {
                ...
                $sql = "UPDATE shop_rating_answers SET notified='1' WHERE id = '" . $answer_id . "'";
                $this->db->query($sql);
            }
        }
    }
    ...
    {
        $this->db->query("INSERT INTO shop_rating_custom_types SET title = '" . $this->db->escape($title) . "', status = '0' ");
        return array('id' => $this->db->getLastId(), 'title' => $title);
    }
    public function statusCustomType($type_id)
    {
        $sql = "UPDATE shop_rating_custom_types SET status=IF(status=0, 1, 0) WHERE id = '".$type_id."'";
    ...
    {
        $sql = "DELETE FROM shop_rating_custom_types WHERE id = '".$type_id."'";
        if ($this->db->query($sql)) {
            return array('id' => $type_id, 'status' => 'success');
        } else {
            return array('id' => $type_id, 'status' => 'error');
    ...
    {
        $sql = "UPDATE shop_rating SET rate_status=IF(rate_status=0, 1, 0) WHERE rate_id = '".$rate_id."'";
    ...
    public function changeDate($rate_id, $new_date)
    {
        ...
        $sql = "UPDATE shop_rating SET date_added='".$date."' WHERE rate_id = '".$rate_id."'";
        $this->db->query($sql);
    }
    ...
    public function changeComment($rate_id, $rating_params)
    {
        ...
        $this->db->query("DELETE FROM shop_rating_description WHERE rate_id = '" . (int)$rate_id . "'");
        foreach ($rating_params['shop_rating_description'] as $language_id => $value) {
            $this->db->query("INSERT INTO shop_rating_description SET 
				rate_id = '" . (int)$rate_id . "', 
    ...
    public function getShopRatingDescriptions($rate_id)
    {
        ...
        $query = $this->db->query("SELECT * FROM shop_rating_description WHERE rate_id = '" . (int)$rate_id . "'");
    ...
    public function toogleEvent($set)
    {
        ...
        $sql = "SELECT COUNT(*) AS srrm FROM event WHERE `code` = 'shop_rating_request_mail'" ;
