...
class ModelCatalogSubscribe extends Model
{
    ...
    {
        $query = $this->db->query("SELECT * FROM subscribe WHERE subscribe_id='" . (int) $subscribe_id . "'");
    ...
    public function getEmailDescription()
    {
        ...
        $query = $this->db->query("SELECT * FROM subscribe_email_description");
    ...
    {
        $this->db->query("DELETE FROM subscribe_email_description");

        foreach ($descriptions as $language_id => $description) {
            $this->db->query("INSERT INTO subscribe_email_description SET subscribe_descriptions = '" . $description . "', language_id = '" . (int) $language_id . "'");
        }
    }
    ...
    public function getAuthDescription()
    {
        ...
        $query = $this->db->query("SELECT * FROM subscribe_auth_description");
    ...
    {
        $this->db->query("DELETE FROM subscribe_auth_description");

        foreach ($descriptions as $language_id => $description) {
            $this->db->query("INSERT INTO subscribe_auth_description SET subscribe_authorization = '" . $description . "', language_id = '" . (int) $language_id . "'");
        }
    }

    public function addSubscribe($data)
    {
        $this->db->query("INSERT INTO subscribe SET email = '" . $data['email'] . "', status = '" . (int) $data['status'] . "'");
    }

    public function editSubscribe($subscribe_id, $data)
    {
        $this->db->query("UPDATE subscribe SET email = '" . $data['email'] . "', status = '" . (int) $data['status'] . "' WHERE subscribe_id = '" . (int) $subscribe_id . "'");
    }

    public function deleteSubscribe($subscribe_id)
    {
        $this->db->query("DELETE FROM subscribe WHERE subscribe_id = '" . (int) $subscribe_id . "'");
    }
    ...
    {
        $query = $this->db->query("SELECT COUNT(*) AS total FROM `subscribe`");
    ...
    {
        $query = $this->db->query("SHOW TABLES LIKE 'subscribe'");
    ...
    {
        $this->db->query("DROP TABLE IF EXISTS `subscribe`");
        $this->db->query("DROP TABLE IF EXISTS `subscribe_email_description`");
        $this->db->query("DROP TABLE IF EXISTS `subscribe_auth_description`");
    }
}
