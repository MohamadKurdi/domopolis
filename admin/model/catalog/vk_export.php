...
class ModelCatalogVkExport extends Model
{
    ...
    {
        $res = $this->db->query('SELECT vea.*, cd.name AS category FROM vk_export_album vea
            LEFT JOIN category_description cd ON cd.category_id = vea.category_id AND cd.language_id = ' . (int)$this->config->get('config_language_id') . '
    ...
    {
        $this->db->query('UPDATE vk_export_album 
    ...
    {
        $sql = 'SELECT * FROM vk_export_album';
    ...
    {
        $res = $this->db->query('SELECT COUNT(*) AS total FROM vk_export_album');
        return $res->row['total'];
    }
    
    public function deleteAllAlbums()
    {
        $this->db->query('DELETE FROM vk_export_album');
        $this->db->query('DELETE FROM vk_export_photo WHERE category_id != 0 AND location = \'albums\'');
    }
    
    public function deleteAlbumByKey($category_id, $vk_album_id, $mode)
    {
        $this->db->query('DELETE FROM vk_export_album WHERE 
            category_id = ' . $category_id . ' AND
            vk_album_id = \'' . $this->db->escape($vk_album_id) . '\' AND
            mode = \'' . $this->db->escape($mode) . '\'
        ');
        $this->db->query('DELETE FROM vk_export_photo WHERE category_id = ' . $category_id . ' AND location = \'albums\'');
    }
    
    public function getAlbumId($category_id, $mode)
    {
        $query = $this->db->query("SELECT vk_album_id FROM vk_export_album WHERE category_id = " . $category_id . " AND mode = '" . $mode . "'");
        if ($query->row) {
    ...
    {
        $query = $this->db->query("SELECT vk_photo_id FROM vk_export_photo WHERE product_id = " . $product_id . ' AND location = \'albums\'');
        if ($query->row) {
    ...
    {
        $res = $this->db->query("INSERT IGNORE vk_export_album 
    ...
    {
        $this->db->query("INSERT vk_export_photo 
    ...
    {
        $this->db->query("DELETE FROM vk_export_photo WHERE product_id = " . $product_id . ' AND location = \'albums\'');
    }
    
    public function deleteAlbumsExportOne($product_id, $photo_id)
    {
        $this->db->query("DELETE FROM vk_export_photo WHERE product_id = " . $product_id . ' AND vk_photo_id = \'' . $photo_id . '\' AND location = \'albums\'');
    }
    
    public function deleteWallExport($product_id)
    {
        $this->db->query("DELETE FROM vk_export_photo WHERE product_id = " . $product_id . ' AND location = \'wall\'');
    }
    
    public function deleteWallExportOne($product_id, $photo_id)
    {
        $this->db->query("DELETE FROM vk_export_photo WHERE product_id = " . $product_id . ' AND vk_photo_id = \'' . $photo_id . '\' AND location = \'wall\'');
    }
    
    public function updateProductExportDate($product_id)
    {
        $this->db->query("UPDATE vk_export_photo SET `date` = NOW() WHERE product_id = " . $product_id . ' AND location = \'albums\'');
    }
    ...
    public function getProducts($data = array())
    {
        if ($data) {
            $sql = "SELECT SQL_CALC_FOUND_ROWS p.*, pd.*,
                (SELECT COUNT(*) FROM vk_export_photo WHERE product_id = p.product_id AND location = 'albums') AS export_albums,
                (SELECT COUNT(*) FROM vk_export_photo WHERE product_id = p.product_id AND location = 'wall') AS export_wall
                FROM product p
            ...
            if (isset($data['filter_export_albums']) && is_numeric($data['filter_export_albums']) && $data['filter_export_albums']) {
                $sql .= " AND (SELECT COUNT(*) FROM vk_export_photo WHERE product_id = p.product_id AND location = 'albums') >= 1";
            } elseif (is_numeric($data['filter_export_albums']) && !$data['filter_export_albums']) {
                $sql .= " AND (SELECT COUNT(*) FROM vk_export_photo WHERE product_id = p.product_id AND location = 'albums') = 0";
            }
            
            if (isset($data['filter_export_wall']) && is_numeric($data['filter_export_wall']) && $data['filter_export_wall']) {
                $sql .= " AND (SELECT COUNT(*) FROM vk_export_photo WHERE product_id = p.product_id AND location = 'wall') >= 1";
            } elseif (is_numeric($data['filter_export_wall']) && !$data['filter_export_wall']) {
                $sql .= " AND (SELECT COUNT(*) FROM vk_export_photo WHERE product_id = p.product_id AND location = 'wall') = 0";
            }
            ...
            if (!$product_data) {
                $query = $this->db->query("SELECT * FROM product p LEFT JOIN product_description pd ON (p.product_id = pd.product_id) WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY pd.name ASC");
    ...
    public function getProductsAutoexport()
    {
        $sql = "SELECT p.*, pd.*, ptc.category_id
            FROM product p
            JOIN product_to_category ptc ON (p.product_id = ptc.product_id)
            LEFT JOIN product_description pd ON (p.product_id = pd.product_id)
            WHERE 
        ...
        if ($this->config->get('vk_export_albums_only_specials')) {
            $sql .= ' AND (SELECT COUNT(*) from product_special WHERE product_id = p.product_id)';
        }
    ...
    public function getProductsToAutoupdate()
    {
        $sql = "SELECT p.*, pd.*, vep.vk_photo_id
            FROM vk_export_photo vep
            JOIN product p ON p.product_id = vep.product_id
            LEFT JOIN product_description pd ON (p.product_id = pd.product_id)
            WHERE 
            vep.location = 'albums'
    ...
    {
        $sql = "SELECT p.*, pd.*, ptc.category_id,
            (SELECT category_id FROM vk_export_photo WHERE product_id = p.product_id AND location = 'albums' ORDER BY date DESC LIMIT 1) AS vkep_category_id
            FROM product p
            JOIN product_to_category ptc ON (p.product_id = ptc.product_id)
            LEFT JOIN product_description pd ON (p.product_id = pd.product_id)
            WHERE
    ...
    {
        $query = $this->db->query("SELECT name FROM option_value_description 
    ...
    public function getProduct($product_id)
    {
        $query = $this->db->query("SELECT p.*, pd.name,
            (SELECT keyword FROM url_alias WHERE query = 'product_id=" . (int)$product_id . "' AND language_id = '" . (int)$this->config->get('config_language_id') . "') AS keyword,
    ...
    {
        $query = $this->db->query("SELECT * FROM vk_export_photo WHERE product_id = " . (int)$product_id . ' ORDER BY date DESC');
    ...
    {
        $query = $this->db->query("SELECT vkep.*, cd.name AS category FROM vk_export_photo AS vkep
			LEFT JOIN category_description cd ON cd.category_id = vkep.category_id AND cd.language_id = " . (int)$this->config->get('config_language_id') . "
    ...
    {
        $query = $this->db->query("SELECT * FROM vk_export_photo WHERE location = 'wall' AND product_id = " . (int)$product_id . ' ORDER BY date DESC');
