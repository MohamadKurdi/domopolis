...
class ModelModuleAffiliate extends Model {
    ...
    public function addTrackingCoupon($data = array()) {
        $query = $this->db->query("SELECT DISTINCT * FROM `affiliate` WHERE NOT coupon");			
        $data['logged'] = 0;
        $data['uses_total'] = 0;
        ...
        foreach ($query->rows as $row) {
            ...
            $this->db->query("UPDATE `affiliate` SET coupon = '" . (int)$idcoupon . "' WHERE affiliate_id = '" . (int) $row['affiliate_id'] . "'");
            $this->db->query("UPDATE `coupon` SET affiliate_id = '" . (int)$row['affiliate_id'] . "' WHERE coupon_id = '" . (int) $idcoupon . "'");
        }				
    }
     public function dellTrackingCoupon($coupon) {
            $this->db->query("UPDATE `affiliate` set coupon = NULL WHERE coupon ='" . $coupon . "'");
     }
     public function updateMaxDate() {
            $this->db->query("UPDATE `coupon` set date_end = ".date('Y-m-d H:i:s', PHP_INT_MAX)." WHERE affiliate_id != 0");
     }
     public function isAffilateCoupon($coupon_id) {
            $query = $this->db->query("SELECT * from `coupon` WHERE (coupon_id ='" . $coupon_id . "' and affiliate_id = 0)");
