<?php
	class ModelReportOnline extends Model {
		public function getCustomersOnline($data = array()) { 
			$sql = "SELECT co.ip, co.customer_id, co.url, co.referer, co.date_added, co.useragent, co.is_bot FROM " . DB_PREFIX . "customer_online co LEFT JOIN " . DB_PREFIX . "customer c ON (co.customer_id = c.customer_id)";
		...
		public function getTotalCustomersOnlineNotBotsFast($data = array()) {
			$sql = "SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "customer_online` WHERE is_bot = 0";
		...
		public function getTotalCustomersOnlineAppSessionsNotBotsFast($data = array()) {
			$sql = "SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "customer_online` WHERE is_bot = 0 AND is_pwa = 1";
