...
class ModelSaleReturn extends Model {
	public function addReturn($data) {
		$this->db->query("INSERT INTO `" . DB_PREFIX . "return` SET 
			...
			price_national  = '" . (int)$data['price_national'] . "',
			pricewd_national  = '" . (float)$data['pricewd_national'] . "',
			total  = '" . (int)$data['total'] . "',
			total_national  = '" . (int)$data['total_national'] . "',
			totalwd_national  = '" . (float)((float)$data['pricewd_national'] * (int)$data['quantity']) . "',
			...
			to_supplier = '" . (int)$data['to_supplier'] . "', 
			...
			$sql = "SELECT *, CONCAT(r.firstname, ' ', r.lastname) AS customer, (SELECT rs.name FROM " . DB_PREFIX . "return_status rs WHERE rs.return_status_id = r.return_status_id AND rs.language_id = '" . (int)$this->config->get('config_language_id') . "') AS status, (SELECT rr.name FROM " . DB_PREFIX . "return_reason rr WHERE rr.return_reason_id = r.return_reason_id AND rr.language_id = '" . (int)$this->config->get('config_language_id') . "') AS reason FROM `" . DB_PREFIX . "return` r";
				...
				if (!empty($data['filter_customer'])) {			
					if ((int)($data['filter_customer']) > 0){
						$implode[] = "r.customer_id = '" . (int)$data['filter_customer'] . "'";
					} else {
				...
				foreach ($query->rows as $row){
					if (!$row['product_id']){
						...
						$guess_id_query = $this->db->query("SELECT product_id FROM " . DB_PREFIX . "product WHERE model LIKE('". $this->db->escape($temp_model) ."')  LIMIT 1");
						
						if ($guess_id_query->row) {
							$product_id = $guess_id_query->row['product_id'];
							$this->db->query("UPDATE `" . DB_PREFIX . "return` SET product_id = '" . (int)$product_id . "' WHERE return_id = '" .(int)$row['return_id']. "'");
			...
			public function addReturnHistory($return_id, $data) {
				...
				if ($data['notify']) {
					...
					if ($return_query->num_rows) {
						...
						$template->data['show_summary'] = isset($data['show_summary']) ? $data['show_summary'] : 0;
