<?php
	class ModelSettingStore extends Model {
		...
		public function deleteStore($store_id) {
			...
			$this->db->query("DELETE FROM " . DB_PREFIX . "collection_to_store WHERE store_id = '" . (int)$store_id . "'");
			$this->db->query("DELETE FROM " . DB_PREFIX . "information_to_store WHERE store_id = '" . (int)$store_id . "'");
			$this->db->query("DELETE FROM " . DB_PREFIX . "information_attribute_to_store WHERE store_id = '" . (int)$store_id . "'");
			$this->db->query("DELETE FROM " . DB_PREFIX . "manufacturer_to_store WHERE store_id = '" . (int)$store_id . "'");
			$this->db->query("DELETE FROM " . DB_PREFIX . "set_to_store WHERE store_id = '" . (int)$store_id . "'");
		...
		public function getStores($data = array()) {
			...
			if ($this->user->getIsManager() && $this->user->getManagerStores()){
				...
				$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "store WHERE store_id IN (" . implode(',', $stores) . ") ORDER BY store_id ASC");
				...
				if (!empty($data['exclude_de'])){
					$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "store WHERE store_id <> 18 ORDER BY store_id ASC");
				}
