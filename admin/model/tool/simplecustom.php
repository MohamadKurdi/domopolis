...
class ModelToolSimpleCustom extends Model {
    ...
    private function getField($type, $id, $field, $set, $label = false) {
        ...
        if ($query->num_rows) {
            ...
            foreach ($data as $key => $field_info) {
                if (!empty($field_info['set']) && $field_info['set'] == 'order' && $field_info['field_id'] == $field) {
                    if (!$label) {
	...
	public function prepareDaDataEmptyFields($data){			
			$emptyArray = array (
		...
		$this->db->query("INSERT IGNORE INTO simple_custom_data SET object_type = 1, object_id = '" . (int)$data['id'] . "', customer_id = '" . (int)$data['customer_id'] . "', data = '" . $this->db->escape(serialize($emptyArray)) . "'");
