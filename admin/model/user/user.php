<?php
	class ModelUserUser extends Model {
		...
		public function getUser($user_id) {
			$query = $this->db->query("SELECT *, CONCAT(firstname, ' ', lastname) as realname  FROM `" . DB_PREFIX . "user` WHERE user_id = '" . (int)$user_id . "'");
			
			return $query->row;
		}
		
		public function getUserISHeadSales($user_id) {
			$query = $this->db->query("SELECT is_headsales FROM `" . DB_PREFIX . "user` WHERE user_id = '" . (int)$user_id . "'");
			
			return $query->row['is_headsales'];
		}
		
		public function getHeadSalesUserID() {
			$query = $this->db->query("SELECT user_id FROM `" . DB_PREFIX . "user` WHERE is_headsales = 1 LIMIT 1");
		...
		public function getUserBitrixIDByID($user_id) {
			$query = $this->db->query("SELECT bitrix_id FROM `" . DB_PREFIX . "user` WHERE user_id = '" . (int)$user_id . "'");
		...
		public function getUserBitrixIDByUserName($username) {
			$query = $this->db->query("SELECT bitrix_id FROM `" . DB_PREFIX . "user` WHERE username = '" . $this->db->escape($username) . "'");
		...
		public function getUserNameById($id) {
			$query = $this->db->query("SELECT username FROM `" . DB_PREFIX . "user` WHERE user_id = '" . $this->db->escape($id) . "' LIMIT 1");
		...
		public function getRealUserNameById($id) {
			$query = $this->db->query("SELECT CONCAT(firstname, ' ', lastname) as realname FROM `" . DB_PREFIX . "user` WHERE user_id = '" . $this->db->escape($id) . "' LIMIT 1");
		...
		public function getRealUserFirstNameByUsername($username) {
			$query = $this->db->query("SELECT firstname as realname FROM `" . DB_PREFIX . "user` WHERE username = '" . $this->db->escape($username) . "' LIMIT 1");
		...
		public function getRealUserLastNameById($id) {
			$query = $this->db->query("SELECT lastname as realname FROM `" . DB_PREFIX . "user` WHERE user_id = '" . $this->db->escape($id) . "' LIMIT 1");
		...
		public function getUsersByGroup($user_group_id, $enabled = false){
			if (!$enabled) {
				$query = $this->db->query("SELECT user_id, CONCAT(firstname, ' ', lastname) as realname, internal_pbx_num FROM `" . DB_PREFIX . "user` WHERE user_group_id = '" . (int)$user_group_id . "'");
				} else {
				$query = $this->db->query("SELECT user_id, CONCAT(firstname, ' ', lastname) as realname, internal_pbx_num FROM `" . DB_PREFIX . "user` WHERE user_group_id = '" . (int)$user_group_id . "' AND status = 1");
			}
		...
		public function getUsersByGroups(array $user_group_id, $enabled = false){
			...
			if (!$enabled) {
				$query = $this->db->query("SELECT user_id, CONCAT(firstname, ' ', lastname) as realname, internal_pbx_num FROM `" . DB_PREFIX . "user` WHERE user_group_id IN (" . $user_group_id . ")");
				} else {
				$query = $this->db->query("SELECT user_id, CONCAT(firstname, ' ', lastname) as realname, internal_pbx_num FROM `" . DB_PREFIX . "user` WHERE user_group_id IN (" . $user_group_id . ") AND status = 1");
			}			
		...
		public function getUsersByGroupsOwnOrders(array $user_group_id, $enabled = false){
			...
			if (!$enabled) {
				$query = $this->db->query("SELECT user_id, CONCAT(firstname, ' ', lastname) as realname, internal_pbx_num FROM `" . DB_PREFIX . "user` WHERE own_orders = 1 AND user_group_id IN (" . $user_group_id . ")");
				} else {
				$query = $this->db->query("SELECT user_id, CONCAT(firstname, ' ', lastname) as realname, internal_pbx_num FROM `" . DB_PREFIX . "user` WHERE own_orders = 1 AND user_group_id IN (" . $user_group_id . ") AND status = 1");
			}			
		...
		public function getUsersTelephonyQueues(){
			...
			$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "user_group` WHERE LENGTH(sip_queue) > 0");
