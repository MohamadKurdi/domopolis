            ...
            <table style="width: 100%;">
                    <tbody>
                        <tr class="filter f_top">
                              <td class="left">
                                <p><i class="fa fa-calendar"></i> Заказы от</p>
                                <input type="date" step="1" name="filter_date_from" value="<?php echo $filter_date_from; ?>" />
                            </td>   

                            <td class="left">
                                <p><i class="fa fa-calendar"></i> Заказы до</p>
                                <input type="date" step="1" name="filter_date_to" value="<?php echo $filter_date_to; ?>" />
                            </td>      
            ...
            <table class="list">
                <thead>
                    <tr> 
                        ...
                        <td class="center" style="width:140px;" width="140px"></td>
                    </tr>
                </thead>
                <tbody>
                    <?php if ($products) { ?>
                        <?php foreach ($products as $product) { ?>
                            <tr>
                                <td class="center" style="width:100px;"><img src="<?php echo $product['image']; ?>" /></td>
                                    ...
                                    <div>
                                        <div style="text-align: left; height:20px; margin-bottom:5px;">
                                           ...
                                           <?php if (mb_strlen($product['native_name']) >= $this->config->get('config_openai_shortennames_length') && $this->config->get('config_openai_enable_shorten_names')) { ?>   
                                               <span class="smallbutton black" onclick="shortenbyai($(this), 'native', '<?php echo $native_language['code']; ?>', <?php echo $product['product_id']; ?>)">🤖 AI до <?php echo $this->config->get('config_openai_shortennames_length'); ?> символов или слов</span><span></span>
                                           <?php } ?>
                                       </div>
                                        ...
                                        <div style="text-align: left; height:20px; margin-bottom:5px; margin-top:5px;">
                                            <img src="<?php echo DIR_FLAGS_NAME; ?><?php echo $amazon_language['image']; ?>" title="<?php echo $amazon_language['name']; ?>" />
                                              ...
                                              <?php if (mb_strlen($product['amazon_name']) >= $this->config->get('config_openai_shortennames_length') && $this->config->get('config_openai_enable_shorten_names')) { ?>   
                                               <span class="smallbutton black" onclick="shortenbyai($(this), 'amazon', '<?php echo $amazon_language['code']; ?>', <?php echo $product['product_id']; ?>)">🤖 AI до <?php echo $this->config->get('config_openai_shortennames_length'); ?> символов или слов</span><span></span>
                                             <?php } ?>                                  
                                            </div>
                                ...
                                <td class="center" style="width:140px;" width="140px"> 
...
<script>
    function filter() {
        url = 'index.php?route=catalog/shortnames2&token=<?php echo $token; ?>';
        
        var filter_date_from = $('input[name=\'filter_date_from\']').prop('value');       
        if (filter_date_from) {
            url += '&filter_date_from=' + encodeURIComponent(filter_date_from);
        }

        var filter_date_to = $('input[name=\'filter_date_to\']').prop('value');       
        if (filter_date_to) {
