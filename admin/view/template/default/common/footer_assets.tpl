...
<style>
    ...
    .admin_button i{font-size:56px;opacity:.8;padding:20px}
    .admin_button_status i{font-size:56px;opacity:.8}
    ...
    #top{bottom:40px;cursor:pointer;display:none;border:3px solid #51a881;position:fixed;right:61px;width:45px;height:45px;line-height:45px;text-align:center;opacity:1;-webkit-transition:.3s all;-o-transition:.3s all;transition:.3s all;z-index:99;color:#51a881;border-radius:100px;box-shadow:1px 1px 10px #626262cc}
    #top:hover{background:#51a881;color:#fff}
    #top svg{width:20px;height:20px;position:absolute;left:0;right:0;top:0;bottom:0;margin:auto}
    #top:hover svg path{stroke:#fff}
    .delayed-load{margin:0 auto}
    .lds-hourglass{display:inline-block;position:relative;width:80px;height:80px}
    .lds-hourglass:after{content:" ";display:block;border-radius:50%;width:0;height:0;margin:8px;box-sizing:border-box;border:32px solid #ff7815;border-color:#ff7815 transparent;animation:lds-hourglass 1.2s infinite}
        ...
        $('.delayed-load').each(function () {
            ...
            if (current.attr('data-reload')) {
                setInterval(function () {
                    loadDelayedLoad(current, false);
                }, parseInt(current.attr('data-reload')));
            }
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        setInterval(function () {
            $.ajax({url: 'index.php?route=common/home/session&nolog=1&token=<? echo $token ?>'});
        }, 10000);
    });
</script>
<? if ($this->user->getIPBX()) { ?>
<style>
    .click2call{padding:2px;display:inline-block;font-family: FontAwesome;font-size:20px;color:#1f4962;cursor:pointer;} .click2call:before{content:"\f098"} .fixed-header {
    ...
    $(document).ready(function () {
        $(document).on('click', '.click2call', function () {
            ...
            swal({
                    ...
                    confirmButtonText: "Позвонить сейчас!",
...
<script>
    function addMyTask(query) {
        $('#add_task_dialog').load('index.php?route=user/ticket/showAddTicketFormAjax&token=<? echo $token ?>',
            ...
            function () {
                $(this).dialog({width:800, modal:true,resizable:true,position:{my: 'center', at:'center center', of: window}, closeOnEscape: true, title: 'Создание новой задачи'});
            });
    }
    ...
    $(document).ready(function () {
        ...
        $('#trigger_add_task').click(function () {
            addMyTask('<?php echo base64_encode($query_string); ?>');
            ;
        });
