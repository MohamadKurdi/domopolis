...
<div id="content">
		...
		<div class="content">
			...
			<div id="tab-empty">
				<table class="list">
					<thead>
						<tr>
							<td class="left" width="50%"><form id="form-delete"><a class="button" onclick="editProducts('delete', 'delete');">Удалить</a></form></td>
							...
							<table class="list">
								...
								<tbody>
									<tr>
										...
										<td class="center"><input style="width: 300px;" type="text" name="product_autocomplete[text_start]" value="" size="64" /></td>
										<td class="left">
											<?php foreach ($autocomplete['tpl'] as $field) { ?>
												<input id="product_autocomplete[tpl][<?php echo $field; ?>]" class="checkbox" type="checkbox" name="product_autocomplete[tpl][<?php echo $field; ?>]" value="1" />
												<label for="product_autocomplete[tpl][<?php echo $field; ?>]"><?php echo ${'column_' . $field}; ?></label>
												<br />
											    <br />
											<?php } ?>
										</td>
										<td class="center"><input style="width: 300px;" type="text" name="product_autocomplete[text_end]" value="" size="64" /></td>
										<td class="left">
											<?php foreach ($autocomplete['app'] as $field) { ?>
												<input id="product_autocomplete[app][<?php echo $field; ?>]" class="checkbox" type="checkbox" name="product_autocomplete[app][<?php echo $field; ?>]" value="1" />
											<label for="product_autocomplete[app][<?php echo $field; ?>]"><?php echo ${'column_' . $field}; ?></label>
												<br />
											    <br />
											<?php } ?>
										</td>
	...
	<script type="text/javascript"><!--
		...
		function addAttribute(table) {
			...
			html += '<img src="view/image/delete.png" title="Удалить" alt="Удалить" /></a></td>';
		...
		function addOptionValue(form, option_row) {
			...
			html += '    <td class="left"><a onclick="$(\'#option-value-row-' + form + '-' + option_value_row + '\').remove();" class="button">Удалить</a></td>';
			html += '  </tr>';
			html += '</tbody>';
		...
		function addSpecial(table) {
			...
			html += '<tr class="filter">';
			html += '<td class="center"><a onclick="$(\'#' + table + ' #special-row' + special_row + '\').remove();"><img alt="Удалить" title="Удалить" src="view/image/delete.png"/></a></td>';
		...
		function addDiscount(table) {
			...
			html += '<tr class="filter">';
			html += '<td class="center"><a onclick="$(\'#' + table + ' #discount-row' + discount_row + '\').remove();"><img alt="Удалить" title="Удалить" src="view/image/delete.png"/></a></td>';
		...
		function addImage(table) {
			...
			html += '<td class="center"><a onclick="$(\'#image-row' + image_row + '\').remove();"><img src="view/image/delete.png" alt="Удалить" title="Удалить" /></a></td>';
					...
					$.ajax({
						...
						success: function(json) {
							response($.map(json, function(item) {
								...
								$('#product #thumb-' + arr[2]).attr('src', data['value']).before('<div class="image_remove" id="image_remove-'  + arr[2] + '" title="Удалить"></div>');
								}
								}
								});
								}
