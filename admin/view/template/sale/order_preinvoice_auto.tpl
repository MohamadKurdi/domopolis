<div>
	...
	<table class="product" id="preinvoice_table" style="width:100%">
		<tr>
			...
			<td colspan="2" style="text-align:left; font-size:14px; padding-bottom:8px;border:0; word-wrap:none; white-space: nowrap;">
				<b><nobr>Товарный чек № <? echo $cheque_num; ?></nobr></b>
			</td>
			<td colspan="1" style="text-align:right; font-size:14px; padding-bottom:8px;border:0">
				<b><nobr><? echo $cheque_date; ?></nobr></b>
			</td>
			<? /* if (isset($order['customer']['discount_card']) && mb_strlen($order['customer']['discount_card']) > 2) { ?>
				<td colspan="3" style="text-align:center; padding-left:5px; font-size:14px; padding-bottom:8px;border:0">
			...
			<tr>
				...
				<td colspan="2">	
					<span style="text-align:left;" type="text" class="invoice_total_input" value="<? echo $order['order_info']['firstname']; ?>&nbsp;<? echo isset($order['order_info']['lastname'])?$order['order_info']['lastname']:''; ?>">
						<? echo $order['order_info']['firstname']; ?>&nbsp;<? echo isset($order['order_info']['lastname'])?$order['order_info']['lastname']:''; ?>
					</span>
				</td>
				<td colspan="3">
					<span style="text-align:left;" type="text" class="invoice_total_input" value="<? echo $order['order_info']['telephone']; ?>"><? echo $order['order_info']['telephone']; ?></span>
				</td>
			</tr>
			...
			<tr>   
				...
				<td colspan="2">
					<? if (isset($not_same_customer)) { ?>
						<span style="text-align:left;" type="text" class="invoice_total_input" value="<? echo $order['order_info']['shipping_firstname']; ?>&nbsp;<? echo isset($order['order_info']['shipping_lastname'])?$order['order_info']['shipping_lastname']:''; ?>">
							<? echo $order['order_info']['shipping_firstname']; ?>&nbsp;<? echo isset($order['order_info']['shipping_lastname'])?$order['order_info']['shipping_lastname']:''; ?>
						</span>
						<? } else { ?>	
						<span style="text-align:left;" type="text" class="invoice_total_input" value=""></span>
					<? } ?>
				</td>
				<td colspan="3">
					<? if (isset($order['order_info']['fax']) && mb_strlen($order['order_info']['fax'])>1) { ?>					
						<span style="text-align:left;" type="text" class="invoice_total_input" value="">
							<? echo (isset($order['order_info']['fax']) && $order['order_info']['fax'])?$order['order_info']['fax']:''; ?>
						</span>
						<? } else { ?>			
						<span style="text-align:left;" type="text" class="invoice_total_input" value=""></span>
					<? } ?>
			...
			<? if (!empty($order['order_info']['shipping_passport_serie'])) { ?>
				<tr>   
					...
					<td colspan="5">
						...
						<span style="text-align:left;" type="text" class="invoice_total_input" value="<? echo $_ppassport; ?>"><? echo $_ppassport; ?></span><br />
						<span style="font-size:10px; color:#666; ">Согласно правилам оформления грузов, определенным в Федеральном законе от 6 июля 2016 г. № 374-ФЗ</span>
					</td>
			...
			<tr>
				...
				<td colspan="2">
					<span style="text-align:left;" type="text" class="invoice_total_input" value="<?php echo $order['shipping_address']; ?>"><?php echo $order['shipping_address']; ?></span>
				</td>
				<td colspan="3">
					<span style="text-align:left;" type="text" class="invoice_total_input" value="<?php echo $order['short_shipping_method']; ?>"><?php echo $order['short_shipping_method']; ?></span>
				</td>
			</tr>
			...
			<tr>
				...
				<td colspan="2" style="text-align:left;" align="left">
					<? if (false) { ?>
						...
						<? } ?>
					<? } ?>
					<span style="text-align:left;" type="text" class="invoice_total_input" value="<? echo $cheque_type; ?>"><? echo $cheque_type; ?></span>
				</td>
				<td colspan="3">
					<span style="text-align:left;" type="text" class="invoice_total_input" value="<?php echo $order['payment_method']; ?>"><?php echo $order['payment_method']; ?></span>
				</td>
			</tr>
			<tr>
				<td colspan="6" style="height:8px; border:0px;"></td>
			</tr>
					...
					<tr>
						...
						<td colspan="2" style="width:100%">
							<? if ($product['short_name'] && mb_strlen($product['short_name']) > 2) { ?>
								<span style="text-align:left;" type="text" class="invoice_total_input" value="<?php echo $product['short_name']; ?>" ><?php echo $product['short_name']; ?></span>
								<? } else { ?>
								<span style="text-align:left;" type="text" class="invoice_total_input" value="<?php echo $product['name']; ?>" ><?php echo $product['name']; ?></span>
							<? } ?>
				...
				<?php $i=0; foreach ($order['total'] as $total) { $i++; ?>
					<tr id="totals_cheque_<? echo $i; ?>" class="tr_total_draggable"> 
						<td align="left" colspan="7" style="cursor:move; white-space: nowrap; padding-left:5px;padding-right:5px;">
							<span style="text-align:left"><b><?php echo $total['title']; ?></b></span>		
						</td>
						<td align="right" style="white-space: nowrap; padding-left:5px;padding-right:5px;">
							<? if (!($total['text'] === false)) { ?>
								<span <? if ($total['code'] == 'total') { ?>id="cheque_total_total" disabled="disabled"<? } elseif ($total['code'] == 'sub_total') { ?>disabled="disabled"<? } else { ?>id="totals_text_input_<? echo $i; ?>"<? } ?> type="text" value="<?php echo $total['text']; ?>" >
