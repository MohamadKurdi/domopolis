      ...
      <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form">
        <table class="form">                   
          ...
          <tr>
           ...
           <td>
            <select name="paymentmethoddiscounts_status">
        ...
        <tr>
           <td>Отображать сумму в описании способа оплаты</td>
           <td>
            <select name="paymentmethoddiscounts_display_in_description">
              <?php if ($paymentmethoddiscounts_display_in_description) { ?>
                <option value="1" selected="selected"><?php echo $text_enabled; ?></option>
                <option value="0"><?php echo $text_disabled; ?></option>
              <?php } else { ?>
                <option value="1"><?php echo $text_enabled; ?></option>
      ...
      <table id="discounts" class="list">
        <thead>
          <tr>
            ...
            <td class="left">Способы доставки</td>
        ...
        <?php foreach ($paymentmethoddiscounts_discount as $discount) { ?>
          <tbody id="discount-row<?php echo $discount_row; ?>">
            <tr>
              <td class="left">
               <input name="paymentmethoddiscounts_discount[<?php echo $discount_row; ?>][paymentmethod]" value="<? echo $discount['paymentmethod']; ?>" style="width:250px;" />                
             </td>

             <td class="left">
               <textarea name="paymentmethoddiscounts_discount[<?php echo $discount_row; ?>][deliveries]" style="width:250px;"><? echo $discount['deliveries']; ?></textarea>                
             </td>
...
function addDiscount() {
  ...
  html += '<tr>';
  html += '<td class="left">';
  html += '<input name="paymentmethoddiscounts_discount['+ discount_row +'][paymentmethod]" value="" style="width:250px;" />    '
  html += '</td>';
  html += '<td class="left">';
