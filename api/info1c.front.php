<?php			
	...
	define('CONFIG_FILE', 'config.slave.php');

	require_once(APPLICATION_DIRECTORY . '/system/jsonconfig.php');
	...
	if (!empty($loaderConfig['preload'])){
		foreach ($loaderConfig['preload'] as $preloadFile){
			require_once(APPLICATION_DIRECTORY . $preloadFile . '.php');
		}
	}
	...
	$registry->set('log', 		new Log('php-errors-api-1c.log'));
	...
	if ($inputData['mode'] == 'product'){
		...
		foreach ($params as $param) {
			if (!isset($inputData[$param]) || !is_numeric($inputData[$param])){
				die("no $param");
			}
		}
		
		if ($inputData['type'] == 'getProductPrice'){
			$action = new Action('api/info1c/getProductPrice', array('product_id' => $inputData['product_id']));	
		}
