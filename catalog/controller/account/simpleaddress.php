...
class ControllerAccountSimpleaddress extends SimpleController {
    ...
    public function insert($args = null) {
        ...
        if ($this->simpleaddress->getAdditionalParams()){
                 $this->_templateData['action'] = $this->url->link('account/simpleaddress/insert', $this->simpleaddress->getAdditionalParams());
        } else {
                $this->_templateData['action'] = $this->url->link('account/simpleaddress/insert');
        }
