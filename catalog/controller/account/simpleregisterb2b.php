...
class ControllerAccountSimpleRegisterB2b extends Controller {

    private $error = array();

    public function index() {
        $this->document->addStyle('catalog/view/theme/mattimeo/stylesheet/loginb2b.css');
	...
	*/	
		$this->data['customer_group_id'] = $this->config->get('config_opt_group_id');
        ...
        if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->data['simple_create_account'] && $this->validate()) {
            ...
            $this->config->set('config_customer_group_id', $this->config->get('config_opt_group_id'));
            ...
            $data = array(
                ...
                'customer_group_id' => $this->config->get('config_opt_group_id')
            );
        ...
        $this->data['text_electronic_post'] = $this->language->get('electronic_post');
        $this->data['text_entry_password'] = $this->language->get('entry_password');
        $this->data['text_registration'] = $this->language->get('registration');
        $this->data['text_commerce_vid'] = $this->language->get('commerce_vid');
        $this->data['text_go_to_main_menu'] = $this->language->get('go_to_main_menu');
        $this->data['text_big_text_1'] = $this->language->get('big_text_1');
        $this->data['text_enter_to_system'] = $this->language->get('enter_to_system');
        $this->data['text_big_text_2'] = $this->language->get('big_text_2');



        $this->data['text_account_already']   = sprintf($this->language->get('text_account_already'), $this->url->link('account/loginb2b', '', 'SSL'));
        ...
        if ($this->config->get('simple_registration_agreement_id')) {
            ...
            if ($information_info) {
                ...
                $this->data['text_agree'] = sprintf($this->language->get($text), $this->config->get('simple_registration_agreement_id'), $information_info['title'], $information_info['title']);
            }
        }
        ...
        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/account/simpleregisterb2b.tpl')) {
            $this->template = $this->config->get('config_template') . '/template/account/simpleregisterb2b.tpl';
            $this->data['template'] = $this->config->get('config_template');
        } else {
            $this->template = 'default/template/account/simpleregisterb2b.tpl';
