<?
	class ControllerCheckoutQuickorder extends Controller {
		...
		public function createorder() {
			...
			if (isset($this->request->post['quickorder-dialog-phone']) && mb_strlen($this->request->post['quickorder-dialog-phone'],'UTF-8')>=7  && $this->validatePhone($this->request->post['quickorder-dialog-phone'])) {
				$phone = $this->request->post['quickorder-dialog-phone'];
				} else {
				$json['error']['phone'] = sprintf($this->language->get('error_required'), 'Телефон');
			}
			
			if (!isset($this->request->post['quickorder_key']) || !isset($this->session->data['quickorder_key']) || ($this->session->data['quickorder_key'] != $this->request->post['quickorder_key'])) {
				//$json['error']['quickorder_key'] = 'key error';
			}
			...
			if (!$json) {	
				...
				if ($product_info) {
					...
					//В корзину добавили товар. Зер гут.
					//теперь, когда товар в корзине, нужно просто подвердить заказ.																																							
