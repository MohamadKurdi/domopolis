	...
	*/
	...
	class ControllerCheckoutSimpleCheckout extends SimpleController {
		...
		public function index($args = null) {
			...
			if ($this->cart->hasProducts() || !empty($this->session->data['vouchers'])) {
				...
				if (isset($this->request->post['reward'])){
					if (!empty($this->request->post['reward'])){
						$this->session->data['reward'] = abs((int)$this->request->post['reward']);
						} else {
			...
			$this->_templateData['enable_reloading_of_payment_form'] = true;//$this->simplecheckout->getSettingValue('enableAutoReloaingOfPaymentFrom');
			...
			$this->_templateData['default_payment_btn_success'] = $this->url->link('checkout/success');
			...
			if (!empty($this->request->post['next_step']) && (int)$this->request->post['next_step'] > 1){
				$this->_templateData['next_step'] = (int)$this->request->post['next_step'];
				} else {
				$this->_templateData['next_step'] = 1;
			}
		...
		private function order() {
			...
			if ($version < 152) {
				...
				foreach ($this->cart->getProducts() as $product) {
                    ...
                    'name'       => $product['name'],
                    'model'      => $product['model'],
					'ao_product_id' => $product['ao_product_id'],
					'ao_id'      => $product['ao_id'],
                    'option'     => $option_data,
			...
			foreach ($data['totals'] as $data_total) {
				if ($data_total['code'] == 'total'){
					$data['total_national'] = $data_total['value_national'];
				}
			}
