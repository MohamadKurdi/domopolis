			...
			if($secret == $data['secret'] && $active == 1) {
				if($data['action'] == 'ItemUnsold') {
					...
					if($product_id != false) {
						...
						if(!empty($rules)) {
							...
							$this->db->query("INSERT INTO `ebay_listing_pending` SET `ebay_item_id` = '".$this->db->escape($data['itemId'])."', `product_id` = '".(int)$product_id."', `key` = '".$this->db->escape($data['key'])."'");
				...
				if($data['action'] == 'ItemListed') {
					...
					if($product_id != false) {
						...
						$this->db->query("DELETE FROM `ebay_listing_pending` WHERE `key` = '".$data['key']."' LIMIT 1");
