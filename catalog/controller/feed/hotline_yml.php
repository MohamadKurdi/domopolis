<?php
/**
* hotline.YML data feed for OpenCart (ocStore) 1.5.5.x
*
...
/**
 * Класс YML экспорта
 * YML (hotline Market Language) - стандарт, разработанный "Яндексом"
 * для принятия и публикации информации в базе данных Яндекс.Маркет
 * YML основан на стандарте XML (Extensible Markup Language)
 * описание формата YML http://partner.market.hotline.ru/legal/tt/
 */
//Версия модуля
define ('hotline_YML_VERSION', '1.6.6.1');
...
//---- Config section ----
	protected $CONFIG_PREFIX = 'hotline_yml_';
	...
	protected function generateYml() {
		if ($this->config->get($this->CONFIG_PREFIX.'status')) {
                        ...
                        if (!defined('HTTP_IMAGE')) {
                            define('HTTP_IMAGE', $HTTP_SERVER . DIR_IMAGE_NAME);
                        }
			...
			$this->setShop('platform', 'hotline.YML for '.$this->config->get('config_name'));
			$this->setShop('version', hotline_YML_VERSION);
			...
			foreach ($currencies as $currency) {
				if ($currency['code'] != $offers_currency && $currency['status'] == 1) {
					if ($this->currency->convert($currency['value'], $offers_currency, $shop_currency) != 0) {
						$this->setCurrency($currency['code'], number_format(1/$this->currency->convert($currency['value'], $offers_currency, $shop_currency), 0, '.', ''));
					} else {
			...
			// Категории
			$categories = $this->model_export_hotline_yml->getCategory();
			...
			$products = $this->model_export_hotline_yml->getProduct($allowed_categories, $blacklist_type, $blacklist, $out_of_stock_id, $vendor_required, $allowed_manufacturers, $product_rel || $product_accessory);

			$numpictures = $this->config->get($this->CONFIG_PREFIX.'numpictures');
			if ($numpictures > 1) {
				//++++ Дополнительные изображения товара ++++
				$product_images = $this->model_export_hotline_yml->getProductImages($numpictures - 1);
				//---- Дополнительные изображения товара ----
			}
			$all_attributes = $this->model_export_hotline_yml->getAttributes($this->config->get($this->CONFIG_PREFIX.'attributes'));
			$this->optioned_name = $this->config->get($this->CONFIG_PREFIX.'optioned_name');
			
			$hotline_yml_categ_mapping = unserialize($this->config->get($this->CONFIG_PREFIX.'categ_mapping'));
...
//				$data['cbid'] = 15;
				...
				if (isset($hotline_yml_categ_mapping[$product['category_id']]) && $hotline_yml_categ_mapping[$product['category_id']]) {
					$data['market_category'] = $hotline_yml_categ_mapping[$product['category_id']];
				}
				$data['delivery'] = 'true';
...
//				$data['barcode'] = $product['sku'];
				...
				*/
				$data['param'] = array();
				$attributes = $this->model_export_hotline_yml->getProductAttributes($product['product_id']);
				$attr_text = array();
	...
	protected function setOptionedOffer($data, $product, $shop_currency, $offers_currency, $decimal_place) {
		...
		if ($this->color_options)
			$coptions = $this->model_export_hotline_yml->getProductOptions($this->color_options, $product['product_id']);
		$soptions = array();
		if ($this->size_options)
			$soptions = $this->model_export_hotline_yml->getProductOptions($this->size_options, $product['product_id']);
