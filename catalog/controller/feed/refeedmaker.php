<?
	class ControllerFeedReFeedMaker extends Controller { 
		...
		protected function printItemFast($product, $changeID = true){
			...
			if ($this->config->get('config_language_id') == $this->exclude_language_id && $changeID){
		...
		public function supplemental(){
			...
			foreach ($this->stores as $store_id => $languages){
				
				foreach ($languages as $language_id){
					foreach (array(0,1) as $changeID){
						
						//Пропускаем если язык не украинский и нужно менять айдишку
						if ($language_id != $this->exclude_language_id && $changeID){
							continue;
						} 
						
						if ($language_id == $this->exclude_language_id){
							if ($changeID){
		...
		protected function printItem($product, $google_base_category, $changeID = true){	
			...
			$link = $this->url->link('product/product', 'product_id=' . $product['product_id'], 'SSL', $this->language_id);
			
			if ($this->urlcode){
				$link = str_replace(parse_url($link, PHP_URL_HOST), parse_url($link, PHP_URL_HOST) . '/' . $this->urlcode, $link);
			}
			
			$output .= '<link>' . $link . '</link>' . PHP_EOL;
