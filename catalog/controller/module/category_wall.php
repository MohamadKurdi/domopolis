<?php
	class ControllerModuleCategoryWall extends Controller {
		...
		*/
			...
			$setting_checksum = sha1(serialize($setting));
			
			$this->bcache->SetFile('module.'.$store_id.$language_id.$currency_id.$setting_checksum.$module.'.tpl', 'categorywall');	
			...
			if ($this->bcache->CheckFile()) {		
				...
				if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/module/' . $tpl)) {
					$this->template = $this->config->get('config_template') . '/template/module/' . $tpl;
					} else {
