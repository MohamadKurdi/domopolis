...
class ControllerModuleDSocialLogin extends Controller {
	protected function index($setting) {
		...
		if($settings['base_url_index']){
			...
			$settings['base_url'] = HTTPS_SERVER.'catalog/model/d_social_login/hybridauth.php';
		}
...
*/
	...
	private function provider_edit_customer($customer_id, $data){
		...
		$this->db->query("UPDATE customer SET `".$data['provider']."_id` = '" . $this->db->escape($data['id']) . "' WHERE customer_id = '" . (int)$customer_id . "'");
		
	}

	private function get_customer_by_provider_id($provider, $id){
		if($provider || $id){
			$query = $this->db->query("SELECT * FROM customer WHERE `".$provider."_id` = '" . $this->db->escape($id) . "'");
			return $query->row;
		}else{
			return false;
		}
	...
	private function getCountryIdByName($country_name){
		$query = $this->db->query("SELECT * FROM country WHERE LOWER(name) LIKE '" . $this->db->escape(utf8_strtolower($country_name)). "' AND status = '1'");
	...
	public function getCustomerByEmail($email) {
		$query = $this->db->query("SELECT * FROM customer WHERE LOWER(email) = '" . $this->db->escape(utf8_strtolower($email)) . "'");
