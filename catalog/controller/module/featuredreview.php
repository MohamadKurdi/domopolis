<?php
	class ControllerModulefeaturedreview extends Controller {
		protected function index($setting) {
			...
			$manufacturer_id = (isset($this->request->get['manufacturer_id']) && (int)$this->request->get['manufacturer_id'] > 0)?(int)$this->request->get['manufacturer_id']:0;
			...
			//TO DO, LATER
			/*	if (isset($this->request->get['route']) && $this->request->get['route'] == 'product/collection' && $collection_id){
				$page_type = 'collection';
				}
			*/
			
			$this->bcache->SetFile('module.'.$store_id.$manufacturer_id.$category_id.$language_id.$currency_id.md5(serialize($setting) . $md5_template).'.tpl', 'featuredreview');
			...
			if ($this->bcache->CheckFile()) {		
				...
				// Microdata
				$microdata_reviews = array(); // отзывы
				$microdata_rating_score = array(); // оценки отзывов
				// Microdata
				...
				if (isset($this->request->get['route']) && ($this->request->get['route'] == 'product/category' || strpos($this->request->get['route'], 'product/manufacturer') !== false)){
					$template = 'featuredreview_catalog';
				}
