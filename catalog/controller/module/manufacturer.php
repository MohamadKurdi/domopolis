...
class ControllerModuleManufacturer extends Controller {
	protected function index($setting) {
		...
		if (isset($this->request->get['manufacturer_id']) && $this->request->get['manufacturer_id'] > 0){
			...
			$this->bcache->SetFile('module.'.$language_id.$currency_id.$manufacturer_id.$category_id.(int)$this->data['do_not_show_manufacturer_list'].$position.md5(serialize($setting)).'.tpl', 'manufacturer_single_module'.$store_id);
		...
		//	$coresults = $this->model_catalog_collection->getCollectionsByManufacturer($manufacturer_id, 300);
			
			$this->data['manufacturer'] = $this->model_catalog_manufacturer->getManufacturer($manufacturer_id);
			$this->data['category_id'] = $category_id;
			
			//У производителя нет коллекций. Значит нам нахуй не нужны и категории
			if (/*count($coresults) > 1 || */true) {						
			...
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/module/manufacturer_single.tpl')) {
				$this->template = $this->config->get('config_template') . '/template/module/manufacturer_single.tpl';
			} else {
				$this->template = 'default/template/module/manufacturer_single.tpl';
			}
		...
		$this->document->addStyle('catalog/view/theme/mattimeo/stylesheet/mattimeomenucategory.css');
		
		$this->bcache->SetFile('module.'.$store_id.$language_id.$currency_id.$position.'.tpl', 'manufacturer_module');
