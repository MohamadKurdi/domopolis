    ...
    class ControllerModuleOCFilter extends Controller {
        ...
        public function __construct($registry) {
            ...
            if (false !== $this->ocfilter) {
                $this->_data = $this->ocfilter->_data;
            ...
            $this->option_keywords['s'] = 'stock';
            $this->option_keywords['m'] = 'brand';
        ...
        public function &__get($key) {
            if (isset($this->_data[$key])) {
                ...
                } else if (!is_null($this->registry->get($key))) {
        ...
        public function index($settings = array()) {
            ...
            $this->data['link']                 = str_replace('&amp;', '&', $this->link());
            
            $this->data['params']               = $this->params;
            
            $this->data['index']   							= $this->config->get('ocfilter_url_index');
            $this->data['show_counter']         = $this->config->get('ocfilter_show_counter');
            $this->data['show_values_limit']   	= $this->config->get('ocfilter_show_values_limit');
            $this->data['manual_price']         = $this->config->get('ocfilter_manual_price');
            ...
            $this->data['symbol_left']      		= $this->currency->getSymbolLeft($this->session->data['currency']);
            $this->data['symbol_right']      		= $this->currency->getSymbolRight($this->session->data['currency']);
            
            if ($this->min_price_get && $this->max_price_get) {
                $this->data['price'] = 'p' . $this->config->get('ocfilter_sep_opt') . $this->min_price_get . '-' . $this->max_price_get;
                } else {
                $this->data['price'] = '';
            }
            ...
            if ($this->config->get('ocfilter_show_options_limit') && $this->config->get('ocfilter_show_options_limit') < count($this->data['options'])) {
                $this->data['show_options_limit'] = $this->config->get('ocfilter_show_options_limit');
                } else {
                $this->data['show_options_limit'] = false;
            }
            
            $this->document->addStyle('catalog/view/javascript/ocfilter/nouislider.min.css');
            $this->document->addStyle('catalog/view/javascript/ocfilter/bootstrap.min.css');
            ...
            $this->document->addScript('catalog/view/javascript/ocfilter/bootstrap.min.js');
            $this->document->addScript('catalog/view/javascript/ocfilter/ocfilter.js');
            
            if (file_exists(DIR_TEMPLATE . $this->config->get('config_theme') . '/template/module/ocfilter.tpl')) {
                $this->template = $this->config->get('config_theme') . '/template/module/ocfilter.tpl';
                } else {
        ...
        public function decode() {
            ...
            if ($this->config->get('config_second_language')){				
				if($_route_){
					...
					$urllanguage = explode('/', trim(utf8_strtolower($_route_), '/'));
					...
					if(isset($urllanguage[0]) && in_array($urllanguage[0], $lang)){
						...
						$_route_ = str_replace($replace_lang, '', $_route_);					
						if($_route_ == '' ||  $_route_ == '/'){
							return;
                        }
                    }
            ...
            if (!$this->path) {
                ...
                if ($or) {
                    $path = $this->cache->get('category.ocfilter.path.' . (int)$this->config->get('config_language_id') . md5(implode(".", $implode)));
                    
                    if ($path) {
                        $this->path = $path;
                        } else {
                        $query = $this->db->query("SELECT GROUP_CONCAT(REPLACE(`query`, 'category_id=', '') ORDER BY FIELD(keyword, " . implode(", ", $implode) . ") SEPARATOR '_') AS path FROM " . DB_PREFIX . "url_alias WHERE (language_id = '" . (int)$this->config->get('config_language_id') . "' OR language_id = '-1') AND `query` LIKE 'category_id=%' AND (" . implode(" OR ", $or) . ")");
                        
                        if (!empty($query->row['path'])) {
                            $this->path = $query->row['path'];
                            
                            $this->cache->set('category.ocfilter.path.' . (int)$this->config->get('config_language_id') . md5(implode(".", $implode)), $this->path);
                        }
                    }
                }
            }
