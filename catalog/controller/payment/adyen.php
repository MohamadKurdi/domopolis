<?php
	class ControllerPaymentAdyen extends Controller {
		...
		private $shoputils = 0;
		private $version;
		private $hmac_key = '75E19BB031E73F24E0A42F14388704A4203FE604399F5F94BED0CD03FECBD2D9';
        ...
        'RUB', 'RUR', 'UAH', 'EUR', 'USD', 'KZT', 'BYN', 'BYN'
		);
		...
		public function laterpay($order_id = false) {
			if ($this->validateLaterpay()) {
				...
				$sAmount = number_format($order_info['total_national'] * 100, 0, '.', '');
				...
				$this->data['adyen_params'] = array(
				'sessionValidity' => date('Y-m-d', strtotime("+3 week")),
				'shipBeforeDate' => date('c', strtotime("+1 day")),
				'shopperLocale'  => $this->locales[$this->config->get('config_store_id')],
				'merchantAccount' => 'VialexGmbHRU',
				'paymentAmount'   => $sAmount,
				'currencyCode'    => $order_info['currency_code'],
				'skinCode'        => 'SxL3OdgY',
				'merchantReference' => 'Order ' . $order_info['order_id'],
				'shopperReference' => $order_info['firstname'] . ' ' . $order_info['lastname'],
				'recurringContract' => 'RECURRING,ONECLICK',
				'shopperEmail'      => $order_info['email'],
				'offset'            => 0								
				);		
				
				$this->data['merchantSig'] = $this->generateSignature($this->data['adyen_params']);
				
				$this->data['noindex'] = true;
				if ($this->config->get('config_icon') && file_exists(DIR_IMAGE . $this->config->get('config_icon'))) {
					$this->data['icon'] = HTTP_SERVER . DIR_IMAGE_NAME . $this->config->get('config_icon');
					} else {
					$this->data['icon'] = '';
				}
				if ($this->config->get('config_logo') && file_exists(DIR_IMAGE . $this->config->get('config_logo'))) {
					$this->data['logo'] = HTTP_SERVER . DIR_IMAGE_NAME . $this->config->get('config_logo');
					} else {
					$this->data['logo'] = '';
				}		
		...
		private function generateSignature($params){
			...
			$binaryHmacKey 	= pack("H*" , $this->hmac_key);
		...
		public function validateRequest(){
			...
			$order_id = isset($this->request->get['merchantReference']) ? preg_replace("([^0-9])", "", $this->request->get['merchantReference']) : 0;
			...
			foreach ($allowed_params as $key) {
				if (!isset($this->request->get[$key])){
					$this->sendForbidden('Not enough params');
					die('Params Mismatch');
					return false;
				}
			}			

			$check_transaction = $this->db->non_cached_query("SELECT * FROM customer_transaction WHERE order_id = '" . (int)$orderid . "' AND added_from = 'adyen' AND pspReference = '" . $this->db->escape($this->request->get['pspReference']) . "'");
			if ($check_transaction->num_rows){				
				$this->sendRepeatError(var_export($this->request->get, true));
			...
			var_dump($this->generateSignature($request_data));
			
			if ($this->request->get['merchantSig'] == $this->generateSignature($request_data) || str_replace(' ', '+', $this->request->get['merchantSig']) == $this->generateSignature($request_data)){
				echo 'ok';
			} else {
				echo 'fail';
			}	
			...
			if ($this->order['order_status_id'] != $this->config->get('shoputils_psb_order_status_id')) {
				if ($this->order['order_status_id']){
					...
					if ($this->isPayingPrepay($this->order['order_id']) == 1) {
						...
						'Adyen: Предоплата по заказу # '.$this->order['order_id'], 
