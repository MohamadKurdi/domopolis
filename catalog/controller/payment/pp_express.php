...
class ControllerPaymentPPExpress extends Controller {
	protected function index() {
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/pp_express_order.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/payment/pp_express_order.tpl';
		} else {
			$this->template = 'default/template/payment/pp_express_order.tpl';
		}
	...
	public function express() {
		...
		unset($this->session->data['payment_secondary_method']);
	...
	public function expressReturn() {
		...
		if($this->session->data['paypal']['guest'] == true) {
			...
			if($this->cart->hasShipping()) {
				...
				$country_info = $this->db->query("SELECT * FROM `country` WHERE `iso_code_2` = '" . $this->db->escape($result['PAYMENTREQUEST_0_SHIPTOCOUNTRYCODE']) . "' AND `status` = '1' LIMIT 1")->row;
				...
				$zone_info = $this->db->query("SELECT * FROM `zone` WHERE (`name` = '" . $this->db->escape($returned_shipping_zone) . "' OR `code` = '" . $this->db->escape($returned_shipping_zone) . "') AND `status` = '1' AND `country_id` = '" . (int)$country_info['country_id'] . "' LIMIT 1")->row;
			...
			if($this->cart->hasShipping()) {
				...
				if($match == false) {
					...
					$country_info = $this->db->query("SELECT * FROM `country` WHERE `iso_code_2` = '" . $this->db->escape($result['PAYMENTREQUEST_0_SHIPTOCOUNTRYCODE']) . "' AND `status` = '1' LIMIT 1")->row;
					$zone_info = $this->db->query("SELECT * FROM `zone` WHERE `name` = '" . $this->db->escape($result['PAYMENTREQUEST_0_SHIPTOSTATE']) . "' AND `status` = '1' AND `country_id` = '" . (int)$country_info['country_id'] . "'")->row;
		...
		if ( (string)$response == "VERIFIED" )  {
			...
			if($transaction) {
				...
				if($transaction['payment_status'] != $this->request->post['payment_status']) {
					$this->db->query("UPDATE `paypal_order_transaction` SET `payment_status` = '" . $this->request->post['payment_status'] . "' WHERE `transaction_id` = '" . $this->db->escape($transaction['transaction_id']) . "' LIMIT 1");
					...
					$this->db->query("UPDATE `paypal_order_transaction` SET `pending_reason` = '" . $this->request->post['pending_reason'] . "' WHERE `transaction_id` = '" . $this->db->escape($transaction['transaction_id']) . "' LIMIT 1");
				}
			} else {
				if($this->config->get('pp_express_debug') == 1) {
				...
				if($parent_transaction) {
					...
					if(isset($this->request->post['payment_status']) && $this->request->post['payment_status'] == 'Refunded') {
						if(($this->request->post['mc_gross'] * -1) == $parent_transaction['amount']) {
							$this->db->query("UPDATE `paypal_order_transaction` SET `payment_status` = 'Refunded' WHERE `transaction_id` = '" . $this->db->escape($parent_transaction['transaction_id']) . "' LIMIT 1");
						} else {
							$this->db->query("UPDATE `paypal_order_transaction` SET `payment_status` = 'Partially-Refunded' WHERE `transaction_id` = '" . $this->db->escape($parent_transaction['transaction_id']) . "' LIMIT 1");
						}
					}
			...
			if (isset($this->request->post['txn_type'])) {
				...
				if ($this->request->post['txn_type'] == 'recurring_payment') {
					...
					if ($profile != false) {
						$this->db->query("INSERT INTO `order_recurring_transaction` SET `order_recurring_id` = '" . (int)$profile['order_recurring_id'] . "', `created` = NOW(), `amount` = '" . (float)$this->request->post['amount'] . "', `type` = '1'");

						//as there was a payment the profile is active, ensure it is set to active (may be been suspended before)
						if ($profile['status'] != 1) {
							$this->db->query("UPDATE `order_recurring` SET `status` = 2 WHERE `order_recurring_id` = '" . (int)$profile['order_recurring_id'] . "'");
						}
					}
				}
				...
				if ($this->request->post['txn_type'] == 'recurring_payment_suspended') {
					...
					if ($profile != false) {
						$this->db->query("INSERT INTO `order_recurring_transaction` SET `order_recurring_id` = '" . (int)$profile['order_recurring_id'] . "', `created` = NOW(), `type` = '6'");
						$this->db->query("UPDATE `order_recurring` SET `status` = 3 WHERE `order_recurring_id` = '" . (int)$profile['order_recurring_id'] . "' LIMIT 1");
					}
				}
				...
				if ($this->request->post['txn_type'] == 'recurring_payment_suspended_due_to_max_failed_payment') {
					...
					if ($profile != false) {
						$this->db->query("INSERT INTO `order_recurring_transaction` SET `order_recurring_id` = '" . (int)$profile['order_recurring_id'] . "', `created` = NOW(), `type` = '7'");
						$this->db->query("UPDATE `order_recurring` SET `status` = 3 WHERE `order_recurring_id` = '" . (int)$profile['order_recurring_id'] . "' LIMIT 1");
					}
				}
				...
				if ($this->request->post['txn_type'] == 'recurring_payment_failed') {
					...
					if ($profile != false) {
						$this->db->query("INSERT INTO `order_recurring_transaction` SET `order_recurring_id` = '" . (int)$profile['order_recurring_id'] . "', `created` = NOW(), `type` = '4'");
					}
				}
				...
				if ($this->request->post['txn_type'] == 'recurring_payment_outstanding_payment_failed') {
					...
					if ($profile != false) {
						$this->db->query("INSERT INTO `order_recurring_transaction` SET `order_recurring_id` = '" . (int)$profile['order_recurring_id'] . "', `created` = NOW(), `type` = '8'");
					}
				}
				...
				if ($this->request->post['txn_type'] == 'recurring_payment_outstanding_payment') {
					...
					if ($profile != false) {
						$this->db->query("INSERT INTO `order_recurring_transaction` SET `order_recurring_id` = '" . (int)$profile['order_recurring_id'] . "', `created` = NOW(), `amount` = '" . (float)$this->request->post['amount'] . "', `type` = '2'");
				...
				if ($this->request->post['txn_type'] == 'recurring_payment_profile_created') {
					...
					if ($profile != false) {
						$this->db->query("INSERT INTO `order_recurring_transaction` SET `order_recurring_id` = '" . (int)$profile['order_recurring_id'] . "', `created` = NOW(), `type` = '0'");
				...
				if ($this->request->post['txn_type'] == 'recurring_payment_profile_cancel') {
					...
					if ($profile != false && $profile['status'] != 3) {
						$this->db->query("INSERT INTO `order_recurring_transaction` SET `order_recurring_id` = '" . (int)$profile['order_recurring_id'] . "', `created` = NOW(), `type` = '5'");
						$this->db->query("UPDATE `order_recurring` SET `status` = 4 WHERE `order_recurring_id` = '" . (int)$profile['order_recurring_id'] . "' LIMIT 1");
					}
				}
				...
				if ($this->request->post['txn_type'] == 'recurring_payment_skipped') {
					...
					if ($profile != false) {
						$this->db->query("INSERT INTO `order_recurring_transaction` SET `order_recurring_id` = '" . (int)$profile['order_recurring_id'] . "', `created` = NOW(), `type` = '3'");
					}
				}
				...
				if ($this->request->post['txn_type'] == 'recurring_payment_expired') {
					...
					if ($profile != false) {
						$this->db->query("INSERT INTO `order_recurring_transaction` SET `order_recurring_id` = '" . (int)$profile['order_recurring_id'] . "', `created` = NOW(), `type` = '9'");
						$this->db->query("UPDATE `order_recurring` SET `status` = 5 WHERE `order_recurring_id` = '" . (int)$profile['order_recurring_id'] . "' LIMIT 1");
					}
				}
			}
