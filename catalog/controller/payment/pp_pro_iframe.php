	...
	class ControllerPaymentPPProIframe extends Controller {
		protected function index() {
			...
			if ($this->config->get('pp_pro_iframe_checkout_method') == 'redirect') {
				...
				if ($this->config->get('pp_pro_iframe_test')) {
					$this->data['url'] = 'https://sandbox.paypal.com/cgi-bin/webscr';
					} else {
					$this->data['url'] = 'https://paypal.com/cgi-bin/webscr';
				}
		...
		public function create() {
			...
			if ($hosted_button_id) {
				...
				if ($this->config->get('pp_pro_iframe_test')) {
					$this->data['url'] = 'https://www.sandbox.paypal.com/cgi-bin/webscr';
					} else {
					$this->data['url'] = 'https://www.paypal.com/cgi-bin/webscr';					
				}
		...
		private function constructButtonData($order_info) {
			...
			$s_data['L_BUTTONVAR0'] = 'subtotal=' . $this->currency->format($order_info['total_national'], $order_info['currency_code'], 1, false);
			...
			$this->model_payment_pp_pro_iframe->log(print_r(serialize($s_data), 1));
