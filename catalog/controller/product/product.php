    ...
    class ControllerProductProduct extends Controller
    {
		...
		public function getEcommerceInfo(){
			...
			if ($product_info) {
				...
				$json['price'] 		= ($product_info['special']) ? $this->currency->format($product_info['special'], '',
				'', false) : $this->currency->format($product_info['price'], '', '', false);
        ...
        {				
            ...
            if ($product_info['is_option_for_product_id']) {
				...
				if (isset($this->request->get['manufacturer_id']) || ($product_info && $product_info['manufacturer_id'] > 0)) {
					...
					if (isset($this->request->get['path'])) {
						if (isset($manufacturer_info['manufacturer_id'])) {
							$mhref = $this->url->link('product/category',
						...
						if (isset($manufacturer_info['manufacturer_id'])) {
							$mhref = $this->url->link('product/manufacturer/info',
            ...
            if ($product_info) {
				...
				if (!$just_price){	
					...
					*/
					...
					if ($product_info['stock_product_id']) {
						...
						'product_id=' . $product_info['stock_product_id']), 'canonical');
						} else {
						$this->document->addLink($this->url->link('product/product',
						'product_id=' . $this->request->get['product_id']), 'canonical');
					}
					...
					if ($product_info['is_markdown']){
						...
						if ($product_info['markdown_product_id']){
							...
							if ((float)$markdown_product['special']) {
								$special = $this->currency->format($this->tax->calculate($markdown_product['special'],
								$markdown_product['tax_class_id'], $this->config->get('config_tax')));
								} else {
								$special = false;
							}
							...
							'description'  			=> utf8_substr(strip_tags(html_entity_decode($markdown_product['description'],
				...
				'certificate') === false) ? $product_info['location'] : false;
				...
				*/
				...
				if (!$just_price){
					...
					if ($product_info['image']) {
						...
						$this->data['second_popup'] = $this->model_tool_image->resize($this->config->get('config_noimage'),
						300, 300);
					}
                ...
                if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {
                    ...
                    if ($product_info['price_recommend']) {
                        $this->data['price_recommend'] = $this->currency->format($this->tax->calculate($product_info['price_recommend'],
                ...
                if ((float)$product_info['special']) {
                    $this->data['special'] = $this->currency->format($this->tax->calculate($product_info['special'],
					$product_info['tax_class_id'], $this->config->get('config_tax')));
                    $this->data['saving'] = round(((preg_replace("([^0-9])", "",
					$this->data['price']) - preg_replace("([^0-9])", "",
					$this->data['special'])) / (preg_replace("([^0-9])", "",
					$this->data['price']) + 0.01)) * 100, 0);
					} else {
                ...
                foreach ($results as $result) {
					...
					? $this->model_tool_image->resize($result['special_attribute_value'],
					$pds_detail_thumbnail_width, $pds_detail_thumbnail_height)
					: $this->model_tool_image->resize($result['image'], $pds_detail_thumbnail_width,
					$pds_detail_thumbnail_height);
					
                    $product_main_image = ($result['image'] != '' && strtolower($result['image']) != 'no_image.jpg')
					? $this->model_tool_image->resize($result['image'], $pds_preview_width,
					$pds_preview_height) //user default main image
					: $this->model_tool_image->resize($result['special_attribute_value'], $pds_preview_width,
					$pds_preview_height); // use series image
					
                    $this->data['pds'][] = array(
					'product_id'         => $result['product_id'],
					'product_link'       => $this->url->link('product/product',
                ...
                if (!isset($this->data['display_add_to_cart'])) {
                    $is_master = $this->model_catalog_product_master->isMaster($this->request->get['product_id'],
					'2'); //2 is Image
                    $pds_allow_buying_series = $this->getData('pds_allow_buying_series', 0);
                        ...
                        foreach ($option['option_value'] as $option_value) {
                            if (!$option_value['subtract'] || ($option_value['quantity'] > 0)) {
                                ...
                                if ((($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) && (float)$option_value['price']) {
                                    $o_price = $this->currency->format($this->tax->calculate($option_value['price'],
									$product_info['tax_class_id'], $this->config->get('config_tax')));
									} else {
                                    $o_price = false;
								}
                                ...
                                if ((($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) && (float)$real_product['price']) {
                                    $o_price = $this->currency->format($this->tax->calculate($real_product['price'],
									$product_info['tax_class_id'], $this->config->get('config_tax')));
									} else {
                                    $o_price = false;
								}
                                ...
                                if ((($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) && (float)$real_product['special']) {
                                    $o_special = $this->currency->format($this->tax->calculate($real_product['special'],
									$product_info['tax_class_id'], $this->config->get('config_tax')));
									} else {
                                    $o_special = false;
								}
								...
								'pbo_image'               => $this->model_tool_image->resize($option_value['image'],
								...
								'real_product_type'       => $this->model_catalog_product->getProductAttributeValueById($option_value['this_is_product_id'],
								1),
                                    ...
                                    if (is_array($r_option['option_value'])) {
                                        foreach ($r_option['option_value'] as &$r_option_value) {
                                            if ($r_option_value['product_option_value_id'] == $option_value['product_option_value_id']) {
                                                $r_option_value['pbo_image'] = $this->model_tool_image->resize($option_value['image'],
												$this->getData('pbo_image_block_width', 50),
												$this->getData('pbo_image_block_height', 50));
											}
										}
                ...
                if ($this->data['options'] && count($this->data['options']) == 1) {
                    ...
                    if (isset($this->data['options'][0]['option_value'][$key]['real_product']) && $product_to_overload = $this->data['options'][0]['option_value'][$key]['real_product']) {
                        ...
                        if ((($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) && (float)$product_to_overload['price']) {
                            $product_to_overload['price'] = $this->currency->format($this->tax->calculate($product_to_overload['price'],
							$product_info['tax_class_id'], $this->config->get('config_tax')));
							} else {
                        ...
                        if ((($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) && (float)$product_to_overload['special']) {
                            $product_to_overload['special'] = $this->currency->format($this->tax->calculate($product_to_overload['special'],
							$product_info['tax_class_id'], $this->config->get('config_tax')));
							} else {
						...
						foreach ($product_product_option['product_option'] as $product_option_value) {
							...
							if ($product_option_info['current_in_stock']  && !empty($product_option_value['stock_status_id']) && $product_option_value['stock_status_id'] != $this->config->get('config_not_in_stock_status_id')){
								...
								if (isset($product_option_value['image']) && file_exists(DIR_IMAGE . $product_option_value['image'])) {
									...
									$popup = $this->model_tool_image->resize($product_option_value['image'],
									...
									$popup = $this->model_tool_image->resize($this->config->get('config_noimage'),
									$this->config->get('config_image_popup_width'),
									$this->config->get('config_image_popup_height'));
								}
								...
								if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {
									$price = $this->currency->format($this->tax->calculate($product_option_info['price'],
									$product_info['tax_class_id'], $this->config->get('config_tax')));
									} else {
									$price = false;
								}
								...
								if ((float)$product_option_info['special']) {
									$special = $this->currency->format($this->tax->calculate($product_option_info['special'],
									$product_info['tax_class_id'], $this->config->get('config_tax')));
									} else {
									$special = false;
								}
				...
				if (!$just_price){
					...
					if (isset($autolinks) && (strpos($this->data['description'],
					'iframe') == false) && (strpos($this->data['description'], 'object') == false)
					) {
						$xdescription = mb_convert_encoding(html_entity_decode($this->data['description'], ENT_COMPAT,
						"UTF-8"), 'HTML-ENTITIES', "UTF-8");
						...
						foreach ($autolinks as $autolink) {
							...
							$xlink = mb_convert_encoding(html_entity_decode($autolink['link'], ENT_COMPAT, "UTF-8"),
							'HTML-ENTITIES', "UTF-8");
					...
					if ($product_info['color_group']) {
						$_cgrouped_products = $this->model_catalog_product->getColorGroupedProducts($this->request->get['product_id'],
						...
						if ($_cgrouped_products) {
							foreach ($_cgrouped_products as $_cgproduct) {
								...
								'_href' => $this->url->link('product/product',
					...
					if ($this->config->get('config_google_remarketing_type') == 'ecomm') {
						...
						'ecomm_totalvalue' => ($product_info['special']) ? $this->currency->format($product_info['special'],
						'', '', false) : $this->currency->format($product_info['price'], '', '', false)
						);
						...
						'dynx_totalvalue' => ($product_info['special']) ? $this->currency->format($product_info['special'],
						'', '', false) : $this->currency->format($product_info['price'], '', '', false)
						);
					}
					...
					'price'      => ($product_info['special']) ? $this->currency->format($product_info['special'], '',
					'', false) : $this->currency->format($product_info['price'], '', '', false),
		...
		public function onereview(){
			...
			$results = $this->model_catalog_review->getBestFuckenReviewForProductID($this->request->get['product_id']);
			...
			if ($this->data['reviews'] && !empty($this->data['reviews'][0])){
				$this->data['review'] = $this->data['reviews'][0];
				} else {
				$this->data['review'] = false;
			}
			
			$this->template = $this->config->get('config_template') . '/template/product/onereview.tpl';
			...
			if ($product_info && $profile_info) {
				...
				if (!$json) {
					...
					if ($profile_info['trial_status'] == 1) {
						$price = $this->currency->format($this->tax->calculate($profile_info['trial_price'] * $quantity,
						$product_info['tax_class_id'], $this->config->get('config_tax')));
						$trial_text = sprintf($this->language->get('text_trial_description'), $price,
						$profile_info['trial_cycle'], $frequencies[$profile_info['trial_frequency']],
						$profile_info['trial_duration']) . ' ';
						} else {
						$trial_text = '';
					}
					
					$price = $this->currency->format($this->tax->calculate($profile_info['price'] * $quantity,
					$product_info['tax_class_id'], $this->config->get('config_tax')));
					
					if ($profile_info['duration']) {
						$text = $trial_text . sprintf($this->language->get('text_payment_description'), $price,
						$profile_info['cycle'], $frequencies[$profile_info['frequency']],
						$profile_info['duration']);
						} else {
						$text = $trial_text . sprintf($this->language->get('text_payment_until_canceled_description'),
						$price, $profile_info['cycle'], $frequencies[$profile_info['frequency']],
						$profile_info['duration']);
					}
				...
				*/
				...
				if (isset($this->request->files) && isset($this->request->files['add-review-image']) && is_array($this->request->files['add-review-image']) && mb_strlen($this->request->files['add-review-image']['name']) > 3) {
					...
					if (!$file_error) {
						...
						if (move_uploaded_file($this->request->files['add-review-image']['tmp_name'],
		...
		public function productload($setId = false)
		{
			...
			if ($setId) {
				...
				if ($results) {
					foreach ($results as $product) {
						...
						if ($product['image']) {
							...
							$image = $this->model_tool_image->resize($this->config->get('config_noimage'), $image_width,
							$image_height);
						}
						...
						foreach ($this->model_catalog_product->getProductOptions($product['product_id']) as $option) {
							...
							if ($option['type'] == 'select' || $option['type'] == 'radio' || $option['type'] == 'checkbox' || $option['type'] == 'image') {
								...
								foreach ($option['option_value'] as $option_value) {
									...
									//if (!$option_value['subtract'] || ($option_value['quantity'] > 0)) {
									if ((($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) && (float)$option_value['price']) {
										$option_price = $this->currency->format($this->tax->calculate($option_value['price'],
										$set_info['tax_class_id'], $this->config->get('config_tax')));
										} else {
										$option_price = false;
									}
									...
									if ($option['type'] == 'checkbox' && isset($product['options'][$option['product_option_id']]) && in_array($option_value['product_option_value_id'],
									...
									'name'                    => $option_value['name'],
									'image'                   => $this->model_tool_image->resize($option_value['image'],
									50, 50),
									'price'                   => $option_price,
									'price_prefix'            => $option_value['price_prefix'],
						...
						if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {
							$price_product = $this->currency->format($this->tax->calculate($real_price,
							$set_info['tax_class_id'], $this->config->get('config_tax')));
							} else {
							$price_product = false;
						}
						if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {
							$price_in_set = $this->currency->format($this->tax->calculate($product['price_in_set'],
							$set_info['tax_class_id'], $this->config->get('config_tax')));
							} else {
							$price_in_set = false;
						}
				...
				$set_save = $this->currency->format($this->tax->calculate($set_pricing['save'],
				$set_info['tax_class_id'], $this->config->get('config_tax')));
				
				if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {
					$set_price = $this->currency->format($this->tax->calculate($set_pricing['special'],
					$set_info['tax_class_id'], $this->config->get('config_tax')));
					} else {
					$set_price = false;
				}
				...
				foreach ($post_option as $option_id => $option_value_id) {
					...
					if ($query->num_rows) {
						...
						if ($query->row['this_is_product_id'] && $real_product = $this->model_catalog_product->getProduct($query->row['this_is_product_id'])) {
							...
							if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {
								$price = $this->currency->format($this->tax->calculate($real_product['price'],
								$real_product['tax_class_id'], $this->config->get('config_tax')));
								} else {
								$price = false;
							}
							
							if ((float)$real_product['special']) {
								$special = $this->currency->format($this->tax->calculate($real_product['special'],
								$real_product['tax_class_id'], $this->config->get('config_tax')));
								} else {
								$special = false;
							}
						...
						if ($query->row['ob_image']) {
							...
							$json['ob_thumb'] = $this->model_tool_image->resize($query->row['ob_image'],
							$this->config->get('config_image_thumb_width'),
							$this->config->get('config_image_thumb_height'));
							$json['ob_popup'] = $this->model_tool_image->resize($query->row['ob_image'],
							$this->config->get('config_image_popup_width'),
							$this->config->get('config_image_popup_height'));
							...
							$json['ob_thumb'] = $this->model_tool_image->resize($query->row['image'],
							$this->config->get('config_image_thumb_width'),
							$this->config->get('config_image_thumb_height'));
							$json['ob_popup'] = $this->model_tool_image->resize($query->row['image'],
							$this->config->get('config_image_popup_width'),
							$this->config->get('config_image_popup_height'));
							} else {
							$json['ob_swatch'] = '';
