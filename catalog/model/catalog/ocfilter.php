...
class ModelCatalogOCFilter extends Model {
  ...
  public function getCategorySeoPathByCategoryId($category_id) {
    $query = $this->db->query("SELECT GROUP_CONCAT(ua.`keyword` ORDER BY cp.`level` SEPARATOR '/') AS path FROM " . DB_PREFIX . "category_path cp LEFT JOIN " . DB_PREFIX . "url_alias ua ON (CONCAT('category_id=', cp.path_id) = ua.`query`) WHERE cp.category_id = '" . (int)$category_id . "'");
    ...
    if ($params) {
      ...
      foreach ($params as $option_id => $values) {
        ...
        if (isRange($values[0])) {
          ...
          foreach ($values as $value_id) {
          	$or[] = "oov2p" . (int)$count . ".value_id = '" . (int)$value_id . "'";
          }
        }
