    ...
    class ModelCatalogProduct extends Model
    {
			...
			if (!$product_data) {
				...
				if ($query->num_rows) {
					...
					if ($query->row['stock_product_id']) {
						$main_stock_query = $this->db->ncquery("SELECT quantity_stock, `" . $this->config->get('config_warehouse_identifier') . "` FROM product WHERE product_id = '" . (int)$query->row['stock_product_id'] . "'");
					...
					if ($this->config->get('config_warehouse_only')){
						if (!$query->row[$this->config->get('config_warehouse_identifier')]){
							...
							$overload_stock_status_query 			= $this->db->ncquery("SELECT name FROM stock_status WHERE stock_status_id = '" . (int)$this->config->get('config_overload_stock_status_id') . "' AND language_id = '" . (int)$this->config->get('config_language_id') . "' LIMIT 1");
					...
					if ($simple == 'simple'){
						...
						$product_data = [
							...
							'name'                     	=> $query->row['name'],
							'short_name'                => $query->row['short_name_d'],
		...
		public function getProductAttributesFlat($product_id){
			if (!$product_attribute_flat_data = $this->cache->get($this->registry->createCacheQueryStringData(__METHOD__, [$product_id]))){
				...
				$product_attribute_flat_data = $this->db->ncquery($sql)->rows;
		...
		public function getProductFeaturesFlat($product_id){
			if (!$product_features_flat_data = $this->cache->get($this->registry->createCacheQueryStringData(__METHOD__, [$product_id]))){
				...
				$product_features_flat_data = $this->db->ncquery($sql)->rows;
		...
		public function getProductAttributes($product_id, $filter_data = []){
			if (!$product_attribute_group_data = $this->cache->get($this->registry->createCacheQueryStringData(__METHOD__, [$product_id], $filter_data))){
				...
				$product_attribute_group_query = $this->db->ncquery($sql);
				...
				foreach ($product_attribute_group_query->rows as $product_attribute_group) {
					...
					$product_attribute_query = $this->db->ncquery($sql);
