	...
	class ModelCheckoutCoupon extends Model {
		...
		public function getCoupon($code) {
			...
			if ($coupon_query->num_rows) {
				...
				//не указан один из параметров days_from_send или actiontemplate_id
				...
				//указаны оба параметра и не залогинен
				if ($coupon_query->row['days_from_send'] && $coupon_query->row['actiontemplate_id'] && !$this->customer->getId()){
					$status = false;
				}										
				
				//Если ниразу не был отправлен этому покупателю при явном условии отправки - хуй ему а не купончик
				if ($coupon_query->row['days_from_send'] && $coupon_query->row['actiontemplate_id'] && $this->customer->getId()){
				...
				if ($coupon_product_data || $coupon_category_data || $coupon_manufacturer_data || $coupon_collection_data) {
					...
					//УДАЛЯЕМ ТОВАРЫ ИЗ ПОДАРОЧНОЙ - ДР КАТЕГОРИИ
