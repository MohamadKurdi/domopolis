...
class ModelCheckoutCoupon extends Model {
	...
	public function getCoupon($code) {
		...
		if ($this->couponRandom->checkIfCouponIsRandom($code)){
			return false;
		}

		if ($codeRandom = $this->couponRandom->getCouponRandomParent($code)){
			if($this->couponRandom->getCouponRandomUsage($code) > 0){
				return false;
			} else {
				$code = $codeRandom;
			}
