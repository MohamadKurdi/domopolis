...
class ModelCheckoutVoucherTheme extends Model {
	...
	public function getVoucherThemes($data = array()) {
		if ($data) {
			$sql = "SELECT * FROM voucher_theme vt LEFT JOIN voucher_theme_description vtd ON (vt.voucher_theme_id = vtd.voucher_theme_id) WHERE vtd.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY vtd.name";	
