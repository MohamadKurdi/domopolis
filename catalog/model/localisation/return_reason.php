...
class ModelLocalisationReturnReason extends Model {
	...
	public function getReturnReason($return_reason_id) {
		$query = $this->db->query("SELECT * FROM return_reason WHERE return_reason_id = '" . (int)$return_reason_id . "' AND language_id = '" . (int)$this->config->get('config_language_id') . "'");
	...
	public function getReturnReasons($data = array()) {
		if ($data) {
			...
			if (!$return_reason_data) {
				$query = $this->db->query("SELECT return_reason_id, name FROM return_reason WHERE language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY name");
