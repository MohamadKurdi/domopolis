...
class ModelOpenbayAmazonProduct extends Model {
	...
	public function linkItems(array $data) {
		foreach($data as $amazonSku => $productId) {
			$varRow = $this->db->query("SELECT `var` FROM `amazon_product`
	...
	public function insertError($data) {
		$this->db->query("
			INSERT INTO `amazon_product_error`
	...
	public function deleteErrors($insertionId) {
		 $this->db->query("DELETE FROM `amazon_product_error` WHERE `insertion_id` = '" . $this->db->escape($insertionId) . "'");
	 }
	...
	public function linkProduct($amazon_sku, $product_id, $var = '') {
		$count = $this->db->query("SELECT COUNT(*) as 'count' FROM `amazon_product_link` WHERE `product_id` = '" . (int)$product_id . "' AND `amazon_sku` = '" . $this->db->escape($amazon_sku) . "' AND `var` = '" . $this->db->escape($var) . "' LIMIT 1")->row;
		if($count['count'] == 0) {
			$this->db->query(
				"INSERT INTO `amazon_product_link`
	...
	public function addListingReport($data) {
		$sql = "INSERT INTO amazon_listing_report (marketplace, sku, quantity, asin, price) VALUES ";
	...
	public function removeListingReportLock($marketplace) {
		...
		if ($marketplaces && ($key = array_search($marketplace, $marketplaces)) !== false) {
			...
			$this->db->query("UPDATE setting SET `value` = '" . $this->db->escape(serialize($marketplaces)) . "', serialized = 1 WHERE `key` = 'openbay_amazon_processing_listing_reports'");
		}
	}
}
