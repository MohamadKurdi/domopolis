...
class ModelOpenbayAmazonusProduct extends Model {
	...
	public function linkItems(array $data) {
		foreach($data as $amazonusSku => $productId) {
			$varRow = $this->db->query("SELECT `var` FROM `amazonus_product`
	...
	public function insertError($data) {
		$this->db->query("
			INSERT INTO `amazonus_product_error`
	...
	public function deleteErrors($insertionId) {
		 $this->db->query("DELETE FROM `amazonus_product_error` WHERE `insertion_id` = '" . $this->db->escape($insertionId) . "'");
	 }

	public function setSubmitError($insertionId, $message) {
		$skuRows = $this->db->query("SELECT `sku`
			FROM `amazonus_product`
			WHERE `insertion_id` = '" . $this->db->escape($insertionId) . "'
			")->rows;
	...
	public function linkProduct($amazonus_sku, $product_id, $var = '') {
		$count = $this->db->query("SELECT COUNT(*) as 'count' FROM `amazonus_product_link` WHERE `product_id` = '" . (int)$product_id . "' AND `amazonus_sku` = '" . $this->db->escape($amazonus_sku) . "' AND `var` = '" . $this->db->escape($var) . "' LIMIT 1")->row;
		if($count['count'] == 0) {
			$this->db->query(
				"INSERT INTO `amazonus_product_link`
	...
	public function updateSearch($results) {
		foreach ($results as $result) {
			...
			$this->db->query("
				UPDATE amazonus_product_search
	...
	public function addListingReport($data) {
		$sql = "INSERT INTO amazonus_listing_report (sku, quantity, asin, price) VALUES ";
	...
	public function removeListingReportLock($marketplace) {
		...
		$this->db->query("UPDATE setting SET `value` = '0', serialized = 0 WHERE `key` = 'openbay_amazonus_processing_listing_reports'");
	}
}
?>