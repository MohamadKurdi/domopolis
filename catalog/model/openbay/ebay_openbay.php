...
class ModelOpenbayEbayOpenbay extends Model{
					...
					if($product_id != false){
						...
						foreach($skuParts as $part){
							$sql = "SELECT
									...
									`o`.`option_id`,
									`o`.`type`
									FROM `product_option_value` `pv`
									LEFT JOIN `option_value` `ov` ON (`pv`.`option_value_id` = `ov`.`option_value_id`)
									LEFT JOIN `option_value_description` `ovd` ON (`ovd`.`option_value_id` = `ov`.`option_value_id`)
									LEFT JOIN `option_description` `od` ON (`ov`.`option_id` = `od`.`option_id`)
									LEFT JOIN `option` `o` ON (`o`.`option_id` = `od`.`option_id`)
	...
	private function updateOrderWithConfirmedData($order_id, $order, $user) {
		...
		foreach ($data['totals'] as $total) {
			$this->db->query("INSERT INTO `order_total` SET `order_id` = '" . (int)$order_id . "', `code` = '" . $this->db->escape($total['code']) . "', `title` = '" . $this->db->escape($total['title']) . "', `text` = '" . $this->db->escape($total['text']) . "', `value` = '" . (double)$total['value'] . "', `sort_order` = '" . (int)$total['sort_order'] . "'");
		}
	}
	...
	private function handleUserAccount($order) {
		...
		if(!empty($order->address->iso2)){
			$country_qry = $this->db->query("SELECT * FROM `country` WHERE `iso_code_2` = '".$this->db->escape($order->address->iso2)."'");
		}
	...
	private function externalApplicationNotify($order_id) {
		...
		if ($this->openbay->addonLoad('mosaic') && !$this->mosaic->isOrderAdded($order_id)) {
			$this->db->query("UPDATE `order` SET `shipping_code` = 'ebay.STD' WHERE `order_id` = '".(int)$order_id."' LIMIT 1");
