...
class ModelOpenbayEbayOrder extends Model{
	public function addOrderLine($data, $order_id, $created) {
		...
		if($orderLine === false) {
			...
			if($order_id != $orderLine['order_id']) {
				...
				$this->db->query("UPDATE `ebay_transaction` SET `order_id` = '".(int)$order_id."', `modified` = now() WHERE `txn_id` = '".$this->db->escape((string)$data['txn_id'])."' AND `item_id` = '".$this->db->escape((string)$data['item_id'])."' LIMIT 1");
			...
			if($orderLine['smp_id'] != $data['smp_id']) {
				...
				$this->db->query("UPDATE `ebay_transaction` SET `smp_id` = '".$this->db->escape($data['smp_id'])."', `modified` = now() WHERE `txn_id` = '".$this->db->escape((string)$data['txn_id'])."' AND `item_id` = '".$this->db->escape((string)$data['item_id'])."' LIMIT 1");
			}
			...
			if($orderLine['containing_order_id'] != $data['containing_order_id']) {
				...
				$this->db->query("UPDATE `ebay_transaction` SET `containing_order_id` = '".$this->db->escape($data['containing_order_id'])."', `modified` = now() WHERE `txn_id` = '".$this->db->escape((string)$data['txn_id'])."' AND `item_id` = '".$this->db->escape((string)$data['item_id'])."' LIMIT 1");
			}
		}
	...
	public function getOrderLine($txn_id, $item_id) {
		...
		$res = $this->db->query("SELECT * FROM `ebay_transaction` WHERE `txn_id` = '".$this->db->escape($txn_id)."' AND `item_id` = '".$this->db->escape($item_id)."' LIMIT 1");
	...
	public function getOrderLines($order_id) {
		...
		$result = $this->db->query("SELECT * FROM `ebay_transaction` WHERE `order_id` = '".(int)$order_id."'");
	...
	private function removeOrderLine($txn_id, $item_id, $line) {
		...
		$this->db->query("DELETE FROM `ebay_transaction` WHERE `txn_id` = '".$this->db->escape($txn_id)."' AND `item_id` = '".$this->db->escape($item_id)."' LIMIT 1");
	...
	public function find($smp_id) {
		 ...
		 */
		if($order_id == 0) {
			$query = $this->db->query("SELECT `order_id` FROM `order_history` WHERE `comment` = '[eBay Import:" . $this->db->escape($smp_id) . "]' LIMIT 1");
	...
	public function getHistory($order_id) {
		...
		$query = $this->db->query("SELECT `order_status_id` FROM `order_history` WHERE `order_id` = '".(int)$order_id."'");
		...
		if ($order_info && !$order_info['order_status_id']) {
			...
			if(isset($order_info['email']) && !empty($order_info['email'])){
				...
				foreach ($order_product_query->rows as $product) {
					...
					$order_option_query = $this->db->query("SELECT * FROM order_option WHERE order_id = '" . (int)$order_id . "' AND order_product_id = '" . $this->db->escape($product['order_product_id']) . "'");
	...
	private function modifyStock($product_id, $qty, $symbol = '-', $sku = '') {
		...
		if ($this->openbay->addonLoad('openstock') && !empty($sku)) {
			$this->db->query("UPDATE `product_option_relation` SET `stock` = (`stock` ".(string)$symbol." " . (int)$qty . ") WHERE `var` = '" . (string)$sku . "' AND `product_id` = '".(int)$product_id."' AND `subtract` = '1'");
			...
			$this->db->query("UPDATE `product` SET `quantity` = (`quantity` ".(string)$symbol." " . (int)$qty . ") WHERE `product_id` = '" . (int)$product_id . "' AND `subtract` = '1'");
	...
	public function getCountryAddressFormat($iso2) {
		...
		$query = $this->db->query("SELECT * FROM `country` WHERE `iso_code_2` = '".$this->db->escape($iso2)."' LIMIT 1");
	...
	public function orderLinkCreate($order_id, $smp_id) {
		...
		$this->db->query("INSERT INTO `ebay_order` SET `order_id` = '" . (int)$order_id . "', `smp_id` = '".(int)$smp_id."', `parent_ebay_order_id` = 0");

		return $this->db->getLastId();
	}

	public function orderLinkGet($smp_id) {
		$query = $this->db->query("SELECT `order_id` FROM `ebay_order` WHERE `smp_id` = '".(int)$smp_id."' LIMIT 1");
	...
	public function delete($order_id) {
		$this->db->query("DELETE FROM `order` WHERE `order_id` = '".(int)$order_id."' LIMIT 1");
	...
	public function lockAdd($smp_id) {
		...
		$this->db->query("INSERT INTO`ebay_order_lock` SET `smp_id` = '".(int)$smp_id."'");
	}

	public function lockDelete($smp_id) {
		$this->openbay->ebay->log('lockDelete() - Delete lock, smp_id: '.$smp_id);
		$this->db->query("DELETE FROM `ebay_order_lock` WHERE `smp_id` = '".(int)$smp_id."'");
	}

	public function lockExists($smp_id) {
		$this->openbay->ebay->log('lockExists() - Check lock, smp_id: '.(int)$smp_id);
		$query = $this->db->query("SELECT * FROM `ebay_order_lock` WHERE `smp_id` = '".(int)$smp_id."' LIMIT 1");
