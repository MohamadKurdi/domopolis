...
class ModelOpenbayEbayProduct extends Model {
	...
	public function importItems($data) {
		...
		$res = $this->db->query("SHOW TABLES LIKE 'ebay_image_import'");
		if($res->num_rows == 0) {
			$this->db->query("
				CREATE TABLE IF NOT EXISTS `ebay_image_import` (
		...
		if ($options['cat'] == 1) {
			...
			foreach($categories as $key1=>$cat1) {
				foreach($cat1 as $key2=>$cat2) {
					...
					$qry = $this->db->query("SELECT * FROM `category`, `category_description` WHERE `category`.`parent_id` = '0' AND `category_description`.`name` = '".$this->db->escape($key2)."' LIMIT 1");

					if($qry->num_rows != 0) {
						$id1 = $qry->row['category_id'];
					}else{
						$this->db->query("INSERT INTO `category` SET `parent_id` = '0', `status` = '1', `top` = '1'");
						$id1 = $this->db->getLastId();
						$this->db->query("INSERT INTO `category_description` SET `name` = '".$this->db->escape($key2)."', `language_id` = '".(int)$this->config->get('config_language_id')."', `category_id` = '".$this->db->escape($id1)."'");
						$this->db->query("INSERT INTO `category_to_store` SET `category_id` = '".$this->db->escape($id1)."', `store_id` = '0'");
					}

					if(!empty($cat2)) {
						foreach($cat2 as $key3=>$cat3) {
							$qry = $this->db->query("SELECT * FROM `category`, `category_description` WHERE `category`.`parent_id` = '".$this->db->escape($id1)."' AND `category_description`.`name` = '".$this->db->escape($key3)."' LIMIT 1");

							if($qry->num_rows != 0) {
								$id2 = $qry->row['category_id'];
							}else{
								$this->db->query("INSERT INTO `category` SET `parent_id` = '".$this->db->escape($id1)."', `status` = '1', `top` = '1'");
								$id2 = $this->db->getLastId();
								$this->db->query("INSERT INTO `category_description` SET `name` = '".$this->db->escape($key3)."', `language_id` = '".(int)$this->config->get('config_language_id')."', `category_id` = '".$this->db->escape($id2)."'");
								$this->db->query("INSERT INTO `category_to_store` SET `category_id` = '".$this->db->escape($id2)."', `store_id` = '0'");
							}

							if(!empty($cat3)) {
								foreach($cat3 as $key4=>$cat4) {
									$qry = $this->db->query("SELECT * FROM `category`, `category_description` WHERE `category`.`parent_id` = '".$this->db->escape($id3)."' AND `category_description`.`name` = '".$this->db->escape($key4)."' LIMIT 1");

									if($qry->num_rows != 0) {
										$id3 = $qry->row['category_id'];
									}else{
										$this->db->query("INSERT INTO `category` SET `parent_id` = '".$this->db->escape($id2)."', `status` = '1', `top` = '1'");
										$id3 = $this->db->getLastId();
										$this->db->query("INSERT INTO `category_description` SET `name` = '".$this->db->escape($key4)."', `language_id` = '".(int)$this->config->get('config_language_id')."', `category_id` = '".$this->db->escape($id3)."'");
										$this->db->query("INSERT INTO `category_to_store` SET `category_id` = '".$this->db->escape($id3)."', `store_id` = '0'");
									}

									if(!empty($cat4)) {
										foreach($cat4 as $key5=>$cat5) {
											$qry = $this->db->query("SELECT * FROM `category`, `category_description` WHERE `category`.`parent_id` = '".$this->db->escape($id4)."' AND `category_description`.`name` = '".$this->db->escape($key5)."' LIMIT 1");

											if($qry->num_rows != 0) {
												$id4 = $qry->row['category_id'];
											}else{
												$this->db->query("INSERT INTO `category` SET `parent_id` = '".$this->db->escape($id3)."', `status` = '1', `top` = '1'");
												$id4 = $this->db->getLastId();
												$this->db->query("INSERT INTO `category_description` SET `name` = '".$this->db->escape($key5)."', `language_id` = '".(int)$this->config->get('config_language_id')."', `category_id` = '".$this->db->escape($id4)."'");
												$this->db->query("INSERT INTO `category_to_store` SET `category_id` = '".$this->db->escape($id4)."', `store_id` = '0'");
											}
	...
	private function getOption($name) {
		$qry = $this->db->query("
			...
			FROM
				`option` `o`
				LEFT JOIN `option_description` `od` ON (`od`.`option_id` = `o`.`option_id`)
			WHERE
	...
	private function createOption($name) {
		$this->db->query("INSERT INTO `option` SET `type` = 'select', `sort_order` = IFNULL((select `sort` FROM (SELECT (MAX(`sort_order`)+1) AS `sort` FROM `option`) AS `i`),0)");
		$option_id = $this->db->getLastId();

		$qry_sort = $this->db->query("SELECT * FROM `option` WHERE `option_id` = '".$option_id."' LIMIT 1");

		$this->db->query("INSERT INTO `option_description` SET `language_id` = '".(int)$this->config->get('config_language_id')."', `name` = '".$this->db->escape($name)."', `option_id` = '".$this->db->escape($option_id)."'");
	...
	private function getOptionValue($name, $option_id) {
		$qry = $this->db->query("
			...
			FROM
				`option_value` ov
				LEFT JOIN `option_value_description` `ovd` ON (`ovd`.`option_value_id` = `ov`.`option_value_id`)
			WHERE
	...
	private function createOptionValue($name, $option_id) {
		$this->db->query("INSERT INTO `option_value` SET `option_id` = '".$this->db->escape($option_id)."', `sort_order` = IFNULL((select `sort` FROM (SELECT (MAX(`sort_order`)+1) AS `sort` FROM `option_value`) AS `i`),0)");
		$id = $this->db->getLastId();
		$this->db->query("INSERT INTO `option_value_description` SET `language_id` = '".(int)$this->config->get('config_language_id')."', `name` = '".$this->db->escape($name)."', `option_id` = '".$this->db->escape($option_id)."', `option_value_id` = '".$id."'");
		return array('id' => (int)$id);
	}

	private function getProductOption($product_id, $option_id) {
		$qry = $this->db->query("SELECT * FROM  product_option WHERE product_id = '" . (int)$product_id . "' AND option_id = '" . (int)$option_id . "' LIMIT 1");

		if($qry->num_rows) {
			return $qry->row['product_option_id'];
		}else{
			$this->db->query("INSERT INTO product_option SET product_id = '" . (int)$product_id . "', option_id = '" . (int)$option_id . "', required = '1'");
	...
	private function createProductVariant($data) {
		$this->db->query("
			INSERT INTO `product_option_relation`
			SET
	...
	private function attributeGroupExists($name) {
		...
		$qry = $this->db->query("SELECT * FROM  `attribute_group_description` WHERE `name` = '".$this->db->escape(htmlspecialchars($name, ENT_COMPAT))."' AND `language_id` = '".(int)$this->config->get('config_language_id')."' LIMIT 1");
		...
		if($qry->num_rows) {
			...
			$qry2 = $this->db->query("SELECT `sort_order` FROM  `attribute_group` ORDER BY `sort_order` DESC LIMIT 1");
			...
			$this->db->query("INSERT INTO `attribute_group` SET `sort_order` = '" . (int)$sort . "'");

			$id = $this->db->getLastId();

			$this->db->query("INSERT INTO `attribute_group_description` SET `attribute_group_id` = '" . (int)$id . "', `language_id` = '".(int)$this->config->get('config_language_id')."', `name` = '".$this->db->escape(htmlspecialchars($name, ENT_COMPAT))."'");
	...
	private function attributeExists($groupId, $name) {
		...
		if($qry->num_rows) {
			...
			$qry2 = $this->db->query("SELECT `sort_order` FROM  `attribute` ORDER BY `sort_order` DESC LIMIT 1");
			...
			$this->db->query("INSERT INTO `attribute` SET `sort_order` = '" . (int)$sort . "', `attribute_group_id` = '" . (int)$groupId . "'");

			$id = $this->db->getLastId();

			$this->db->query("INSERT INTO `attribute_description` SET `attribute_id` = '" . (int)$id . "', `language_id` = '".(int)$this->config->get('config_language_id')."', `name` = '".$this->db->escape(htmlspecialchars($name, ENT_COMPAT))."'");
	...
	private function attributeAdd($product_id, $attrId, $name) {
		...
		$sql = $this->db->query("SELECT * FROM `product_attribute` WHERE `product_id` = '".(int)$product_id."' AND `attribute_id` = '".(int)$attrId."' AND `language_id` = '".(int)$this->config->get('config_language_id')."'");

		if($sql->num_rows == 0) {
			$this->db->query("INSERT INTO `product_attribute` SET `product_id` = '".(int)$product_id."', `attribute_id` = '".(int)$attrId."', `text` = '".$this->db->escape(htmlspecialchars($name, ENT_COMPAT))."', `language_id` = '".(int)$this->config->get('config_language_id')."'");
		}
	}
	...
	private function manufacturerExists($name) {
		...
		$qry = $this->db->query("SELECT * FROM  `manufacturer` WHERE LCASE(`name`) = '".$this->db->escape(htmlspecialchars($name, ENT_COMPAT))."' LIMIT 1");
		...
		if($qry->num_rows) {
			...
			$qry2 = $this->db->query("SELECT `sort_order` FROM  `manufacturer` ORDER BY `sort_order` DESC LIMIT 1");
			...
			$this->db->query("INSERT INTO `manufacturer` SET `sort_order` = '" . (int)$sort . "', `name` = '".$this->db->escape(htmlspecialchars($name, ENT_COMPAT))."'");

			$id = $this->db->getLastId();

			$this->db->query("INSERT INTO `manufacturer_to_store` SET `manufacturer_id` = '" . (int)$id . "', `store_id` = '0'");
	...
	private function weightClassExists($name) {
		...
		$qry = $this->db->query("SELECT `weight_class_id` FROM `weight_class_description` WHERE LCASE(`title`) = '".$this->db->escape(strtolower($name))."' LIMIT 1");
		...
		if($qry->num_rows) {
			...
			$this->db->query("INSERT INTO `weight_class` SET `value` = '1'");

			$id = $this->db->getLastId();

			$this->db->query("INSERT INTO `weight_class_description` SET `language_id` = '".(int)$this->config->get('config_language_id')."', `weight_class_id` = '" . (int)$id . "', `title` = '".$this->db->escape(htmlspecialchars($name, ENT_COMPAT))."', `unit` = '".$this->db->escape(htmlspecialchars($name, ENT_COMPAT))."'");
	...
	private function lengthClassExists($name) {
		...
		$qry = $this->db->query("SELECT `length_class_id` FROM `length_class_description` WHERE LCASE(`title`) = '".$this->db->escape(strtolower($name))."' LIMIT 1");
		...
		if($qry->num_rows) {
			...
			$this->db->query("INSERT INTO `length_class` SET `value` = '1'");

			$id = $this->db->getLastId();

			$this->db->query("INSERT INTO `length_class_description` SET `language_id` = '".(int)$this->config->get('config_language_id')."', `length_class_id` = '" . (int)$id . "', `title` = '".$this->db->escape(htmlspecialchars($name, ENT_COMPAT))."', `unit` = '".$this->db->escape(htmlspecialchars($name, ENT_COMPAT))."'");
	...
	private function addImage($orig, $new, $name, $product_id, $imgCount) {
		...
		$this->db->query("
			INSERT INTO `ebay_image_import` SET
