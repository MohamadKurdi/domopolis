	...
	private $api;
  	...
  	public function getMethod($address, $total) {
		...
		if ($this->config->get('cod_cdek_geo_zone_id')) {
			...
			$query = $this->db->query("SELECT * FROM zone_to_geo_zone WHERE geo_zone_id IN(" . implode(',', $cod_cdek_geo_zone_id) . ") AND country_id = '" . (int)$address['country_id'] . "' AND (zone_id = '" . (int)$address['zone_id'] . "' OR zone_id = '0')");
		...
		if(isset($codeParts[1])) {
			...
			if(isset($tariffParts[1])) {
				...
				if(in_array($tariffId, array(301, 302))) {
					return $method_data;
				}
			}
