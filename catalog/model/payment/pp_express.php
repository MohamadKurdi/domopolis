<?php
	class ModelPaymentPPExpress extends Model {
		...
		public function getMethod($address, $total, $explicit_show = false) {
			...
			$query = $this->db->query("SELECT * FROM `zone_to_geo_zone` WHERE `geo_zone_id` = '" . (int)$this->config->get('pp_express_geo_zone_id') . "' AND `country_id` = '" . (int)$address['country_id'] . "' AND (`zone_id` = '" . (int)$address['zone_id'] . "' OR `zone_id` = '0')");
			...
			} elseif (false/*(int)$address['country_id'] == 109 || (int)$address['country_id'] == 20*/) {
				$status = true;			
			} else {
				$status = false;		
			}
		...
		public function addOrder($order_data) {
			...
			*/
			...
			$this->db->query("INSERT INTO `paypal_order` SET
		...
		public function addTransaction($transaction_data) {
			...
			*/
			...
			$this->db->query("INSERT INTO `paypal_order_transaction` SET
			...
			foreach ($total_data as $total_row) {
				if (!in_array($total_row['code'], array('total', 'sub_total'))) {
					if ($total_row['value_national'] != 0) {
						...
						$data['L_PAYMENTREQUEST_0_AMT' . $i] = $total_row['value_national'];//$this->currency->format($total_row['value_national'], $this->config->get('config_regional_currency'), 1, false);
						$data['L_PAYMENTREQUEST_0_QTY' . $i] = 1;
		...
		public function getTransactionRow($transaction_id) {
			$qry = $this->db->query("SELECT * FROM `paypal_order_transaction` `pt` LEFT JOIN `paypal_order` `po` ON `pt`.`paypal_order_id` = `po`.`paypal_order_id`  WHERE `pt`.`transaction_id` = '" . $this->db->escape($transaction_id) . "' LIMIT 1");
		...
		public function totalCaptured($paypal_order_id) {
			$qry = $this->db->query("SELECT SUM(`amount`) AS `amount` FROM `paypal_order_transaction` WHERE `paypal_order_id` = '" . (int)$paypal_order_id . "' AND `pending_reason` != 'authorization' AND `pending_reason` != 'paymentreview' AND (`payment_status` = 'Partially-Refunded' OR `payment_status` = 'Completed' OR `payment_status` = 'Pending') AND `transaction_entity` = 'payment'");
			
			return $qry->row['amount'];
		}
		
		public function totalRefundedOrder($paypal_order_id) {
			$qry = $this->db->query("SELECT SUM(`amount`) AS `amount` FROM `paypal_order_transaction` WHERE `paypal_order_id` = '" . (int)$paypal_order_id . "' AND `payment_status` = 'Refunded'");
			
			return $qry->row['amount'];
		}
		
		public function updateOrder($capture_status, $order_id) {
			$this->db->query("UPDATE `paypal_order` SET `modified` = now(), `capture_status` = '".$this->db->escape($capture_status)."' WHERE `order_id` = '".(int)$order_id."'");
		}
