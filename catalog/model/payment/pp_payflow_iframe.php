...
class ModelPaymentPPPayflowIframe extends Model {
	public function getMethod($address, $total) {
		...
		$query = $this->db->query("SELECT * FROM zone_to_geo_zone WHERE geo_zone_id = '" . (int)$this->config->get('pp_payflow_iframe_geo_zone_id') . "' AND country_id = '" . (int)$address['country_id'] . "' AND (zone_id = '" . (int)$address['zone_id'] . "' OR zone_id = '0')");
	...
	public function getOrderId($secure_token_id) {
		$result = $this->db->query("SELECT `order_id` FROM `paypal_payflow_iframe_order` WHERE `secure_token_id` = '" . $this->db->escape($secure_token_id) . "'")->row;
	...
	public function addOrder($order_id, $secure_token_id) {
		$this->db->query("INSERT INTO `paypal_payflow_iframe_order` SET `order_id` = '" . (int)$order_id . "', `secure_token_id` = '" . $this->db->escape($secure_token_id) . "'");
	}

	public function updateOrder($data) {
		$this->db->query("
			UPDATE `paypal_payflow_iframe_order`
	...
	public function addTransaction($data) {
		$this->db->query("
			INSERT INTO paypal_payflow_iframe_order_transaction
