...
class ModelPaymentPPProIframe extends Model {
	public function getMethod($address, $total) {
		...
		$query = $this->db->query("SELECT * FROM zone_to_geo_zone WHERE geo_zone_id = '" . (int)$this->config->get('pp_pro_iframe_geo_zone_id') . "' AND country_id = '" . (int)$address['country_id'] . "' AND (zone_id = '" . (int)$address['zone_id'] . "' OR zone_id = '0')");
		...
		if ($status) {  
			$method_data = array(
				...
				'checkactive' => true,
				'status'      => $status,
				'description' => sprintf($this->language->get('text_description'), 'pp_pro_iframe'),
