...
class ModelShippingCdek extends Model {
	...
	function getQuote($address) 
	{
		...
		$to_data = $this->db->query("SELECT name FROM country WHERE country_id = '" . (int)$address['country_id'] . "' LIMIT 1")->row;
		...
		if (!$empty_zone) {
			$to_data = $this->db->query("SELECT name FROM zone WHERE zone_id = '" . (int)$address['zone_id'] . "' LIMIT 1")->row;
		}

		if(!$address['city'] && $address['zone']) 
		{
			$zoneNameParts = explode(' ', $address['zone']);
			$cdekZoneData = $this->db->query("SELECT cityName FROM `cdek_city` WHERE (`regionName` LIKE '%".$zoneNameParts[0]."%' OR `cityName` LIKE '%".$zoneNameParts[0]."%') AND `center` = '1' LIMIT 1")->row;
			if($cdekZoneData) {
		...
		if ($to_data && $address['city']) {
			...
			if (is_array($cdek_cities) && isset($cdek_cities['geonames']) && count($cdek_cities['geonames'])) {
				...
				{
					$sql = "SELECT regionName FROM  `cdek_city` WHERE `cityName` = '".$address['zone']."'";
				...
				if ($count = count($available)) {
					...
					if ($this->config->get('cdek_log')) 
					{
						...
						$zoneQuerySql_debug = "SELECT * FROM  `cdek_city` WHERE `id` = '".$city_to."'";
					...
					if ($status) {
						...
						if ($status) {
							...
							$query = $this->db->query("SELECT DISTINCT geo_zone_id FROM zone_to_geo_zone WHERE country_id = '" . (int)$address['country_id'] . "' AND (zone_id = '" . (int)$address['zone_id'] . "' OR zone_id = '0')");
								...
								if (!empty($results)) {
									...
									foreach ($results as $shipping_info) {
										...
										if (array_key_exists($shipping_info['tariffId'], $cdek_tariff_list)) {
											...
											if(in_array((int)$shipping_info['tariffId'], $this->getInpostTariffs())) {
