		...
		public function getTotal(&$total_data, &$total, &$taxes) {
			...
			if (isset($this->session->data['coupon'])) {
				...
				if ($coupon_info) {
					...
					//Обрабатываем наше чудное условие
					if (isset($coupon_info['discount_sum']) && $coupon_info['discount_sum'] != ''){
						...
						//Иначе работаем по стандартному методу
						foreach ($this->cart->getProducts() as $product) {										
							$discount = 0;
								...
								if ($coupon_info['type'] == 'F') {
									...
									if ($coupon_info['currency']!=''){
										if ($coupon_info['currency'] == $this->config->get('config_regional_currency')){
											...
											$discount = $this->currency->convert($discount, $coupon_info['currency'], $this->config->get('config_currency'));	
										}
									}
