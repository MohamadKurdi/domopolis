			...
			if(isset($this->session->data['payment_method'])){
				...
				foreach ($discounts as $discount){
					...
					if ($status && $discount['paymentmethod'] == $this->session->data['payment_method']['code'] || (isset($this->session->data['payment_secondary_method']) && $discount['paymentmethod'] == $this->session->data['payment_secondary_method']['code'])) {
						...
						if ((bool)$this->customer->getDiscountCard() && $number_national < 0){
							$disable_discount = true;
						}
						
						//нет дисконтной карты, но пользователь залогинен и у него есть скидка - пиздец блять, дичь какая-то)
						if (!$this->customer->getDiscountCard() && $this->customer->isLogged() && $number_national < 0){
							$this->load->model('total/shoputils_cumulative_discounts');
							
							//проверим пересчитанный процент скидки
							if ($this->customer->getDiscountPercent()){
								$disable_discount = true;
							}
							
							if (!$disable_discount && $this->model_total_shoputils_cumulative_discounts->getLoggedCustomerDiscount()){
								$disable_discount = true;
							}						
						}
						
						//если есть купон и это скидка то не применять
						if (!$disable_discount && $coupon_exists && $number_national < 0){
							$disable_discount = true;			
						}
						
						//ВТОРАЯ ЛОГИКА, ПОХУЙ НА ПРЕДЫДУЩУЮ
						$disable_discount = false;		
