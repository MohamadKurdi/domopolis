...
function simplecheckout_reload(from) {
	...
	console.log('dataLayer.push Checkout ' + from);
    
    //try to determine shipping and payment methods
    let ecommerceOptionValue = '';
    
    if (from == 'shipping_changed'){        
        ecommerceOptionValue = jQuery('#simplecheckout_form').find('input[name=shipping_method_current]').val() + ' (' + jQuery('#simplecheckout_form').find('input#checkout_customer_main_address_1').val() + ') ';
    }
    
    if (from == 'payment_changed'){
        ecommerceOptionValue = jQuery('#simplecheckout_form').find('input[name=payment_method_current]').val();
    }
    ...
    dataLayer.push({
        ...
        'ecommerce': {
            'checkout_option': {
                'actionField': {'step': from, 'option': ecommerceOptionValue}
            }
        }
    });
...
function simple_login_tabs() {
    jQuery('.login_tabs').load('index.php?'+simple_route+'route=checkout/simplecheckout_customer/login');
    return false;
}
...
jQuery(function(){
    ...
    jQuery('#simplecheckout_form input, #simplecheckout_form textarea').on('keydown', function(){
    ...
    jQuery('#simplecheckout_form input[type=radio]:not([reload]),#simplecheckout_form input[type=checkbox]:not([reload]),#simplecheckout_form select:not([reload])').on('change', function(){
        if (jQuery(this).parents('#simplecheckout_payment_form').length) {
            return;
        }
        customer_field_changed();
    ...
    jQuery('#checkout_customer_main_email').on('change',function(){
    ...
    jQuery('#email_confirm').on('change',function(){
    ...
    jQuery('table.cart td.remove img,.mini-cart-info td.remove img,table.s_cart_items a.s_button_remove').on('click', function(){
