                                ...
                                <tr>
                                    ...
                                    <td class="title" valign="middle">
										...
										<?php if (!empty($quote['description']) && (!$display_for_selected || ($display_for_selected && $quote['code'] == $code))) { ?>
											<label for="<?php echo $quote['code']; ?>" class="description_tr"><?php echo $quote['description']; ?></label>
										<?php } ?>
									</td>
                                    <td class="quote">
		...
		function initBrowserNavigationTrigger(){
			if (navigator.geolocation) {
				...
				if ($('#shipping_courier_city_shipping_address').length > 0){
					$('#shipping_courier_city_shipping_address').after('<div class="navigator-geo-init" data-for="shipping_courier_city_shipping_address"><span><i class="fas fa-map-marker-alt"></i> Определить мое местоположение</span></div>');
				}
				
				if ($('#shipping_cdek_street').length > 0){
					$('#shipping_cdek_street').after('<div class="navigator-geo-init" data-for="shipping_cdek_street"><span><i class="fas fa-map-marker-alt"></i> Определить мое местоположение</span></div>');
				}
						...
						jQuery.ajax({
							...
							success: function(json){
								...
								if (json.value != null && json.value){																											
									...
									elem.html('<i class="fas fa-sad-tear"></i> Не получилось определить ваше местоположение').addClass('simplecheckout-error-text');
								}						
							},
							error: function(){
								elem.html('<i class="fas fa-sad-tear"></i> Не получилось определить ваше местоположение').addClass('simplecheckout-error-text');								
