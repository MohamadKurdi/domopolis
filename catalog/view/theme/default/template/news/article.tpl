...
<div class="wrap">
	...
	jQuery(document).ready(function ($) {
			...
			<table class="table_art1">
	    		...
	    		<?php } ?>
				<?php if( is_array($recipe) ) { ?>
					<?php foreach ($recipe as $r){ ?>
						<?php if ($r[0] && $r[1] && $r[2]) {?>
						<tr class="ingredient">
							<td class="numb_art1">
								<?php echo $r[0]. '&nbsp' .$r[1]; ?>
							</td>
							<td class="ingred_art1">
								<?php echo $r[2]; ?>
							</td>
						</tr>
						<?php } ?>
			...
			<div class="coment-article" style="margin-top: 30px;" hidden>
				...
				<?php if ($acom != 0 && !$disqus_status && !$fbcom_status) { ?>
				    <h3><a name="comments" id="comments"></a> <?php echo $text_coms; ?> "<?php echo $heading_title; ?>"</h3>
					<?php if ($comment) { ?>
					    <?php foreach ($comment as $comment) { ?>
						    <div class="content blog-comment">
								<div  class="comment-header">
									<span class="comment-icon"></span>
									<b><?php echo $comment['author']; ?></b> 
									<?php echo $text_posted_on; ?> <?php echo $comment['date_added']; ?>
								</div>
				    ...
				    <div class="content" id="comment-form">
					    <h2 id="com-title">
					    	<span class="blog-write"></span>
					    	<?php echo $writec; ?>
					    </h2>
						<input type="hidden" name="reply_id" value="0" id="reply-id-field"/>
						<form id="coment-article">
							<div class="comment-left">
								<div class="form-group">
									<label><?php echo $entry_name; ?></label>
									<input type="text" name="name" value="<?php echo $this->customer->getFirstName(); ?>" style="" />
								</div>
								<div class="form-group">
							...
							<div class="comment-right">
								<div class="form-group">
								    ...
								    <textarea name="text" cols="40" rows="4" style="width: 100%;height: 113px;"></textarea>
								    <span style="font-size: 11px;"><?php echo $text_note; ?></span>
							    </div>
							</div>						    
					    </form>
					    <div class="buttons">
					      	<a id="button-comment" class="btn"><span><?php echo $text_send; ?></span></a>
					    </div>
				  	</div>
		...
		<!-- right-block -->
		<?php if ($article) { ?>
			<div class="content blog-content-article-right">
						...
						<div class="artblock<?php if (count($article)>1) { ?> artblock-2<?php } ?>">
			                ...
			                <div class="article-meta">
			                    ...
			                    <?php if ($articles['date_added']) { ?>
			                    	<?php if ($articles['author']) { ?><?php echo $text_posted_on; ?><?php } else { ?><?php } ?><span><i class="fas fa-calendar-alt"></i> <?php echo $articles['date_added']; ?></span>
			                    <?php } ?>
			                    <?php if ($articles['du']) { ?>
			                    ...
			                    <?php } ?>
			                    <div class="total-article-views"><i class="fas fa-eye"></i><span> <?php echo $articles['viewed']; ?></span></div>
			                </div>
							
							<?php if ($articles['thumb']) { ?>
			                <a href="<?php echo $articles['href']; ?>">
			                        <div class="image-article-right" style="margin-bottom: 7px;">
			                            <img src="<?php echo $articles['thumb']; ?>" title="<?php echo $articles['name']; ?>" alt="<?php echo $articles['name']; ?>" width="396" height="246" />
			                        </div>
			                    </a>
							<?php } ?>
							...
							<!-- <?php if ($articles['description']) { ?>
								<div class="description-right"><?php echo $articles['description']; ?></div>
							<?php } ?> -->
							...
							<!-- <?php if ($articles['button']) { ?>
								<div class="blog-button-right"><a class="button" href="<?php echo $articles['href']; ?>"><?php echo $button_more; ?></a></div>
							<?php } ?> -->
						</div>
					<?php } ?>
				</div>
...
$('#button-comment').bind('click', function() {
	$.ajax({
		...
		beforeSend: function() {
			...
			$('#com-title').after('<div class="attention"><img src="catalog/view/theme/default/image/loading.gif" alt="" /> <?php echo $text_wait; ?></div>');
		},
		complete: function() {
