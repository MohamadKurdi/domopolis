...
MegaFilter.prototype = {
	...
	*/
	...
	initResponsive: function(){
		console.log('initResponsive called');
	},
	...
	_initEvents: function() {
		...
		self._box.find('.mfilter-button a').on('click', function(e){
			e.preventDefault();
	...
	createUrl: function( url, attribs, force ) {
		var self	= this,
			...
			urlSep	= self._urlSep;
		console.log('createUrl1');
		if( typeof url == 'undefined' ) {
			url = self._url;
			console.log('createUrl 2');

		} else {
			urlSep = self._parseSep( url.split('#')[0] ).urlSep;
			url = self._parseSep( url.split('#')[0] ).url;
			console.log('createUrl 3');
		...
		if( params || filters ) {
			...
			if( params ) {
				url += params;
				console.log('createUrl 4');
			}
			
			if( filters ) {
				if( params ) {
					url += urlSep.n;
					console.log('createUrl 5');
					var isResizeble = false;
					if(!isResizeble) {
						$('html,body').animate({
							scrollTop: $('.catalog__content.product-grid').offset().top
						}, 350);
						isRezeble = true;
					}
				}
				...
				console.log('createUrl 6');
			} else if( force ) {
				var mfp = self.filtersToUrl( self.urlToFilters() );
				console.log('createUrl 7');
				if( mfp ) {
					url += urlSep.n;
					url += 'mfp' + ( urlSep.n == '&' ? '=' : urlSep.n ) + mfp;
					console.log('createUrl 8');
				}
			}
