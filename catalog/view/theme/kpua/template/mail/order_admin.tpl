...
<thead>
	<tr>
    	<th bgcolor="#ededed" class="textCenter" style="text-align:center; font:16px Verdana;">Деталізація замовлення #<?php echo $order_id; ?></th>
   	</tr>
</thead>
<tbody>
	<tr>
			...
			<tbody>
				<tr>
			    	<td width="60%" valign="top">
							<b>Номер замовлення:</b> <?php echo $order_id; ?><? if ($changed>0) { ?>-<? echo $changed; ?><? } ?>							
							<br /><b>Дата замовлення</b>: <?php echo $date_added; ?>
							<?php if($payment_method){ ?>	
								<br /><b>Спосіб оплати:</b> <?php echo $payment_method; ?>
							<? } ?>	
							<?php if ($shipping_method) { ?>					
								<br /><b>Спосіб доставки:</b> <?php echo $shipping_method; ?>
							<? } ?>
			        <td width="40%" valign="top">
						...
						<? if (isset($fax) && $fax) { ?>
						<br /><b>Дод. телефон:</b> <a href="tel:<?php echo $fax; ?>"><?php echo $fax; ?></a>
						<? } ?>						
...
<tbody>
    ...
    <tr>
		...
		<?php if($config['table_quantity']){ ?>
			<td bgcolor="<?php echo $row_style_background; ?>" align="center" class="textCenter" style="word-break: normal">Немає в наявності</td>
...
<? if ($payment_will_be_done_on_delivery) { ?>
	<span style="font-size:16px;">Будь ласка, підтвердіть замовлення на умовах даної пропозиції:</span>
<? } else { ?>
	<span style="font-size:16px;">Підтвердити та сплатити замовлення на умовах даної пропозиції:</span>
<? } ?>
<br /><br />
<a style="text-align:center; background-color:#DEDEDE; color:#363636; border:1px solid #363636; font-size:14px; font-weight:700; padding:10px 30px; border-radius:5px;" href="<? echo $customer_confirm_url; ?>">
	...
	<? } else { ?>
		<? if (isset($only_full_payment) && $only_full_payment) { ?>
			Підтвердити та сплатити замовлення
		<? } else { ?>
			Підтвердити та сплатити замовлення повністю або частково
		<? } ?>
	<? } ?>
</a>
</div><br /><br />
