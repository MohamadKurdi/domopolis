			...
			foreach ($data as $order_info) {
				...
				foreach ($order_info['package'] as $package_id => $package_info) {
					...
					foreach ($package_info['item'] as $item) {
						...
						$xml .= '<Item WareKey="' . $item['ware_key'] . '" Cost="' . $this->normalizePrice($item['cost']) . '" Payment="' . $this->normalizePrice($item['payment']) . '" Weight="' . (float)$item['weight'] . '" Amount="' . (int)$item['amount'] . '" Comment="' . htmlspecialchars($item['comment']) . '" />';
					}
