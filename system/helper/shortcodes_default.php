...
class ShortcodesDefault extends Controller {
   ...
   function module_filter_module($atts) {
      extract($this->shortcodes->shortcode_atts(array(
         'filter_sort_order' => false,
		 'filter_group' => false
      ), $atts));
      
         $module = $this->getChild('common/module_filter', array(
                     'filter_sort_order' => $filter_sort_order,
                     'filter_group'  => $filter_group,
                  ));
         
         $html = '<div class="shortcode-module sc-module_filter">' . $module . '</div>';
         
         return $html;
   }
   
   function collection_title($atts) {
		...
		if ($title) {
			$html = '<h1 class="headline-collection" >'.str_replace('_newline_', '<br />', $title).'</h1>';
		} else {
			return '';
		}
   ...
   function responsive_block_with_text_and_boobs($atts) {
		
		extract($this->shortcodes->shortcode_atts(array(		
			'title'  => '',
			'orientation'  => 'layout-left',
			'image' => '',
			'imagehref' => '',
			'text' => '',
			'buttonhref' => '',
		    ...
		    $module = $this->getChild('blocks/imagetextblock', array(
                     'title'  => $title,
         ...
         $html = '<div class="shortcode-module sc-' . md5($title) . '">' . $module . '</div>';
         
        return $html;
   }
   
   function text_block_toggle($atts, $content = '') {
      extract($this->shortcodes->shortcode_atts(array(     
		 'button'   => '',
      ), $atts));
      ...
      if ($content) {         
         ...
         $html .= '<div class="shortcode-module sc-' . md5($content) . '"><div class="category-info">' . $content . '</div></div>';
		 $html .= "<script>openTextCreate('buttonOpenText', '.category-info', 100);</script>";
		 $html .= '<div style="clear:both;"></div>';
   ...
   function module_collection_list($atts) {
      extract($this->shortcodes->shortcode_atts(array(
         'list'   => '',
		 'title'  => '',
		 'image_height'  => 242,
	  ...
	  $collections = explode(',', $list);
	  
      if (is_array($collections) && $collections) {
         $module = $this->getChild('module/collection_list', array(
                     'list'   => $collections,
                     'title'  => str_replace('_newline_', '<br />', $title),
	...
	function module_category_list($atts) {
      extract($this->shortcodes->shortcode_atts(array(
         'list'   => '',
		 'title'  => '',
		 'manufacturer_id' => 0,
      ...
      if (is_array($categories) && $categories) {
         $module = $this->getChild('module/category_list', array(
                     'list'   => $categories,
   ...
   function module_product_list($atts) {
      extract($this->shortcodes->shortcode_atts(array(
         'list'   => '',
		 'title'  => ''
      ), $atts));
      ...
      if (is_array($products) && $products) {
         $module = $this->getChild('module/product_list', array(
                     'list'   => $products,
                     'title'  => $title,                     
                  ));
   ...
   function product_list_in_blog($atts) {
      extract($this->shortcodes->shortcode_atts(array(
         'list'   => ''
      ), $atts));
      ...
      if (is_array($products) && $products) {
         $module = $this->getChild('shortcode/product_list_in_blog', array(
                     'list'   => $products                    
                  ));
    ...
    function module_category_content($atts) {
		 extract($this->shortcodes->shortcode_atts(array(
			'category_id'   => false,	
		), $atts));
		 ...
		 if ($category_id) {
			...
			$_html = $this->getChild('product/category', array(
                     'path'   	=> (int)$category_id,
                     'no_shop'  => true,                     
                  ));
				  
			$html = '<div class="shortcode-module sc-' . md5($category_id) . '">' . $_html . '</div>';	  
