...
final class Installer
{
    ...
    {
        $dir_catalog_root = dirname(__FILE__) . DIRECTORY_SEPARATOR . '..' . DIRECTORY_SEPARATOR . '..' . DIRECTORY_SEPARATOR . '..' . DIRECTORY_SEPARATOR. '..' . DIRECTORY_SEPARATOR;
        $system_root = $dir_catalog_root . 'system' . DIRECTORY_SEPARATOR;
        $installed_file = $system_root . 'config.parts' . DIRECTORY_SEPARATOR . 'INSTALLED';

        echoLine('[Installer::_construct] Catalog root is: ' . $dir_catalog_root, 'd');
        echoLine('[Installer::_construct] System root is: ' . $system_root, 'd');
        echoLine('[Installer::_construct] INSTALLED file is: ' . $installed_file, 'd');
        ...
        if (is_file($installed_file)) {
            ...
            echoLine('[Installer::_construct] Installation-ready marker file not found', 's');
        }
    ...
    {
        $samples_catalog_root = dirname(__FILE__) . DIRECTORY_SEPARATOR . 'Samples' . DIRECTORY_SEPARATOR . 'Catalog' . DIRECTORY_SEPARATOR;
        $samples_admin_root = dirname(__FILE__) . DIRECTORY_SEPARATOR . 'Samples' . DIRECTORY_SEPARATOR . 'Admin' . DIRECTORY_SEPARATOR;
        $dir_catalog_root = dirname(__FILE__) . DIRECTORY_SEPARATOR . '..' . DIRECTORY_SEPARATOR . '..' . DIRECTORY_SEPARATOR . '..' . DIRECTORY_SEPARATOR. '..' . DIRECTORY_SEPARATOR;
        $dir_admin_root = dirname(__FILE__) . DIRECTORY_SEPARATOR . '..' . DIRECTORY_SEPARATOR . '..' . DIRECTORY_SEPARATOR . '..' . DIRECTORY_SEPARATOR. '..' . DIRECTORY_SEPARATOR . self::admin_directory . DIRECTORY_SEPARATOR;
        $system_root = $dir_catalog_root . 'system' . DIRECTORY_SEPARATOR;

        if (!is_file($dir_catalog_root . 'config.php')) {
            echoLine('[Installer::preinstall] Copying config.sample.php to config.php', 'w');
            copy($samples_catalog_root . 'config.sample.php', $dir_catalog_root . 'config.php');

            echoLine('[Installer::preinstall] Copying config.parts.sample directory', 'w');
            self::copyDirectory($samples_catalog_root . 'config.parts.sample', $system_root . 'config.parts', false);

            echoLine('[Installer::preinstall] Copying config.sample directory', 'w');
            self::copyDirectory($samples_catalog_root . 'config.sample', $system_root . 'config', false);
        } else {
            echoLine('[Installer::preinstall] Main config already exists', 's');
        }

        if (!is_file($dir_admin_root . 'config.php')) {
            echoLine('[preinstall] Copying admin config.sample.php to config.php', 'w');
            copy($samples_admin_root . 'config.sample.php', $dir_admin_root . 'config.php');

            echoLine('[preinstall] Copying admin config.parts.sample directory', 'w');
            self::copyDirectory($samples_admin_root . 'config.parts.sample', $dir_admin_root . 'config.parts', false);
        }  else {
    ...
    public function install(): void
    {
        ...
        require_once (DIR_SYSTEM . 'library' . DIRECTORY_SEPARATOR . 'db.php');
        ...
        if (defined('DB_USERNAME') && DB_USERNAME != 'DB_USERNAME'){
            ...
            if ($query->num_rows){
                ...
                if (file_exists(dirname(__FILE__) . DIRECTORY_SEPARATOR . 'SQL' . DIRECTORY_SEPARATOR . 'structure-demo.sql')){
                    echoLine('[Installer::install] Demo data exists, importing it', 'i');
                    $db->importSQL(dirname(__FILE__) . DIRECTORY_SEPARATOR . 'SQL' . DIRECTORY_SEPARATOR . 'structure-demo.sql');
                } else {
                    echoLine('[Installer::install] Demo data does not exist, importing just structure', 'i');
                    $db->importSQL(dirname(__FILE__) . DIRECTORY_SEPARATOR . 'SQL' . DIRECTORY_SEPARATOR . 'structure-main.sql');
                }
            }
    ...
    public function setdomain($domain): void
    {
        ...
        self::replaceStringInFiles(DIR_SYSTEM . 'config', self::default_domain, $domain);
        self::replaceStringInFiles(DIR_SYSTEM . 'config.parts', self::default_domain, $domain);
        self::replaceStringInFiles(DIR_SYSTEM . '..' . DIRECTORY_SEPARATOR . self::admin_directory . DIRECTORY_SEPARATOR . 'config.parts', self::default_domain, $domain);

        touch(DIR_SYSTEM . 'config.parts' . DIRECTORY_SEPARATOR . 'INSTALLED');
    }
