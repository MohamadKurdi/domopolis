...
namespace hobotix\Search\ElasticFramework;
...
class Indexer
{
    ...
    public function deleteIndexes()
    {
        ...
        $response = $this->registry->get('searchAdaptor')->connection()->indices()->delete($deleteParams);
    ...
    public function recreateEntitiesIndex()
    {
        ...
        try {
            $response = $this->registry->get('searchAdaptor')->connection()->indices()->create($entitiesIndex);
        } catch (\Exception $e) {
    ...
    public function recreateProductsIndex(){
        ...
        try {
            $response = $this->registry->get('searchAdaptor')->connection()->indices()->create($productsIndex);
        } catch (\Exception $e) {
            echoLine('[Indexer:exception]' . $e->getMessage(), 'e');
        }
    ...
    public function fillEntitiesIndex()
    {
        ...
        foreach ($query->rows as $category) {
            ...
            $params['body'] = [];
            $params['body']['priority'] = \hobotix\Search\Elastic::CATEGORY_PRIORITY;
            ...
            $response = $this->registry->get('searchAdaptor')->connection()->index($params);
        }
        ...
        if ($this->config->get('config_elasticseach_index_manufacturers')){
            ...
            foreach ($query->rows as $manufacturer) {
                ...
                $params['body'] = [];
                $params['body']['priority'] = \hobotix\Search\Elastic::MANUFACTURER_PRIORITY;
            ...
            foreach ($query->rows as $manufacturer) {
                ...
                if ($namequery2->num_rows) {
                    foreach ($namequery2->rows as $row) {
                        ...
                        $params['body'] = [];
                        $params['body']['priority'] = \hobotix\Search\Elastic::KEYWORDER_PRIORITY;
            ...
            foreach ($query->rows as $manufacturer) {
                ...
                if ($namequery2->num_rows) {
                    foreach ($namequery2->rows as $row) {
                        ...
                        $params['body'] = [];
                        $params['body']['priority'] = \hobotix\Search\Elastic::COLLECTION_PRIORITY;
    ...
    public function reindexProducts($products)
    {
        foreach ($products as $product_id) {
            $this->reindexProduct($product_id);
        }
    }
    ...
    public function deleteProduct($product_id)
    {
        ...
        if ($this->registry->get('searchAdaptor')->connection()) {
            try {
                echoLine('[Indexer::deleteProduct] Deleting product ' . $product_id . 'from index', 'w');
                $response = $this->registry->get('searchAdaptor')->connection()->delete($params);
