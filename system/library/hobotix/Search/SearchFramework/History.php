...
namespace hobotix\Search\SearchFramework;
...
class History{
    ...
    public function getSearchHistory(){
        ...
        if ($this->customer->isLogged()){
            ...
            if (!empty($this->session->data['customer_search_history'])){
                usort($this->session->data['customer_search_history'], 'date_added_sorter');
    ...
    public function clearSearchHistory($id = false){
        ...
        if ($this->customer->isLogged()){
            if ($id == 'all'){
                $this->db->non_cached_query("DELETE FROM customer_search_history WHERE customer_id = '" . (int)$this->customer->isLogged() . "'");
            } else {
                $this->db->non_cached_query("DELETE FROM customer_search_history WHERE customer_id = '" . (int)$this->customer->isLogged() . "' AND customer_history_id = '" . (int)$id . "'");
            }
        } else {
    ...
    public function writeSearchHistory($search_history, $results = 0){
        ...
        $this->db->non_cached_query("INSERT INTO search_history SET text = '" . $this->db->escape($search_history) . "', times = 1, results = '" . (int)$results . "' ON DUPLICATE KEY UPDATE times = times + 1, results = '" . (int)$results . "'");

        if ($this->customer->isLogged()){

            $this->db->non_cached_query("INSERT INTO customer_search_history SET customer_id = '" . (int)$this->customer->isLogged() . "', text = '" . $this->db->escape($search_history) . "', date_added = NOW() ON DUPLICATE KEY UPDATE date_added = NOW()");

            if (!empty($this->session->data['customer_search_history'])){

                foreach ($this->session->data['customer_search_history'] as $history){
                    $this->db->non_cached_query("INSERT IGNORE INTO customer_search_history SET customer_id = '" . $this->customer->isLogged() . "', text = '" . $this->db->escape($history['text']) . "', date_added = '" . date('Y-m-d H:i:s', strtotime($history['date_added'])) . "'");
                }
