...
class RainforestClient
{
    ...
    /** @var bool $debugHttp      Set true to enable httpclient debug */
    private $debugHttp = false;
    /** @var int $httpTimeout      timeout for requests */
    private $httpTimeout = 120;
    /** @var \Registry for backward connecting to OpenCart framework */
    private $externalRegistry = null;
    ...
    public function __construct($config, LoggerInterface $logger = null) {
        ...
        if (!empty($config['http_timeout'])) {
            $this->httpTimeout = (int)$config['http_timeout'];
        }

        if (!empty($config['debug_http'])) {
            $this->debugHttp = (bool)$config['debug_http'];
        }
    ...
    public function addExternalRegistry(\Registry $registry){
    ...
    {
        $client = new Client(['debug' => $this->debugHttp, 'timeout' => $this->httpTimeout]);
        $promiseRequests = [];
        $rfResponses = [];
        ...
        foreach ($responses as $key => $response) {
            ...
            try {
            ...
            } catch (\GuzzleHttp\Exception\GuzzleException | \GuzzleHttp\Exception\ConnectException  $e) {
                $this->logMessage('GuzzleHttp Error caught', LoggerService::DEBUG);
                ...
                if ($this->externalRegistry){                    
                    $externalRainforestAmazon = $this->externalRegistry->get('rainforestAmazon');
                    if (!empty($externalRainforestAmazon)){
                        $externalRainforestAmazon->parseException($e);
                    }
                }
            } catch (\Exception $e) {
    ...
    private function validateResponseAndReturnData($response, $responseClass) {
        ...
        foreach (forward_static_call([$responseClass, 'getMainKeys']) as $key) {
            ...
            } elseif (isset($dataArray[$key]) && empty($dataArray[$key])){
            //  $this->logMessage("Dump of data object returned from Rainforest:", LoggerService::DEBUG);
            //  $this->logMessage(print_r($dataArray, true), LoggerService::DEBUG);
                $this->logMessage("Rainforest response may be buggy. JSON is parsed, it has set, but empty $key", LoggerService::DEBUG);               
            }
        }
