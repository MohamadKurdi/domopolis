	...
	class User {
		...
		public function __construct($registry) {
			...
			if (isset($this->session->data['user_id'])) {
				...
				if ($user_query->num_rows) {
					...
					$user_group_store_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "user_group_to_store WHERE user_group_id = '" . (int)$user_query->row['user_group_id'] . "'");
		...
		public function login($username, $password) {
			...
			if ($this->config->get('config_ldap_auth_enable')){
				...
				//попытка аутентификации в домене
				if (is_resource($connection)){
					...
					if($bind = @ldap_bind($ldap, $username . '@' . $this->ldap_domain, $password)){
						...
						if ($exists_query->rows){
							...
							//пользователь в базе не существует, создаем
							$_data = array(
							'username'  => $username,
							'firstname' => $firstname,
							'lastname'  => $lastname,
						...
						if($this->config->get('adminlog_enable') && $this->config->get('adminlog_hacklog')){
							$this->db->query("INSERT INTO " . DB_PREFIX . "adminlog SET user_id = '" . (int)$this->user_id . "', `user_name` = '" . $username . "', `action` = 'ldap_login', `allowed` = '1', `url` = '".$this->db->escape($this->db->escape($this->request->server['REQUEST_URI']))."', `ip` = '" . $this->request->server['REMOTE_ADDR'] . "', date = NOW()");																		
						}										
						
						} else {
						//коннект прошел, аутентификация не удалась
						$this->db->query("UPDATE user SET status = 0 WHERE username = '" . $username . "'");
						
						$next_user_id = 0;
						
						if($this->config->get('adminlog_enable') && $this->config->get('adminlog_hacklog')){
							$this->db->query("INSERT INTO " . DB_PREFIX . "adminlog SET user_id = '" . (int)$this->user_id . "', `user_name` = '" . $username . "', `action` = 'ldap_login', `allowed` = '0', `url` = '".$this->db->escape($this->db->escape($this->request->server['REQUEST_URI']))."', `ip` = '" . $this->request->server['REMOTE_ADDR'] . "', date = NOW()");																		
						}
					...
					//если нет соединения с директорией - попытаемся авторизоваться как бы внутри	
					} else {
					
					$user_check_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "user WHERE username = '" . $this->db->escape($username) . "' AND (password = SHA1(CONCAT(salt, SHA1(CONCAT(salt, SHA1('" . $this->db->escape($password) . "'))))) OR password = '" . $this->db->escape(md5($password)) . "') AND status = '1'");
					...
					if ($user_check_query->num_rows) {
						...
						if($this->config->get('adminlog_enable') && $this->config->get('adminlog_hacklog')){
							$this->db->query("INSERT INTO " . DB_PREFIX . "adminlog SET user_id = '" . (int)$this->user_id . "', `user_name` = '" . $username . "', `action` = 'login', `allowed` = '0', `url` = '".$this->db->escape($this->request->server['REQUEST_URI'])."', `ip` = '" . $this->request->server['REMOTE_ADDR'] . "', date = NOW()");
						}
			...
			$user_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "user WHERE user_id = '" . (int)$next_user_id . "'");
			...
			if ($user_query->num_rows) {
				...
				if($this->config->get('adminlog_enable') && $this->config->get('adminlog_login')){
					$this->db->query("INSERT INTO " . DB_PREFIX . "adminlog SET user_id = '" . (int)$this->user_id . "', `user_name` = '" . $this->username . "', `action` = 'login', `allowed` = '1', `url` = '".$this->db->escape($this->db->escape($this->request->server['REQUEST_URI']))."', `ip` = '" . $this->request->server['REMOTE_ADDR'] . "', date = NOW()");
				}
		...
		public function logout() {
			...
			if($this->config->get('adminlog_enable') && $this->config->get('adminlog_logout')){
				$this->db->query("INSERT INTO " . DB_PREFIX . "adminlog SET user_id = '" . (int)$this->user_id . "', `user_name` = '" . $this->username . "', `action` = 'logout', `allowed` = '1', `url` = '".$this->db->escape($this->request->server['REQUEST_URI'])."', `ip` = '" . $this->request->server['REMOTE_ADDR'] . "', date = NOW()");
			}
		...
		public function hasPermission($key, $value) {
			...
			if (isset($this->permission[$key])) {
	    			...
	    			( ($this->config->get('adminlog_allowed') == 0 || $this->config->get('adminlog_allowed') == 2) && !(in_array($value, $this->permission[$key])) )  ){
						if(($this->config->get('adminlog_access') && $key == "access") || ($this->config->get('adminlog_modify') && $key == "modify") ){
							...
							if (!$this->excludeFromAdminLog($this->request->server['REQUEST_URI'])) {
								...
								$this->db->query("INSERT INTO " . DB_PREFIX . "adminlog SET user_id = '" . (int)$this->user_id . "', `user_name` = '" . $this->username . "', `action` = '".$key."', `allowed` = '".in_array($value, $this->permission[$key])."', `url` = '".$this->db->escape($this->request->server['REQUEST_URI'])."', `ip` = '" . $this->request->server['REMOTE_ADDR'] . "', date = NOW()");
				...
				if($this->config->get('adminlog_enable') && ($this->config->get('adminlog_allowed') == 0 || $this->config->get('adminlog_allowed') == 2)){
					$this->db->query("INSERT INTO " . DB_PREFIX . "adminlog SET user_id = '" . (int)$this->user_id . "', `user_name` = '" . $this->username . "', `action` = '".$key."', `allowed` = '0', `url` = '".$this->db->escape($this->request->server['REQUEST_URI'])."', `ip` = '" . $this->request->server['REMOTE_ADDR'] . "', date = NOW()");
				}
